...
const DEBUG = (() => {
  try {
    return localStorage.getItem('plagues-call.debug') === '1';
  } catch {
    return false;
  }
})();
...
const CONDITION_GROUPS = {
  negative: ['Blinded', 'Charmed', 'Deafened', 'Frightened', 'Grappled', 'Incapacitated', 'Paralyzed', 'Petrified', 'Poisoned', 'Prone', 'Restrained', 'Slowed', 'Stunned', 'Unconscious', 'Exhausted', 'On Fire', 'Bleeding', 'Cursed', 'Marked', 'Diseased'],
  positive: ['Blessed', 'Hasted', 'Raging', 'Guidance', 'Inspired', 'Shielded', 'Resistant', 'Invisible'],
  neutral: ['Concentrating', 'Hidden', 'Dead', 'Broken']
};
const CONDITIONS = [...CONDITION_GROUPS.negative, ...CONDITION_GROUPS.positive, ...CONDITION_GROUPS.neutral];
...
const DAMAGE_TYPES = ['Slashing', 'Piercing', 'Bludgeoning', 'Fire', 'Cold', 'Acid', 'Lightning', 'Thunder', 'Poison', 'Necrotic', 'Radiant', 'Psychic', 'Force'];
...
const DEFAULT_SETTINGS = {
  ...
  obfuscateHp: false,
  // DM: show health estimates instead of exact numbers to players
  moveRangeOpacity: 0.55,
  // v7.8: opacity of the combat movement-range markers
  grain: true,
  // v7.8: subtle film-grain texture on menu/panel surfaces
  physicalDice: false,
  // v8.3: enter real-life dice results instead of auto-rolling
...
const TUNING = {
  // Sync & authority
  pushSoonMs: 60,
  // delay before a targeted post-action state push to one player
  claimResendMs: 2500,
  // onboarding: re-send a pending claim until the DM confirms it
  claimGiveUpMs: 12000,
  // onboarding: stop the spinner if a claim never confirms
  connectTimeoutMs: 20000,
  // player↔DM peer connection attempt timeout
  // Chat
  chatMaxMessages: 250,
  // most-recent messages retained in synced state
  chatMaxChars: 600,
  // per-message character cap
  // On-map measurement
  measureLingerMs: 10000,
  // total life of a lingering measurement (full-visible + fade-out)
...
const BUILTIN_TOKEN_PRESETS = [...BNB_TOKEN_PRESETS, {
  id: 'builtin:goblin',
  name: 'Goblin',
  builtin: true,
  entity: {
...
{
  id: 'builtin:unfinished_puppet',
  name: 'Unfinished Puppet',
  builtin: true,
  entity: {
    type: 'Monster',
    name: 'Unfinished Puppet',
...
He has not tried to leave. Ask him why.`
  ...
  id: 'builtin:ernest_broken',
  name: 'Ernest the Broken Puppet',
  builtin: true,
  entity: {
    type: 'Monster',
    name: 'Ernest',
...
Does not open doors. Does not cross running water. Does not stop moving.`
  ...
  id: 'builtin:laughing_puppet',
  name: 'The Laughing Puppet',
  builtin: true,
  entity: {
    type: 'Monster',
    name: 'The Laughing Puppet',
...
function initiativeWithEntity(initiative, entities, entityId, rollValue) {
  ...
  const r = typeof rollValue === 'number' && isFinite(rollValue) ? rollValue : roll(20) + (ent.initBonus || 0);
...
function walkSpeedOf(e) {
  const w = e?.speeds?.walk;
  return w != null && w !== '' ? Number(w) || 0 : e?.speed > 0 ? e.speed : 30;
}
function flySpeedOf(e) {
  return Math.max(0, Number(e?.speeds?.fly) || 0);
}
function jumpSpeedOf(e) {
  return Math.max(0, Number(e?.speeds?.jump) || 0);
}
function swimSpeedOf(e) {
  return Math.max(0, Number(e?.speeds?.swim) || 0);
}
function climbSpeedOf(e) {
  return Math.max(0, Number(e?.speeds?.climb) || 0);
}
...
function damagingHazardsAt(state, mapId, x, y) {
  ...
  return list.filter(h => h && h.damage && (h.damage.perTurn || h.damage.onEntry) && Array.isArray(h.points) && pointInPoly(x, y, h.points));
}
...
function movementReset(state, initiative) {
  ...
  const tok = Object.values(state.tokens || {}).find(t => t.entityId === eid && t.mapId === state.currentMapId);
  const base = {
    entityId: eid,
    usedFt: 0,
    mode: 'walk',
...
const makeDefaultState = () => {
  ...
  return {
    ...
    playerThemes: {},
    // v7.6: DM-pushed UI theme per peerId -> { theme, ts }
    chat: [],
    // v7.6: synced chat log [{id,ts,senderId,senderName,text,whisperTo,whisperToName}]
    mapScale: 1.0,
...
const makeEntity = (overrides = {}) => ({
  ...
  passivePerception: 10,
  passiveHiding: 0,
  // v7.5: stealth threshold; players see this token only if their passive perception >= this (0 = always visible)
  conditions: [],
  notes: '',
  playerDescription: '',
  imageUrl: null,
  ...
  alignment: '',
  attacks: '',
  // weapons & attacks (free text - legacy)
  weapons: [],
  // v7.9: structured weapons [{id,name,equipped,attacks:[{id,name,toHit,range,damage:[{count,sides,modifier,type}]}]}]
  spells: '',
  // spell list / spellcasting notes
  features: '',
  // features & traits
  proficiencies: '',
...
function presetAssetSlugs(preset) {
  ...
  const cands = [suffix, slugify(suffix), slugify(preset.name), slugify(preset.entity?.name), cased(preset.name), cased(preset.entity?.name)];
...
function migrateState(raw) {
  ...
  state.movement = state.movement && typeof state.movement === 'object' ? state.movement : {
    entityId: null,
    startX: 0,
    startY: 0,
    usedFt: 0
...
function reducer(state, action) {
  switch (action.type) {
    ...
    case 'REPLACE':
      {
        ...
        for (const [mid, list] of Object.entries(incomingLayers)) {
          ...
          mergedLayers[mid] = (Array.isArray(list) ? list : []).map(l => {
            if (l?.imageUrl === IMG_SENTINEL && curById[l.id]?.imageUrl && curById[l.id].imageUrl !== IMG_SENTINEL) {
              return {
                ...l,
                imageUrl: curById[l.id].imageUrl
              };
            }
            return l;
          });
    ...
    case 'LAYER_IMAGE_RECEIVED':
      {
        ...
        return {
          ...
          layers: {
            ...state.layers,
            [action.mapId]: list.map(l => l.id === action.layerId ? {
              ...l,
    ...
    case 'LAYER_DELETE':
      {
        ...
        return {
          ...
          layers: {
            ...state.layers,
            [mid]: list.filter(l => l.id !== action.id)
          }
        };
      }
    ...
    case 'INIT_SET':
      {
        ...
        return {
          ...state,
          initiative,
          movement: movementReset(state, initiative)
        };
      }
    ...
    case 'INIT_ADD':
      return {
        ...state,
        initiative: initiativeWithEntity(state.initiative, state.entities, action.entityId, action.roll)
      };
    case 'INIT_ADVANCE':
      {
        const {
    ...
    case 'MOVEMENT_USE':
      {
        ...
        return {
          ...
          movement: {
            ...mv,
            usedFt: (mv.usedFt || 0) + Math.max(0, action.addFt || 0)
          }
        };
      }
    ...
    case 'MOVEMENT_MODE':
      {
        ...
        const ent = state.entities[mv.entityId];
        const walk = walkSpeedOf(ent),
          fly = flySpeedOf(ent),
          jump = jumpSpeedOf(ent);
        if (action.mode === 'dash') {
        ...
        if (action.mode === 'jump') {
          ...
          return {
            ...
            movement: {
              ...
              jumpPending: true,
              preMode: mv.mode,
              preJumpUsedFt: mv.usedFt || 0,
              budgetFt: (mv.usedFt || 0) + jump
            }
          };
        }
        return state;
    ...
    case 'GRANT_PC_CONTROL':
      {
        ...
        return {
          ...
          claims: {
            ...
            [action.peerId]: {
              ...prev,
              controlledPcs: [...list, action.entityId]
            }
          }
        };
      }
    case 'REVOKE_PC_CONTROL':
      {
        ...
        return {
          ...
          claims: {
            ...
            [action.peerId]: {
              ...prev,
              controlledPcs: (prev.controlledPcs || []).filter(id => id !== action.entityId)
            }
          }
        };
      }
    ...
    case 'HAZARD_QUEUE':
      {
        ...
        const exists = (state.hazardPending || []).some(e => e.tokenId === action.event.tokenId && e.hazardId === action.event.hazardId && e.reason === action.event.reason);
        if (exists) return state;
        return {
          ...state,
          hazardPending: [...(state.hazardPending || []), action.event]
        };
      }
    case 'HAZARD_PENDING_REMOVE':
      return {
        ...state,
        hazardPending: (state.hazardPending || []).filter(e => e.id !== action.id)
      };
      ...
      this.peer.on('connection', conn => {
        ...
        conn.on('data', data => {
          ...
          try {
            ...
            console.warn('[plagues-call] dropped malformed message', e?.message);
          }
        });
        conn.on('close', () => {
    ...
    try {
      ...
      this.peer.on('open', id => {
        ...
        conn.on('data', data => {
          try {
            if (!data || typeof data !== 'object') return;
            if (data.type === 'state_update') this.onStateUpdate?.(data.payload);else if (data.type === 'awaiting_approval') this.onAwaitingApproval?.();else if (data.type === 'map_image') this.onMapImage?.(data.mapId, data.dataUrl, data.layerId);else if (data.type === 'token_pos') this.onTokenPos?.(data.tokenId, data.x, data.y, data.mapId);else if (data.type === 'sound_data') this.onSoundData?.(data.soundId, data.name, data.dataUrl);else if (data.type === 'kicked') {
...
function bandHpForPlayer(hp) {
  ...
  let c;
  if (cur <= 0) c = 0;else if (pct < 0.30) c = 15;else if (pct <= 0.70) c = 50;else c = 85;
  return {
    current: c,
    max: 100
  };
...
function filterStateForPlayer(state, peerId, obfuscateHp = false) {
  ...
  if (claim.spectator) {
    ...
    for (const id of specVisibleIds) {
      ...
      if (e) sanitizedEntities[id] = sanitizeEntityForPlayer(e, {
        obfuscateHp,
        isOwned: false
      });
    }
    ...
    return {
      ...
      initiative: {
        ...state.initiative,
        entries: specInitEntries
      },
      reminders: {
        [peerId]: state.reminders?.[peerId] || []
      },
  ...
  finalTokens = Object.fromEntries(Object.entries(finalTokens).filter(([, t]) => t.mapId === effectiveMapId));
...
function stripHeavyAssetsForWire(state) {
  ...
  const leanLayers = {};
  for (const [mid, list] of Object.entries(state.layers || {})) {
    leanLayers[mid] = (Array.isArray(list) ? list : []).map(l => l?.imageUrl && typeof l.imageUrl === 'string' && l.imageUrl.startsWith('data:') ? {
      ...l,
      imageUrl: IMG_SENTINEL
    } : l);
  }
...
const syncStatusClass = status => status === 'live' ? 'live' : status === 'connecting' ? 'connecting' : status === 'error' ? 'error' : '';
...
function AuthScreen({
  ...
  }, "Burrows and Badgers"), /*#__PURE__*/React.createElement("div", {
...
function TokenView({
  ...
  useEffect(() => {
    ...
    for (const c of curConds) {
      if (!prevSet.has(c)) adds.push({
        kind: 'status',
        color: CONDITION_COLORS[c] || '#9b6ac4'
      });
    }
    if (adds.length) {
      setFlashes(f => [...f, ...adds.map(a => ({
        ...a,
        id: ++fxIdRef.current
      }))]);
      ...
      const rtype = adds.some(a => a.kind === 'damage') ? 'hit' : adds.some(a => a.kind === 'heal') ? 'heal' : 'buff';
      setReaction(null);
  ...
  useEffect(() => {
    ...
    if (entity && !entity.imageUrl && entity.name) {
      resolveEntityImage(entity.name).then(url => {
        if (live && url) setNameImg(url);
      });
    } else {
      setNameImg(null);
    }
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    className: `token-inner ${reaction ? 'tok-react-' + reaction : ''}`
  ...
  }, portraitUrl ? /*#__PURE__*/React.createElement("img", {
    src: portraitUrl,
    alt: "",
  ...
  }) : /*#__PURE__*/React.createElement("span", null, initial), flashes.map(f => /*#__PURE__*/React.createElement("div", {
    key: f.id,
    className: `token-fx token-fx-${f.kind}`,
    style: {
      '--fx-color': f.color
    },
    onAnimationEnd: () => setFlashes(cur => cur.filter(x => x.id !== f.id))
  }))), passivelyHidden && /*#__PURE__*/React.createElement("div", {
    className: "token-hiding-badge",
    title: `Hidden from players with passive perception below ${hideThreshold}`
  }, /*#__PURE__*/React.createElement("span", {
    className: "thb-glyph"
  }, "◐"), hideThreshold), showLabel && /*#__PURE__*/React.createElement("div", {
...
function MapCanvas({
  ...
  onDrawingUpsert,
  onDrawingDelete,
  // v7.5: (drawingId) - erase one drawing
  ...
  const renderMeasureSvg = (m, fading, keyId) => {
    ...
    return /*#__PURE__*/React.createElement("svg", {
      key: keyId,
      className: `measure-overlay${fading ? ' measure-fade' : ''}`,
  ...
  const startInertia = useCallback((vx, vy) => {
    ...
    const step = nowT => {
      ...
      vx *= decay;
      vy *= decay;
      if (Math.hypot(vx, vy) > 0.02) inertiaRef.current = requestAnimationFrame(step);else inertiaRef.current = null;
    };
  ...
  useEffect(() => {
    ...
    const onMove = e => {
      ...
      if (last && now > last.t) {
        ...
        const ivx = (e.clientX - last.x) / dtv,
          ivy = (e.clientY - last.y) / dtv;
        panVelRef.current = {
          x: e.clientX,
          y: e.clientY,
          t: now,
    ...
    const onUp = e => {
      ...
      if (e.pointerType === 'touch' && vel && moved >= 8 && performance.now() - vel.t < 80 && Math.hypot(vel.vx, vel.vy) > 0.05) {
  ...
  useEffect(() => {
    ...
    return () => {
      if (_touchDropTarget === target) _touchDropTarget = null;
    };
  }, []);
  const startTokenDrag = (tokenId, e) => {
    ...
    const isMyActiveTurn = mode === 'player' && initiative?.active && initiative.entries[initiative.turn]?.entityId === token.entityId && (ownedEntityIds ? ownedEntityIds.has(token.entityId) : claimedEntityId === token.entityId);
    ...
    const isDmActiveTurn = mode === 'dm' && initiative?.active && initiative.entries[initiative.turn]?.entityId === token.entityId && ent && ent.type !== 'PC' && ent.type !== 'Familiar';
  ...
  useEffect(() => {
    ...
    const onMove = e => {
      ...
      if (ref.clamp) {
        const dx = x - ref.clamp.originX,
          dy = y - ref.clamp.originY;
  ...
  const moveMarker = (() => {
    ...
    const tok = (tokens ? Object.values(tokens) : []).find(t => t.entityId === currentInitEntityId && t.mapId === map?.id);
    if (!tok) return null;
  ...
  const visibleTokens = useMemo(() => Object.values(tokens).filter(t => {
  ...
  const startLayerDrag = useCallback((layer, e) => {
    ...
    const onMove = ev => {
      ...
      if (d.mode === 'move') {
        ...
        next = {
          ...
          w: d.w,
          h: d.h,
          rotation: d.startRot + (ang - d.startAngle)
        };
      }
    ...
    const onUp = () => {
      ...
      const target = {
        ...
        x: Math.round(cur.x),
        y: Math.round(cur.y),
        rotation: Math.round(cur.rotation),
        w: cur.w,
        h: cur.h
      };
      // commit (DM applies instantly; a player's commit round-trips to the DM)
      if (d.mode === 'move') onLayerTransform(d.id, {
        x: target.x,
        y: target.y
      });else onLayerTransform(d.id, {
  ...
  return /*#__PURE__*/React.createElement("div", {
  ...
  }), moveMarker && moveRangeOpacity > 0.01 && /*#__PURE__*/React.createElement("div", {
    className: "move-range-group",
    style: {
      opacity: moveRangeOpacity
    }
  }, /*#__PURE__*/React.createElement("div", {
    className: "move-range move-range-max",
    style: {
      left: moveMarker.startX - moveMarker.maxR,
      top: moveMarker.startY - moveMarker.maxR,
      width: moveMarker.maxR * 2,
      height: moveMarker.maxR * 2
    }
  }), moveMarker.remR > 0 && /*#__PURE__*/React.createElement("div", {
    className: "move-range move-range-rem",
    style: {
      left: moveMarker.curX - moveMarker.remR,
      top: moveMarker.curY - moveMarker.remR,
      width: moveMarker.remR * 2,
      height: moveMarker.remR * 2
    }
  }, /*#__PURE__*/React.createElement("span", {
    className: "move-range-label"
  }, Math.round(moveMarker.remFt), " ft", moveMarker.mode === 'fly' ? ' · fly' : moveMarker.mode === 'swim' ? ' · swim' : moveMarker.mode === 'climb' ? ' · climb' : moveMarker.mode === 'jump' ? ' · jump' : moveMarker.dashed ? ' · dash' : ''))), !layerHidden('tokens') && visibleTokens.map(t => {
    const ent = entities[t.entityId];
    if (!ent) return null;
    ...
    const patch = p => onReminderUpsert?.({
      ...er,
      ...p
    });
    const setColor = c => {
      ...
      setReminderDefault(d => ({
        ...d,
        color: c
      }));
    };
    const setSize = s => {
      const v = clamp(Number(s) || 1, REMINDER_SIZE_MIN, REMINDER_SIZE_MAX);
      patch({
        size: v
      });
      setReminderDefault(d => ({
    ...
    return ReactDOM.createPortal(/*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
      className: "reminder-edit-backdrop",
      onPointerDown: () => setEditingReminder(null)
    }), /*#__PURE__*/React.createElement("div", {
      className: "reminder-edit-pop",
      style: {
        left,
        top
      },
      ...
      className: "reminder-edit-head"
    }, /*#__PURE__*/React.createElement("span", {
      style: {
        color: er.color
      }
    }, "◆"), " Reminder", /*#__PURE__*/React.createElement("button", {
      className: "reminder-edit-x",
      onClick: () => setEditingReminder(null)
    }, "×")), /*#__PURE__*/React.createElement("input", {
      className: "reminder-edit-label",
      value: er.label,
      ...
      className: "reminder-edit-row-label"
    }, "Colour"), /*#__PURE__*/React.createElement("div", {
      className: "reminder-swatches"
    }, REMINDER_PALETTE.map(c => /*#__PURE__*/React.createElement("button", {
      key: c,
      className: `reminder-swatch ${er.color === c ? 'active' : ''}`,
      style: {
        background: c
      },
      onClick: () => setColor(c),
      ...
      className: "reminder-size-val"
    }, Math.round(rsize * 100), "%")), /*#__PURE__*/React.createElement("input", {
      className: "reminder-size-slider",
      type: "range",
      min: REMINDER_SIZE_MIN,
      max: REMINDER_SIZE_MAX,
      step: "0.1",
      value: rsize,
      onChange: e => setSize(e.target.value)
    }), /*#__PURE__*/React.createElement("div", {
      className: "reminder-edit-actions"
    }, /*#__PURE__*/React.createElement("button", {
      className: "btn sm ghost danger",
      onClick: () => {
        onReminderDelete?.(er.id);
      ...
      className: "reminder-edit-hint"
    }, "New pins use this colour & size."))), document.body);
  })(), !layerHidden('walls') && (blockZones.length > 0 || drawingBlock || drawingPoly || drawingCircle) && /*#__PURE__*/React.createElement("svg", {
    ...
    if (isPoly) {
      return /*#__PURE__*/React.createElement("polygon", {
        ...
        points: z.points.map(([x, y]) => `${x},${y}`).join(' '),
        ...dmProps
      }, /*#__PURE__*/React.createElement("title", null, "Double-click to delete"));
    }
    if (isCircle) {
      return /*#__PURE__*/React.createElement("circle", {
        key: z.id,
    ...
    strokeWidth: "2",
    strokeDasharray: "4 4"
  })), !layerHidden('drawings') && (drawings.length > 0 || drawingNow) && /*#__PURE__*/React.createElement("svg", {
    ...
    return null;
  })()), !layerHidden('hazards') && hazards.length > 0 && /*#__PURE__*/React.createElement("svg", {
    ...
    return /*#__PURE__*/React.createElement("polygon", {
      ...
      ...style,
      ...dmHandlers
    }, /*#__PURE__*/React.createElement("title", null, h.hazardKind, h.visible === false ? ' (hidden)' : '', h.label ? ` - ${h.label}` : ''));
  ...
  })(), measuring && renderMeasureSvg({
    ...measuring,
    isRadius: measureMode === 'radius'
  }, false, 'live'), lingerMeasure && renderMeasureSvg(lingerMeasure, true, lingerMeasure.key), mode === 'player' && visionEnabled && visionSources.length > 0 && (() => {
...
function ConditionPicker({
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    return /*#__PURE__*/React.createElement("div", {
      ...
      type: "button",
      className: `cond-section-head cond-sec-${key} ${isOpen ? 'open' : ''}`,
      onClick: () => toggleOpen(key)
    }, /*#__PURE__*/React.createElement("span", {
      className: "cond-sec-chevron"
    }, isOpen ? '▾' : '▸'), /*#__PURE__*/React.createElement("span", {
      className: "cond-section-label"
    }, label), activeCount > 0 && /*#__PURE__*/React.createElement("span", {
      className: "cond-sec-count"
    }, activeCount)), isOpen && /*#__PURE__*/React.createElement("div", {
      className: "cond-grid"
...
function EntityForm({
  ...
  return /*#__PURE__*/React.createElement("div", {
  ...
  })), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("label", null, "Walk speed"), /*#__PURE__*/React.createElement("input", {
    type: "number",
    value: walkSpeedOf(entity),
    onChange: e => {
    ...
    className: "form-row-3"
  }, /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("label", null, "Fly speed"), /*#__PURE__*/React.createElement("input", {
    type: "number",
    value: entity.speeds?.fly ?? 0,
    onChange: e => update({
      speeds: {
        ...(entity.speeds || {}),
        fly: Number(e.target.value) || 0
      }
    })
  })), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("label", null, "Jump speed"), /*#__PURE__*/React.createElement("input", {
    type: "number",
    value: entity.speeds?.jump ?? 0,
    onChange: e => update({
      speeds: {
        ...(entity.speeds || {}),
        jump: Number(e.target.value) || 0
      }
    })
  })), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("label", null, "Swim speed"), /*#__PURE__*/React.createElement("input", {
    type: "number",
    value: entity.speeds?.swim ?? 0,
    onChange: e => update({
      speeds: {
        ...(entity.speeds || {}),
        swim: Number(e.target.value) || 0
      }
    })
  })), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("label", null, "Climb speed"), /*#__PURE__*/React.createElement("input", {
    type: "number",
    value: entity.speeds?.climb ?? 0,
    onChange: e => update({
      speeds: {
        ...(entity.speeds || {}),
        climb: Number(e.target.value) || 0
      }
    })
  ...
  }))), ['PC', 'Familiar', 'Monster', 'Neutral Beast', 'NPC'].includes(entity.type) && /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("label", null, "Passive Hiding ", /*#__PURE__*/React.createElement("span", {
    style: {
      color: 'var(--ink-mute)',
      fontWeight: 400,
      textTransform: 'none',
  ...
  }, "- 0 = always seen; otherwise hidden from players whose passive perception is below this")), /*#__PURE__*/React.createElement("input", {
    type: "number",
    min: "0",
    value: entity.passiveHiding || 0,
    onChange: e => update({
      passiveHiding: Math.max(0, Number(e.target.value) || 0)
    }),
    placeholder: "0"
  })), entity.type === 'PC' && /*#__PURE__*/React.createElement("div", {
    className: "form-row-2"
  }, /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("label", null, "Class"), /*#__PURE__*/React.createElement(ClassSelect, {
    value: entity.class || '',
    onCommit: v => update({
      class: v
    })
  ...
  }, lvl === 0 ? 'Healthy' : SICKNESS_DESCRIPTORS[lvl]))))), (entity.type === 'Monster' || entity.type === 'Neutral Beast') && /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("label", null, "Abilities / DM Notes"), /*#__PURE__*/React.createElement(AutoTextarea, {
  ...
  }, "- shown to players when revealed / on hover")), /*#__PURE__*/React.createElement(AutoTextarea, {
  ...
  })), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("label", null, "Conditions"), /*#__PURE__*/React.createElement(ConditionPicker, {
    active: entity.conditions,
    onToggle: c => update({
      conditions: entity.conditions.includes(c) ? entity.conditions.filter(x => x !== c) : [...entity.conditions, c]
    })
  })), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("label", null, "DM Notes ", /*#__PURE__*/React.createElement("span", {
    style: {
      color: 'var(--ink-mute)',
      fontWeight: 400,
      textTransform: 'none',
  ...
  }, "- never shown to players")), /*#__PURE__*/React.createElement(AutoTextarea, {
    value: entity.notes,
    onChange: e => update({
...
function EntitySidebar({
  ...
  const newFromPreset = async preset => {
    ...
    if (!ent.imageUrl) {
      try {
        const img = await resolvePresetImage(preset);
        if (img) ent.imageUrl = img;
      } catch {}
    }
    onEditEntity(ent);
  ...
  return /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
    ...
    }, "✎"), onOpenSheet && /*#__PURE__*/React.createElement("button", {
      className: "btn sm",
      onClick: () => onOpenSheet(e),
...
function InitiativeTracker({
  ...
  const addEntry = entityId => dispatch({
    type: 'INIT_ADD',
    entityId
  });
  ...
  const tokensFor = entityId => Object.values(state.tokens).filter(t => t.entityId === entityId && t.mapId === currentMapId);
  ...
  return /*#__PURE__*/React.createElement(FloatPanel, {
  ...
  }, /*#__PURE__*/React.createElement("span", null, "⚔ Initiative · Round ", initiative.round), /*#__PURE__*/React.createElement("button", {
    className: "close-x",
    onClick: onClose
  }, "×")), /*#__PURE__*/React.createElement("div", {
    className: "float-panel-body"
  }, mode === 'dm' && /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
    style: {
      display: 'flex',
      gap: 6,
      marginBottom: 8
    ...
    onClick: () => dispatch({
      type: 'SET_LOCK_OFF_TURN',
      value: !state.lockOffTurn
    }),
    title: "When locked, players can only move the token whose turn it currently is."
  }, state.lockOffTurn ? '🔒 Off-turn movement locked' : '🔓 Off-turn movement allowed'))), /*#__PURE__*/React.createElement("div", {
    className: "init-list"
    ...
    return /*#__PURE__*/React.createElement("div", {
      key: entry.entityId,
      className: `init-entry ${idx === initiative.turn ? 'current' : ''} ${hiddenFromPlayers ? 'init-hidden' : ''}`
      ...
      className: "init-name"
    }, e.name), hiddenFromPlayers && /*#__PURE__*/React.createElement("span", {
      className: "init-hidden-flag",
      title: "Hidden from players - they'll see this roll once you reveal the token"
    }, "hidden"), showExactHp ? /*#__PURE__*/React.createElement("div", {
      className: "init-hp"
  ...
  })), mode === 'dm' && initiative.active && missingEntities.length > 0 && /*#__PURE__*/React.createElement("div", {
    className: "init-add-section"
  }, /*#__PURE__*/React.createElement("div", {
    className: "init-add-label"
  }, "Not in the order - click to roll & add"), missingEntities.map(e => /*#__PURE__*/React.createElement("div", {
    key: e.id,
    className: "init-add-row"
  }, /*#__PURE__*/React.createElement("div", {
    className: "entity-swatch",
    style: {
      background: e.color,
    ...
    className: "init-name"
  }, e.name), isHiddenFromPlayers(e.id) && /*#__PURE__*/React.createElement("span", {
    className: "init-hidden-flag",
    title: "Currently hidden from players"
  }, "hidden"), /*#__PURE__*/React.createElement("button", {
    className: "btn sm",
    onClick: () => addEntry(e.id),
    title: "Roll initiative and add to the order"
  }, "+ Add"))))));
}
...
function MapManager({
  ...
  const patchLayer = (mapId, id, patch) => dispatch({
    type: 'LAYER_UPDATE',
    mapId,
    id,
    patch
  });
  const deleteLayer = (mapId, id) => dispatch({
    type: 'LAYER_DELETE',
    mapId,
    id
  });
  ...
  if (editing) {
    ...
    }, "Embedded as base64 - stays in session.")), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("label", null, "Notes (DM only)"), /*#__PURE__*/React.createElement(AutoTextarea, {
      value: editing.notes,
      onChange: e => setEditing({
        ...editing,
        notes: e.target.value
  ...
  return /*#__PURE__*/React.createElement(FloatPanel, {
    ...
    return /*#__PURE__*/React.createElement("div", {
      ...
      className: `btn sm ghost ${layersExpanded ? 'active' : ''}`,
      title: "Image layers",
      onClick: () => setLayersOpen(layersExpanded ? null : m.id)
    }, "🗇", mapLayers.length ? ` ${mapLayers.length}` : ''), /*#__PURE__*/React.createElement("button", {
      className: "btn sm",
      onClick: () => dispatch({
        type: 'MAP_SWITCH',
        id: m.id
    ...
    }, "×")), layersExpanded && /*#__PURE__*/React.createElement("div", {
      className: "layer-manager"
    }, /*#__PURE__*/React.createElement("div", {
      className: "layer-manager-head"
    }, /*#__PURE__*/React.createElement("span", null, "Image layers"), /*#__PURE__*/React.createElement("button", {
      className: "btn sm primary",
      onClick: () => addLayer(m.id)
    }, "＋ Add image")), mapLayers.length === 0 ? /*#__PURE__*/React.createElement("div", {
      className: "layer-empty"
    }, "No layers yet. Add an image to overlay this map.") : mapLayers.map(l => {
      ...
      }, l.imageUrl && l.imageUrl !== IMG_SENTINEL ? /*#__PURE__*/React.createElement("img", {
        src: l.imageUrl,
        alt: ""
      ...
      }, ['locked', 'move', 'rotate'].map(md => /*#__PURE__*/React.createElement("button", {
        key: md,
        className: `layer-mode-btn ${l.mode === md ? 'on' : ''}`,
        onClick: () => patchLayer(m.id, l.id, {
          mode: md
        })
      }, md === 'locked' ? '🔒 Lock' : md === 'move' ? '✥ Move' : '⟳ Rotate'))), /*#__PURE__*/React.createElement("div", {
        className: "layer-size"
      }, /*#__PURE__*/React.createElement("span", null, "Size"), /*#__PURE__*/React.createElement("input", {
        type: "range",
        min: "40",
        max: "2000",
        value: Math.round(l.w),
        onChange: e => {
          const w = Number(e.target.value);
          patchLayer(m.id, l.id, {
            w,
            h: Math.max(20, Math.round(w / (aspect || 1)))
          });
        }
      })), /*#__PURE__*/React.createElement("div", {
        className: "layer-row-actions"
      }, /*#__PURE__*/React.createElement("button", {
        className: `layer-flag ${l.dmOnly ? 'on' : ''}`,
        onClick: () => patchLayer(m.id, l.id, {
          dmOnly: !l.dmOnly
        }),
        title: "When on, only the DM can move or change this layer"
      }, l.dmOnly ? '👑 DM-only' : '👥 Shared'), l.rotation ? /*#__PURE__*/React.createElement("button", {
        className: "layer-flag",
        onClick: () => patchLayer(m.id, l.id, {
          rotation: 0
        }),
        title: "Reset rotation"
      }, "⟲ 0°") : null, /*#__PURE__*/React.createElement("button", {
        className: "layer-del",
        onClick: () => deleteLayer(m.id, l.id),
        title: "Delete layer"
      }, "🗑"))));
    })));
  }))));
...
const PLAGUE_PRESET_IDS = new Set(['unfinished_puppet', 'jake', 'tully', 'coalan', 'yevgeny', 'ernest_broken', 'laughing_puppet', 'angmar', 'barry', 'ivar', 'charles', 'elisia', 'marta', 'oswin', 'gerrit', 'pip']);
const TAG_DIMENSIONS = [{
  key: 'world',
  label: 'World',
  order: ['D&D', "Plague's Call", 'Burrows & Badgers', 'Custom']
}, {
  key: 'kind',
  label: 'Kind',
  order: ['Character', 'Non-character']
}, {
  key: 'type',
  label: 'Type',
  order: ['Beast', 'Humanoid', 'Monster', 'Ooze', 'Object', 'Undead', 'Celestial', 'Fiend']
}, {
  key: 'habitat',
  label: 'Habitat',
  order: ['Terrestrial', 'Aquatic', 'Flying']
}];
function presetTags(p) {
  ...
  if (id.startsWith('bnb:') || cat.startsWith('b&b')) tags.add('world:Burrows & Badgers');else if (PLAGUE_PRESET_IDS.has(idSuffix)) tags.add("world:Plague's Call");else if (id.startsWith('builtin:')) tags.add('world:D&D');else tags.add('world:Custom');

  // Kind
  tags.add(type === 'PC' || type === 'NPC' ? 'kind:Character' : 'kind:Non-character');

  // Creature type (coarse, best-effort)
  if (cat.includes('ooze') || /\booze|slime|jelly|pudding|mold\b/.test(text)) tags.add('type:Ooze');else if (/\bcelestial|angelic|seraph|empyrean\b/.test(text)) tags.add('type:Celestial');else if (/\bundead|skeleton|zombie|wraith|ghost|spectre|specter|lich|ghoul\b/.test(text)) tags.add('type:Undead');else if (/\bfiend|demon|devil\b/.test(text)) tags.add('type:Fiend');else if (type === 'Object' || cat.includes('object')) tags.add('type:Object');else if (type === 'PC' || type === 'NPC' || cat.includes('humanoid')) tags.add('type:Humanoid');else if (type === 'Neutral Beast' || cat.includes('animal') || cat.startsWith('b&b') || /\bbeast\b/.test(text)) tags.add('type:Beast');else tags.add('type:Monster');
...
function BestiaryMenu({
  ...
  const tagged = useMemo(() => [...builtins.map(p => ({
    ...p,
    _source: 'Built-in',
  ...
  useEffect(() => {
    setIndex(0);
  }, [search, filterCategory, activeTags]);
  useEffect(() => {
    setIndex(i => Math.min(Math.max(0, i), Math.max(0, n - 1)));
  }, [n]);
  ...
  return ReactDOM.createPortal(/*#__PURE__*/React.createElement("div", {
    ...
    className: "bestiary-carousel-modal",
    onClick: e => e.stopPropagation()
  }, /*#__PURE__*/React.createElement("div", {
    className: "bes-top"
  }, /*#__PURE__*/React.createElement("div", {
    className: "bes-title"
  }, "⚔ Bestiary"), /*#__PURE__*/React.createElement("input", {
    className: "bes-search",
    ...
    key: c,
    value: c
  }, c === 'All' ? 'All categories' : c)), custom.length > 0 && /*#__PURE__*/React.createElement("option", {
    value: "Custom"
  }, "Custom")), /*#__PURE__*/React.createElement("div", {
    className: "bes-counter"
  }, n === 0 ? '0 / 0' : `${index + 1} / ${n}`), /*#__PURE__*/React.createElement("button", {
    className: "bes-close",
    onClick: onClose,
    title: "Close (Esc)"
  }, "✕ Close")), /*#__PURE__*/React.createElement("div", {
    className: "bes-filters"
  }, TAG_DIMENSIONS.map(dim => {
    const chips = dim.order.filter(v => availableTags.has(`${dim.key}:${v}`));
    if (chips.length === 0) return null;
    return /*#__PURE__*/React.createElement("div", {
      key: dim.key,
      className: "bes-filter-group"
    }, /*#__PURE__*/React.createElement("span", {
      className: "bes-filter-label"
    }, dim.label), chips.map(v => {
      const tag = `${dim.key}:${v}`;
      return /*#__PURE__*/React.createElement("button", {
        key: tag,
        className: `bes-chip ${activeTags.has(tag) ? 'active' : ''}`,
        onClick: () => toggleTag(tag)
      }, v);
    }));
  }), (activeTags.size > 0 || filterCategory !== 'All') && /*#__PURE__*/React.createElement("button", {
    className: "bes-chip clear",
    onClick: () => {
      clearTags();
      setFilterCategory('All');
    }
  }, "✕ Clear")), n === 0 ? /*#__PURE__*/React.createElement("div", {
    className: "bes-empty"
  }, "No matches. Try clearing the filters or search.") : /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
    className: "bes-stage"
  }, /*#__PURE__*/React.createElement("button", {
    className: "bes-arrow left",
    onClick: () => go(-1),
    disabled: index === 0,
    "aria-label": "Previous"
  }, "‹"), /*#__PURE__*/React.createElement("div", {
    className: "bes-carousel"
  }, /*#__PURE__*/React.createElement("div", {
    className: "bes-track",
    style: {
      transform: `translateX(calc(-1 * ${index} * (${CARDW}px + ${GAP}px)))`
    }
  }, filtered.map((p, i) => {
    const e = p.entity || {};
    const st = e.stats || {};
    ...
    return /*#__PURE__*/React.createElement("div", {
      key: p.id,
      className: `bes-card ${isA ? 'is-active' : ''}`,
      style: {
        '--accent': e.color || '#8a7a55'
      },
      onClick: () => {
        if (!isA) jump(i);
      }
    }, /*#__PURE__*/React.createElement("div", {
      className: "bes-card-band"
    }), /*#__PURE__*/React.createElement("div", {
      className: "bes-card-head"
    }, /*#__PURE__*/React.createElement("div", {
      className: "bes-card-name"
    }, p.name), p.cr && /*#__PURE__*/React.createElement("div", {
      className: "bes-card-cr"
    }, "CR ", p.cr)), /*#__PURE__*/React.createElement("div", {
      className: "bes-card-sub"
    }, e.type, p._category && p._category !== 'Other' ? ` · ${p._category}` : '', e.role ? ` · ${e.role}` : ''), /*#__PURE__*/React.createElement("div", {
      className: "bes-core"
    }, /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("b", null, e.ac ?? '-'), /*#__PURE__*/React.createElement("span", null, "AC")), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("b", null, e.hp?.max ?? '-'), /*#__PURE__*/React.createElement("span", null, "HP")), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("b", null, e.speed ?? '-'), /*#__PURE__*/React.createElement("span", null, "Speed")), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("b", null, (e.initBonus >= 0 ? '+' : '') + (e.initBonus ?? 0)), /*#__PURE__*/React.createElement("span", null, "Init")), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("b", null, e.passivePerception ?? '-'), /*#__PURE__*/React.createElement("span", null, "Pass."))), /*#__PURE__*/React.createElement("div", {
      className: "bes-stats"
    }, ['str', 'dex', 'con', 'int', 'wis', 'cha'].map(k => /*#__PURE__*/React.createElement("div", {
      key: k,
      className: "bes-stat"
    }, /*#__PURE__*/React.createElement("span", {
      className: "bes-stat-k"
    }, k.toUpperCase()), /*#__PURE__*/React.createElement("span", {
      className: "bes-stat-v"
    }, st[k] ?? 10), /*#__PURE__*/React.createElement("span", {
      className: "bes-stat-m"
    }, mod(st[k]))))), /*#__PURE__*/React.createElement("div", {
      className: "bes-scroll"
    }, e.abilities && /*#__PURE__*/React.createElement("div", {
      className: "bes-abilities"
    }, e.abilities), e.playerDescription && /*#__PURE__*/React.createElement("div", {
      className: "bes-flavor"
    }, e.playerDescription), !e.abilities && !e.playerDescription && /*#__PURE__*/React.createElement("div", {
      className: "bes-noinfo"
    }, "No description.")), /*#__PURE__*/React.createElement("div", {
      className: "bes-card-actions"
    }, /*#__PURE__*/React.createElement("button", {
      className: "btn primary bes-add",
      onClick: ev => {
        ev.stopPropagation();
        onPick(p);
      }
    }, "＋ Add to map"), p._source === 'Custom' && /*#__PURE__*/React.createElement("button", {
    ...
    className: "bes-strip",
    ref: stripRef
  }, filtered.map((p, i) => /*#__PURE__*/React.createElement("button", {
    key: p.id,
    className: `bes-thumb ${i === index ? 'is-active' : ''}`,
    style: {
      '--accent': p.entity?.color || '#8a7a55'
    },
    onClick: () => jump(i),
    title: `${p.name}${p.cr ? ' · CR ' + p.cr : ''}`
  }, /*#__PURE__*/React.createElement("span", {
    className: "bes-thumb-dot"
  }), /*#__PURE__*/React.createElement("span", {
    className: "bes-thumb-name"
  }, p.name))))), /*#__PURE__*/React.createElement("div", {
    className: "bes-footer"
  }, "Browse with ‹ › or the arrow keys · ", /*#__PURE__*/React.createElement("b", null, "＋ Add to map"), " opens the creature pre-filled · drag a sidebar card → \"Save as preset\" to add your own"))), document.body);
}
...
function PresetsPanel({
  ...
  return /*#__PURE__*/React.createElement(FloatPanel, {
    ...
    }, p.tokens.length, " tokens · ", map?.name || 'unknown map'), /*#__PURE__*/React.createElement("div", {
      style: {
        display: 'flex',
        gap: 4
      }
...
function PlayerActionRequest({
  ...
  const bump = (s, d) => setCounts(c => ({
    ...c,
    [s]: Math.max(0, Math.min(50, (c[s] | 0) + d))
  }));
  const totalDice = Object.values(counts).reduce((a, b) => a + b, 0);
  const flash = msg => {
    setSent(msg);
    clearTimeout(sentTimer.current);
    sentTimer.current = setTimeout(() => setSent(null), 3000);
  };
  const submit = (kind, data) => {
    if (dmMode && onDmSubmit) onDmSubmit({
      kind,
      data
    });else playerActionSender({
      type: 'submit_request',
      payload: {
        kind,
        data
  ...
  const sendComponents = (components, opts = {}) => {
    submit(mode === 'heal' ? 'apply_heal' : 'apply_damage', {
      ...
      components,
      toHit: opts.toHit ?? null,
      d20a: opts.d20a ?? null,
      d20b: opts.d20b ?? null,
      advMode: opts.advMode ?? 'normal',
      effect: opts.effect || null,
      weaponName: opts.weaponName || '',
      attackName: opts.attackName || ''
    });
  };
  const sendManual = () => {
    const dice = [];
    for (const s of ALLOWED_DIE_SIDES) for (let i = 0; i < (counts[s] | 0); i++) dice.push({
      sides: s,
      result: 1 + Math.floor(Math.random() * s)
    });
    const flatN = Math.max(0, Math.round(Number(flat)) || 0);
    if (dice.length === 0 && flatN === 0 && mode === 'damage') {
      flash('Add some dice or a number first.');
      return;
    }
    const type = mode === 'damage' ? dmgType === '__custom__' ? customType.trim() || 'Untyped' : dmgType : '';
    sendComponents([{
      dice,
      modifier: Math.round(Number(modifier)) || 0,
      flat: flatN,
      type
    }]);
    setCounts({
  ...
  const sendWeaponAttack = (weapon, atk) => {
    if (!canReach(atk.range)) {
      flash(`Out of range - ${dist} ft away, reach ${atk.range} ft.`);
      return;
    }
    const components = (atk.damage || []).map(c => {
      const dice = [];
      const sides = c.sides || 6;
      for (let i = 0; i < (c.count || 0); i++) dice.push({
        sides,
        result: 1 + Math.floor(Math.random() * sides)
      });
      return {
  ...
  const beginWeaponAttack = (weapon, atk) => {
    ...
    if (!physicalDice) {
      sendWeaponAttack(weapon, atk);
      return;
    }
    const dieVals = (atk.damage || []).map(c => Array.from({
      length: c.count || 0
    }, () => ''));
    setManual({
      weapon,
  ...
  const confirmManual = () => {
    ...
    const components = (atk.damage || []).map((c, ci) => {
      const sides = c.sides || 6;
      const dice = (dieVals[ci] || []).map(v => ({
        sides,
        result: clampDie(v, sides)
      }));
      return {
        dice,
        modifier: c.modifier || 0,
  ...
  const sendManeuver = m => {
    if (!maneuverReaches(m.range)) {
      flash(`Out of range - ${dist} ft away, reach ${m.range || 5} ft.`);
      return;
    }
    let effect = null;
    if (m.condition) {
  ...
  const modes = attackOnly ? [['damage', '⚔ Attack'], ['maneuver', '🤼 Maneuver']] : [['damage', '⚔ Attack'], ['maneuver', '🤼 Maneuver'], ['heal', '✚ Heal'], ['condition', '✦ Effect']];
  return /*#__PURE__*/React.createElement("div", {
    className: "par-box"
  }, /*#__PURE__*/React.createElement("div", {
    className: "par-head"
    ...
    className: "par-src",
    style: {
      background: sourceEntity.color
    }
  }, (sourceEntity.name[0] || '?').toUpperCase()), sourceOptions.length > 1 ? /*#__PURE__*/React.createElement("select", {
    className: "par-source-select",
    value: sourceId,
    onChange: e => setSourceId(e.target.value),
    title: "Who is acting"
  }, sourceOptions.map(e => /*#__PURE__*/React.createElement("option", {
    key: e.id,
    value: e.id
  }, e.name, e.type === 'Familiar' ? ' (familiar)' : e.type !== 'PC' ? ` (${e.type})` : ''))) : /*#__PURE__*/React.createElement("span", {
    className: "par-head-text"
  }, sourceEntity.name), /*#__PURE__*/React.createElement("span", {
    className: "par-head-arrow"
  }, "→ ", /*#__PURE__*/React.createElement("strong", null, targetEntity.name))), inCombat && /*#__PURE__*/React.createElement("div", {
    className: `par-range-note ${hasDist ? '' : 'far'}`
  }, hasDist ? `In combat · ${dist} ft to target` : 'In combat · attacker not on this map'), modes.length > 1 && /*#__PURE__*/React.createElement("div", {
    className: "par-modes"
  }, modes.map(([m, label]) => /*#__PURE__*/React.createElement("button", {
    key: m,
    className: `par-mode ${mode === m ? 'active' : ''}`,
    onClick: () => setMode(m)
  }, label))), mode === 'damage' && equippedWeapons.length > 0 && /*#__PURE__*/React.createElement("div", {
    className: "par-weapon-pick"
  }, /*#__PURE__*/React.createElement("button", {
    className: `par-wchip ${weaponSel === '' ? 'active' : ''}`,
    onClick: () => setWeaponSel('')
  }, "✎ Custom"), equippedWeapons.flatMap(w => w.attacks.map(a => {
    const key = `${w.id}|${a.id}`;
    const reach = canReach(a.range);
    return /*#__PURE__*/React.createElement("button", {
      key: key,
      disabled: !reach,
      className: `par-wchip ${weaponSel === key ? 'active' : ''} ${reach ? '' : 'out'}`,
      title: reach ? formatAttack(a) : `Out of range: ${dist} ft away, reach ${a.range || 0} ft`,
      onClick: () => reach && setWeaponSel(key)
    }, w.name, ": ", a.name, !reach && ' ⛌');
  }))), mode === 'damage' && /*#__PURE__*/React.createElement("div", {
    className: "par-adv"
  }, /*#__PURE__*/React.createElement("button", {
    className: `par-adv-btn adv ${advMode === 'advantage' ? 'on' : ''}`,
    title: "Roll the to-hit twice, keep the highest",
    onClick: () => setAdvMode(m => m === 'advantage' ? 'normal' : 'advantage')
  }, advMode === 'advantage' ? '☑' : '☐', " Advantage"), /*#__PURE__*/React.createElement("button", {
    className: `par-adv-btn dis ${advMode === 'disadvantage' ? 'on' : ''}`,
    title: "Roll the to-hit twice, keep the lowest",
    onClick: () => setAdvMode(m => m === 'disadvantage' ? 'normal' : 'disadvantage')
  }, advMode === 'disadvantage' ? '☑' : '☐', " Disadvantage")), mode === 'maneuver' ? /*#__PURE__*/React.createElement("div", {
    className: "par-maneuvers"
  }, !inCombat && hasDist && /*#__PURE__*/React.createElement("div", {
    className: "par-range-note"
  }, dist, " ft to target · melee maneuvers reach 5 ft"), /*#__PURE__*/React.createElement("div", {
    className: "par-man-grid"
  }, MANEUVER_PRESETS.map(m => {
    const reach = maneuverReaches(m.range);
    return /*#__PURE__*/React.createElement("button", {
      key: m.key,
      disabled: !reach,
      className: `par-man ${manSel === m.key ? 'active' : ''} ${reach ? '' : 'out'}`,
    ...
    className: `par-man ${manSel === 'custom' ? 'active' : ''}`,
    onClick: () => setManSel('custom')
  }, "✎ Custom")), manSel && (() => {
    ...
    return /*#__PURE__*/React.createElement("div", {
      className: "par-man-detail"
    }, /*#__PURE__*/React.createElement("div", {
      className: "par-man-row"
    }, /*#__PURE__*/React.createElement("label", {
      className: "par-field",
      style: {
        flex: 1
      }
    }, "Effect", /*#__PURE__*/React.createElement("select", {
      value: manCond,
      onChange: e => setManCond(e.target.value)
    }, /*#__PURE__*/React.createElement("option", {
      value: ""
    }, "(no condition - DM resolves)"), CONDITIONS.map(c => /*#__PURE__*/React.createElement("option", {
      key: c,
      value: c
      ...
      className: "par-field"
    }, "Save", /*#__PURE__*/React.createElement("select", {
      value: manAbil,
      onChange: e => setManAbil(e.target.value)
    }, ABILITIES.map(a => /*#__PURE__*/React.createElement("option", {
      key: a,
      value: a
    }, a)))), /*#__PURE__*/React.createElement("label", {
      className: "par-field"
    }, "DC", /*#__PURE__*/React.createElement("input", {
      type: "number",
      min: "1",
      max: "40",
      value: manDc,
      onChange: e => setManDc(e.target.value)
    }))), !reach && /*#__PURE__*/React.createElement("div", {
      className: "par-oor"
    }, "Out of range - ", dist, " ft away, reach ", range, " ft."), /*#__PURE__*/React.createElement("button", {
      className: "btn primary par-send",
      disabled: !reach,
      onClick: () => sendManeuver({
        label,
        condition: manCond,
        ability: manAbil,
        dc: manDc,
        range,
        contest: preset ? preset.contest : false
      })
    }, dmMode ? '🤼 Launch' : '🤼 Send', " ", label));
  })()) : mode === 'condition' ? /*#__PURE__*/React.createElement("div", {
    className: "par-man-detail"
  }, /*#__PURE__*/React.createElement("div", {
    className: "par-man-row"
  }, /*#__PURE__*/React.createElement("label", {
    ...
    value: effCond,
    onChange: e => setEffCond(e.target.value)
    ...
    value: effAbil,
    onChange: e => setEffAbil(e.target.value)
  }, ABILITIES.map(a => /*#__PURE__*/React.createElement("option", {
    key: a,
    value: a
  }, a)))), /*#__PURE__*/React.createElement("label", {
    ...
    max: "40",
    value: effDc,
    onChange: e => setEffDc(e.target.value)
  }))), /*#__PURE__*/React.createElement("div", {
    className: "settings-hint",
    style: {
      marginBottom: 6
    }
  }, targetEntity.name, " rolls a ", effAbil, " save vs DC ", effDc, " or becomes ", effCond, "."), /*#__PURE__*/React.createElement("button", {
    className: "btn primary par-send",
    onClick: () => sendEffect(effCond, effAbil, effDc)
  }, dmMode ? '✦ Launch' : '✦ Send', " ", effCond)) : showWeapon ? /*#__PURE__*/React.createElement("div", {
    className: "par-wpreview"
  }, /*#__PURE__*/React.createElement("div", {
    className: "par-wp-line"
  }, /*#__PURE__*/React.createElement("b", null, selWeapon.name, " - ", selAttack.name)), /*#__PURE__*/React.createElement("div", {
    className: "par-wp-meta"
  }, "+", selAttack.toHit || 0, " to hit · ", selAttack.range || 0, " ft. range"), (selAttack.damage || []).map((c, i) => /*#__PURE__*/React.createElement("div", {
    key: i,
    className: "par-wp-comp"
  }, /*#__PURE__*/React.createElement("span", {
    className: "par-wp-dice"
  }, formatDamageComp(c)))), selAttack.effect && selAttack.effect.condition && /*#__PURE__*/React.createElement("div", {
    className: "par-wp-effect"
  }, "✦ ", selAttack.effect.condition, selAttack.effect.save ? ` (${selAttack.effect.save.ability} save DC ${selAttack.effect.save.dc})` : ' on hit'), !selReach && /*#__PURE__*/React.createElement("div", {
    className: "par-oor"
  }, "Out of range - ", dist, " ft away, reach ", selAttack.range || 0, " ft."), manual ? /*#__PURE__*/React.createElement("div", {
    className: "par-manual"
  }, /*#__PURE__*/React.createElement("div", {
    className: "par-manual-title"
  }, "Enter your rolls"), /*#__PURE__*/React.createElement("div", {
    className: "par-manual-row"
  }, /*#__PURE__*/React.createElement("span", {
    className: "par-manual-lbl"
  }, "To-hit d20", advMode !== 'normal' ? ' ×2' : ''), /*#__PURE__*/React.createElement("input", {
    className: "par-manual-die",
    type: "number",
    min: "1",
    max: "20",
    autoFocus: true,
    value: manual.d20a,
    onChange: e => setManual(s => ({
      ...s,
      d20a: e.target.value
    }))
  }), advMode !== 'normal' && /*#__PURE__*/React.createElement("input", {
    className: "par-manual-die",
    type: "number",
    min: "1",
    max: "20",
  ...
  })), (manual.atk.damage || []).map((c, ci) => /*#__PURE__*/React.createElement("div", {
    key: ci,
    className: "par-manual-row"
  }, /*#__PURE__*/React.createElement("span", {
    className: "par-manual-lbl"
  }, c.count, "d", c.sides, " ", c.type), manual.dieVals[ci].map((v, di) => /*#__PURE__*/React.createElement("input", {
    key: di,
    className: "par-manual-die",
    type: "number",
    min: "1",
    ...
    value: v,
    onChange: e => setManual(s => {
      const dv = s.dieVals.map(a => a.slice());
      dv[ci][di] = e.target.value;
      return {
        ...s,
        dieVals: dv
      };
    ...
    className: "par-manual-actions"
  }, /*#__PURE__*/React.createElement("button", {
    className: "btn primary",
    onClick: confirmManual
  }, "Send with these rolls"), /*#__PURE__*/React.createElement("button", {
    className: "btn",
    onClick: () => setManual(null)
  }, "Cancel"))) : /*#__PURE__*/React.createElement("button", {
    className: "btn primary par-send",
    disabled: !selReach,
    onClick: () => beginWeaponAttack(selWeapon, selAttack)
  }, physicalDice ? dmMode ? '✎ Attack with' : '✎ Enter rolls for' : dmMode ? '⚔ Roll & attack' : '⚔ Roll & send', " ", selAttack.name)) : /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
    className: "par-dice"
  }, ALLOWED_DIE_SIDES.map(s => /*#__PURE__*/React.createElement("div", {
    key: s,
    className: "par-die"
  }, /*#__PURE__*/React.createElement("span", {
    className: "par-die-label"
  }, "d", s), /*#__PURE__*/React.createElement("button", {
    className: "par-die-btn",
    onClick: () => bump(s, -1),
    disabled: (counts[s] | 0) <= 0
  }, "−"), /*#__PURE__*/React.createElement("span", {
    className: "par-die-count"
    ...
    className: "par-field"
  }, "Manual roll", /*#__PURE__*/React.createElement("input", {
    type: "number",
    min: "0",
    value: flat,
    onChange: e => setFlat(e.target.value),
    placeholder: "real dice"
  }))), mode === 'damage' && /*#__PURE__*/React.createElement("div", {
    className: "par-row"
  }, /*#__PURE__*/React.createElement("label", {
    className: "par-field",
  ...
  }, "Damage type", /*#__PURE__*/React.createElement("select", {
    value: dmgType,
    onChange: e => setDmgType(e.target.value)
  }, DAMAGE_TYPES.map(t => /*#__PURE__*/React.createElement("option", {
    key: t,
    value: t
  }, t)), /*#__PURE__*/React.createElement("option", {
    value: "__custom__"
  }, "Custom…"))), dmgType === '__custom__' && /*#__PURE__*/React.createElement("label", {
    className: "par-field",
    style: {
      flex: 1
    }
    ...
    className: "btn primary par-send",
    onClick: sendManual
  }, mode === 'heal' ? dmMode ? '✚ Apply heal' : '✚ Send heal to DM' : dmMode ? '⚔ Launch attack' : '⚔ Send attack to DM', totalDice > 0 ? ` (${totalDice} dice)` : '')), sent && /*#__PURE__*/React.createElement("div", {
    className: "par-sent"
  }, sent));
}
...
function RadialTokenMenu(props) {
  ...
  const ownedIds = myPeerId && typeof ownedByPeer === 'function' ? ownedByPeer(state, myPeerId) : new Set(claimedEntityId ? [claimedEntityId] : []);
  ...
  const hasAttack = isDM ? Object.values(state.entities).some(e => e && e.id !== entity.id && ['PC', 'Monster', 'NPC', 'Familiar', 'Neutral Beast'].includes(e.type) && Object.values(state.tokens).some(t => t.entityId === e.id && token && t.mapId === token.mapId)) : !isOwnPC && [...ownedIds].some(id => {
    const e = state.entities[id];
    return e && e.id !== entity.id && (e.type === 'PC' || e.type === 'Familiar');
  });
  const cats = [{
    key: 'info',
    icon: 'ⓘ',
  ...
  if (isActiveMover && ownsThis) cats.push({
    key: 'move',
    icon: '🏃',
    label: 'Move'
  });
  if (hasAttack) cats.push({
    key: 'attack',
    icon: '⚔',
    label: 'Attack'
  });
  if (canEditHp) cats.push({
    key: 'hp',
    icon: '✚',
    label: 'HP'
  });
  if (canEditConditions) cats.push({
    key: 'conditions',
    icon: '✦',
    label: 'Status'
  });
  ...
  return ReactDOM.createPortal(/*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
    ...
    src: entity.imageUrl,
    alt: ""
  }) : (entity.name[0] || '?').toUpperCase()), /*#__PURE__*/React.createElement("div", {
    className: "radial-hub-name"
  }, entity.name), /*#__PURE__*/React.createElement("button", {
    className: "radial-hub-close",
    onClick: onClose,
    title: "Close"
  }, "×")), cats.map((c, i) => {
    ...
    return /*#__PURE__*/React.createElement("button", {
      key: c.key,
      className: `radial-seg ${section === c.key ? 'on' : ''}`,
      style: {
        transform: `translate(-50%, -50%) translate(${x}px, ${y}px)`
      },
      onClick: () => setSection(s => s === c.key ? null : c.key),
      title: c.label
    }, /*#__PURE__*/React.createElement("span", {
      className: "radial-seg-ic"
    }, c.icon), /*#__PURE__*/React.createElement("span", {
      className: "radial-seg-lbl"
    }, c.label));
  }))), section && /*#__PURE__*/React.createElement(TokenDetailPanel, {
    ...props,
...
function TokenDetailPanel({
  ...
  const dmLaunch = ({
    ...
    const components = (data.components || []).map(c => ({
      dice: c.dice || [],
      diceSum: (c.dice || []).reduce((x, d) => x + (d.result || 0), 0),
      modifier: c.modifier || 0,
      flat: c.flat || 0,
      type: c.type || ''
    }));
    dispatch({
      ...
      attack: {
        id: uid('atk_'),
        attackerId: data.sourceEntityId,
        attackerName: src?.name || 'Attacker',
        attackerColor: src?.color || '#888',
        attackerImg: src?.imageUrl || null,
        targetId: entity.id,
        targetName: entity.name,
        targetColor: entity.color,
        targetImg: entity.imageUrl || null,
        targetAc: ac,
        weaponName: data.weaponName || '',
        attackName: data.attackName || '',
        toHit: data.toHit ?? null,
        toHitRoll,
        hit,
        d20a,
        d20b,
  ...
  const mvWalk = walkSpeedOf(entity),
    mvFly = flySpeedOf(entity),
    mvJump = jumpSpeedOf(entity),
    mvSwim = swimSpeedOf(entity),
    mvClimb = climbSpeedOf(entity);
  ...
  const sendMovementMode = m => {
    if (isDM) dispatch({
      ...
      entityId: entity.id,
      mode: m
    });else if (playerActionSender) playerActionSender({
      type: 'movement_mode',
      mode: m,
      entityId: entity.id
    });
  ...
  return /*#__PURE__*/React.createElement(FloatPanel, {
    ...
    className: "float-panel-body"
  }, show('info') && /*#__PURE__*/React.createElement("div", {
    style: {
      display: 'grid',
  ...
  }, monsterStatus) : showExactHp ? /*#__PURE__*/React.createElement("div", {
    className: "mono",
    style: {
      fontSize: 18,
      fontWeight: 600
  ...
  }, "/", entity.hp.max)) : /*#__PURE__*/React.createElement("div", {
    className: `status-label status-${hpLabel(hpPct).text.toLowerCase()}`
  }, hpLabel(hpPct).text)), !isOpaqueForPlayer && /*#__PURE__*/React.createElement("div", {
    style: {
      textAlign: 'center',
      padding: 8,
      background: 'var(--bg-0)',
  ...
  }, mvWalk, mvFly > 0 ? /*#__PURE__*/React.createElement("span", {
    style: {
      color: 'var(--ink-mute)',
      fontSize: 11
    },
    title: "Fly speed"
  }, " 🪽", mvFly) : null))), show('info') && /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 11,
      color: 'var(--ink-dim)',
      marginBottom: 8
  ...
  }, entity.playerDescription), show('hp') && canEditHp && /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("label", null, "Adjust HP ", isOwnPC && !isDM && /*#__PURE__*/React.createElement("span", {
    style: {
      color: 'var(--gold-dim)',
      fontWeight: 400,
      textTransform: 'none',
    ...
    className: "btn",
    onClick: () => applyHp(+1)
  }, "+ Heal"))), show('conditions') && canEditConditions && /*#__PURE__*/React.createElement("div", {
    style: {
      marginBottom: 10
    }
  }, /*#__PURE__*/React.createElement("label", null, "Conditions"), /*#__PURE__*/React.createElement(ConditionPicker, {
    active: entity.conditions,
    onToggle: emitToggleCondition,
    canEdit: canEditConditions
  })), show('move') && isActiveMover && ownsThis && /*#__PURE__*/React.createElement("div", {
    className: "move-actions",
    style: {
      marginBottom: 10
    }
  }, /*#__PURE__*/React.createElement("label", null, "Movement ", mvFly > 0 ? `· walk ${mvWalk} / fly ${mvFly} ft` : `· ${mvWalk} ft`, mvJump > 0 ? ` · jump ${mvJump} ft` : ''), /*#__PURE__*/React.createElement("div", {
    className: "move-actions-row"
  }, /*#__PURE__*/React.createElement("button", {
    className: `move-act ${mvMode === 'walk' && !mvDashed ? 'on' : ''}`,
    onClick: () => sendMovementMode('walk'),
    title: "Normal walking speed"
  }, "🥾 Walk"), /*#__PURE__*/React.createElement("button", {
    className: `move-act ${mvDashed ? 'on' : ''}`,
    onClick: () => sendMovementMode('dash'),
    title: "Double your movement for this turn"
  }, "💨 Dash"), mvFly > 0 && /*#__PURE__*/React.createElement("button", {
    className: `move-act ${mvMode === 'fly' ? 'on' : ''}`,
    onClick: () => sendMovementMode('fly'),
    title: "Move using your fly speed"
  }, "🪽 Fly"), mvSwim > 0 && /*#__PURE__*/React.createElement("button", {
    className: `move-act ${mvMode === 'swim' ? 'on' : ''}`,
    onClick: () => sendMovementMode('swim'),
    title: "Move using your swim speed"
  }, "🌊 Swim"), mvClimb > 0 && /*#__PURE__*/React.createElement("button", {
    className: `move-act ${mvMode === 'climb' ? 'on' : ''}`,
    onClick: () => sendMovementMode('climb'),
    title: "Move using your climb speed"
  }, "🧗 Climb"), mvJump > 0 && /*#__PURE__*/React.createElement("button", {
    className: `move-act ${mvMode === 'jump' ? 'on' : ''}`,
    onClick: () => sendMovementMode('jump'),
    title: "Leap up to your jump speed, then revert to your remaining walk"
  }, "🦗 Jump")), /*#__PURE__*/React.createElement("div", {
    className: "move-actions-rem"
  }, /*#__PURE__*/React.createElement("b", null, mvRemaining, " ft"), " remaining", mvMode === 'jump' ? ' · jump armed (next move)' : mvDashed ? ' · dashing' : mvMode === 'fly' ? ' · flying' : mvMode === 'swim' ? ' · swimming' : mvMode === 'climb' ? ' · climbing' : '')), show('attack') && !isDM && !isOwnPC && (() => {
    const ownedIds = myPeerId ? [...ownedByPeer(state, myPeerId)] : claimedEntityId ? [claimedEntityId] : [];
    const sourceOptions = ownedIds.map(id => state.entities[id]).filter(e => e && e.id !== entity.id && (e.type === 'PC' || e.type === 'Familiar'));
    ...
    const defaultSourceId = claimedEntityId && sourceOptions.some(e => e.id === claimedEntityId) ? claimedEntityId : sourceOptions[0].id;
    return /*#__PURE__*/React.createElement(PlayerActionRequest, {
    ...
    const sourceOptions = Object.values(state.entities).filter(e => e && e.id !== entity.id && ['PC', 'Monster', 'NPC', 'Familiar', 'Neutral Beast'].includes(e.type) && Object.values(state.tokens).some(t => t.entityId === e.id && token && t.mapId === token.mapId));
    if (sourceOptions.length === 0) {
      return /*#__PURE__*/React.createElement("div", {
        className: "settings-hint",
        style: {
    ...
    }, /*#__PURE__*/React.createElement("label", null, "Attack ", entity.name, " from…"), /*#__PURE__*/React.createElement(PlayerActionRequest, {
      sourceOptions: sourceOptions,
      defaultSourceId: defaultSourceId,
      targetToken: token,
      targetEntity: entity,
  ...
  })(), show('info') && isDM && entity.type === 'Monster' && entity.abilities && /*#__PURE__*/React.createElement("div", {
    style: {
      marginBottom: 10
    }
  ...
  }, entity.abilities)), show('info') && isDM && entity.notes && /*#__PURE__*/React.createElement("div", {
    style: {
      marginBottom: 10
    }
  ...
  }, entity.notes)), show('info') && isDM && entity.type === 'PC' && /*#__PURE__*/React.createElement("div", {
    style: {
      marginBottom: 10
    }
  ...
  }, "Clear death saves"))), show('info') && isDM && entity.type === 'Familiar' && state && /*#__PURE__*/React.createElement("div", {
    style: {
      marginBottom: 10
    }
  ...
  })()), show('info') && isDM && ['PC', 'Familiar', 'Monster', 'Neutral Beast', 'NPC'].includes(entity.type) && /*#__PURE__*/React.createElement("div", {
    style: {
      marginBottom: 10
    }
  ...
  })))), show('info') && isDM && ['PC', 'NPC', 'Monster', 'Neutral Beast', 'Familiar'].includes(entity.type) && /*#__PURE__*/React.createElement("div", {
    style: {
      marginBottom: 10
    }
    ...
    className: "sickness-label"
  }, lvl === 0 ? 'Healthy' : SICKNESS_DESCRIPTORS[lvl]))))), show('info') && isDM && /*#__PURE__*/React.createElement("div", {
    style: {
      marginBottom: 10
    }
  ...
  }, "Reset"))), show('info') && isDM && /*#__PURE__*/React.createElement("div", {
    style: {
      display: 'flex',
      gap: 6,
      marginTop: 12,
  ...
  }, "⛭ Long Rest"), onShortRest && (entity.type === 'PC' || entity.type === 'Familiar') && /*#__PURE__*/React.createElement("button", {
    className: "btn",
    onClick: () => onShortRest(entity.id),
    title: "Short rest - restore half of max HP"
  }, "◑ Short Rest"), /*#__PURE__*/React.createElement("button", {
    className: "btn danger",
    onClick: removeToken
  }, "Remove"))));
}
...
function TokenTooltip({
  ...
  }, ent.type)), ent.hp.max > 0 && (showExactHp ? /*#__PURE__*/React.createElement("div", {
    className: "token-tooltip-hp mono"
  }, "HP ", ent.hp.current, "/", ent.hp.max) : /*#__PURE__*/React.createElement("div", {
    className: `status-label status-${label.text.toLowerCase()}`
  }, label.text)), sicknessLabel && /*#__PURE__*/React.createElement("div", {
...
function SettingsModal({
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    className: `settings-tab ${tab === 'aesthetic' ? 'active' : ''}`,
    onClick: () => setTab('aesthetic')
  }, "🎨 Aesthetic"), /*#__PURE__*/React.createElement("button", {
    className: `settings-tab ${tab === 'gameplay' ? 'active' : ''}`,
    onClick: () => setTab('gameplay')
  }, "🎲 Gameplay")), tab === 'aesthetic' && /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
    ...
    className: "theme-sub"
  }, "Navy · gilded")), /*#__PURE__*/React.createElement("button", {
    className: `theme-option ${settings.theme === 'forest' ? 'active' : ''}`,
    onClick: () => onChange({
      theme: 'forest'
    })
  }, /*#__PURE__*/React.createElement("span", {
    className: "theme-swatch forest"
  }), /*#__PURE__*/React.createElement("span", null, "Dark Forest"), /*#__PURE__*/React.createElement("span", {
    className: "theme-sub"
  }, "Moss · lichen")), /*#__PURE__*/React.createElement("button", {
    className: `theme-option ${settings.theme === 'darkcherry' ? 'active' : ''}`,
    onClick: () => onChange({
      theme: 'darkcherry'
    })
  }, /*#__PURE__*/React.createElement("span", {
    className: "theme-swatch darkcherry"
  }), /*#__PURE__*/React.createElement("span", null, "Dark Cherry"), /*#__PURE__*/React.createElement("span", {
    className: "theme-sub"
  }, "Wine · cherry-red")), /*#__PURE__*/React.createElement("button", {
    className: `theme-option ${settings.theme === 'ocean' ? 'active' : ''}`,
    onClick: () => onChange({
      theme: 'ocean'
    })
  }, /*#__PURE__*/React.createElement("span", {
    className: "theme-swatch ocean"
  }), /*#__PURE__*/React.createElement("span", null, "Deep Ocean"), /*#__PURE__*/React.createElement("span", {
    className: "theme-sub"
  }, "Abyss · aqua")), /*#__PURE__*/React.createElement("button", {
    ...
    className: "theme-sub"
  }, "Parchment · oak")), /*#__PURE__*/React.createElement("button", {
    className: `theme-option ${settings.theme === 'cherry' ? 'active' : ''}`,
    onClick: () => onChange({
      theme: 'cherry'
    })
  }, /*#__PURE__*/React.createElement("span", {
    className: "theme-swatch cherry"
  }), /*#__PURE__*/React.createElement("span", null, "Cherry Blossom"), /*#__PURE__*/React.createElement("span", {
    className: "theme-sub"
  }, "Petal · plum")), /*#__PURE__*/React.createElement("button", {
    className: `theme-option ${settings.theme === 'river' ? 'active' : ''}`,
    onClick: () => onChange({
      theme: 'river'
    })
  }, /*#__PURE__*/React.createElement("span", {
    className: "theme-swatch river"
  }), /*#__PURE__*/React.createElement("span", null, "River Blue"), /*#__PURE__*/React.createElement("span", {
    className: "theme-sub"
  }, "Water · slate")), /*#__PURE__*/React.createElement("button", {
    className: `theme-option ${settings.theme === 'meadow' ? 'active' : ''}`,
    onClick: () => onChange({
      theme: 'meadow'
    })
  }, /*#__PURE__*/React.createElement("span", {
    className: "theme-swatch meadow"
  }), /*#__PURE__*/React.createElement("span", null, "Flowery Meadow"), /*#__PURE__*/React.createElement("span", {
    className: "theme-sub"
  }, "Grass · bloom")))), /*#__PURE__*/React.createElement("div", {
    className: "settings-section"
  }, /*#__PURE__*/React.createElement("label", {
    className: "settings-label"
  }, "Movement Range Marker"), /*#__PURE__*/React.createElement("div", {
    className: "toggle-row",
    style: {
      display: 'flex',
      alignItems: 'center',
    ...
    onChange: e => onChange({
      moveRangeOpacity: Number(e.target.value)
    }),
    style: {
      flex: 1,
  ...
  }, Math.round((settings.moveRangeOpacity ?? 0.55) * 100), "%")), /*#__PURE__*/React.createElement("div", {
    className: "settings-hint"
  }, "Opacity of the combat movement rings. Set to 0% to hide them entirely.")), /*#__PURE__*/React.createElement("div", {
    className: "settings-section"
  }, /*#__PURE__*/React.createElement("label", {
    className: "settings-label"
  }, "Page Texture"), /*#__PURE__*/React.createElement("div", {
    className: "toggle-row",
    style: {
      display: 'flex',
      alignItems: 'center',
  ...
  }, "Soft grain on menus and panels")), /*#__PURE__*/React.createElement("div", {
    className: "settings-hint"
  }, "A gentle, fading film grain over menu backgrounds. Purely cosmetic.")), mode === 'player' && /*#__PURE__*/React.createElement("div", {
    className: "settings-section"
  }, /*#__PURE__*/React.createElement("label", {
    className: "settings-label"
    ...
    onChange: e => onChange({
      sicknessEffects: e.target.checked
    }),
    style: {
      width: 16,
      height: 16,
    ...
    className: "settings-hint"
  }, "Disable if you find the motion distracting or experience motion sensitivity."))), tab === 'gameplay' && /*#__PURE__*/React.createElement(React.Fragment, null, mode === 'dm' && /*#__PURE__*/React.createElement("div", {
    className: "settings-section"
  }, /*#__PURE__*/React.createElement("label", {
    className: "settings-label"
    ...
    className: "settings-hint"
  }, "Scales the entire map rendering uniformly. Pan/zoom still works on top.")), mode === 'dm' && /*#__PURE__*/React.createElement("div", {
    className: "settings-section"
  }, /*#__PURE__*/React.createElement("label", {
    className: "settings-label"
  }, "Player HP Display"), /*#__PURE__*/React.createElement("div", {
    className: "toggle-row",
    style: {
      display: 'flex',
      alignItems: 'center',
    ...
    htmlFor: "obfuscate-hp-toggle",
    style: {
      cursor: 'pointer',
      fontSize: 13
    }
  }, "Hide exact HP from players")), /*#__PURE__*/React.createElement("div", {
    className: "settings-hint"
  }, "Players see ", /*#__PURE__*/React.createElement("em", null, "Strong / Rough / Waning / Down"), " instead of numbers. You still see exact values.")), mode === 'dm' && /*#__PURE__*/React.createElement("div", {
    className: "settings-section"
  }, /*#__PURE__*/React.createElement("label", {
    className: "settings-label"
  }, "New Players"), /*#__PURE__*/React.createElement("div", {
    className: "toggle-row",
    style: {
      display: 'flex',
      alignItems: 'center',
    ...
    onChange: e => onChange({
      approveNewPlayers: e.target.checked
    }),
    style: {
      width: 16,
      height: 16,
    ...
    htmlFor: "approve-players-toggle",
    style: {
      cursor: 'pointer',
      fontSize: 13
    }
  }, "Approve each new player before they can join")), /*#__PURE__*/React.createElement("div", {
    className: "settings-hint"
  }, "New arrivals wait on a \"waiting to be admitted\" screen until you accept them. Reconnecting players who already have a character skip the gate.")), /*#__PURE__*/React.createElement("div", {
    className: "settings-section",
    style: mode === 'dm' ? {} : {
      display: 'none'
    }
    ...
    className: "settings-label"
  }, "Physical Dice"), /*#__PURE__*/React.createElement("div", {
    className: "toggle-row",
    style: {
      display: 'flex',
      alignItems: 'center',
    ...
    htmlFor: "physical-dice-toggle",
    style: {
      cursor: 'pointer',
      fontSize: 13
    }
  }, "Let the table enter real-life dice results instead of auto-rolling")), /*#__PURE__*/React.createElement("div", {
    className: "settings-hint"
  }, "When on, everyone's attacks and saving throws let them type the numbers they rolled at the table. Only you (the DM) can turn this on; players follow this setting automatically."))), /*#__PURE__*/React.createElement("div", {
    className: "settings-section"
  }, /*#__PURE__*/React.createElement("div", {
    className: "settings-hint",
    style: {
...
// Weapons
{
  ...
  cost: '2 gp',
  weight: '1 lb',
  desc: 'Hardened thorn blade, throwable.'
}, {
  cat: 'Weapons',
  name: 'Root Club',
  cost: '1 sp',
  weight: '2 lb',
  desc: 'Knotted root used as a club.'
}, {
  cat: 'Weapons',
  name: 'Fallen Branch',
  cost: '2 sp',
  weight: '8 lb',
  desc: 'Heavy limb used as a greatclub.'
}, {
  cat: 'Weapons',
  name: 'Bark Cleaver',
  cost: '5 gp',
  weight: '2 lb',
  desc: 'Light axe used for forestry work.'
}, {
  cat: 'Weapons',
  name: 'Reed Spear',
  cost: '1 gp',
  weight: '3 lb',
  desc: 'Flexible spear made from giant reeds.'
}, {
  cat: 'Weapons',
  name: 'Nutcracker Hammer',
  cost: '2 gp',
  weight: '2 lb',
  desc: 'Small hammer used by scouts and smiths.'
}, {
  cat: 'Weapons',
  name: 'Harvest Hook',
  cost: '1 gp',
  weight: '2 lb',
  desc: 'Curved farming blade.'
}, {
  cat: 'Weapons',
  name: 'Vine Lash',
  cost: '2 gp',
  weight: '2 lb',
  desc: 'Braided vine whip.'
}, {
  cat: 'Weapons',
  name: 'Scout Sword',
  cost: '10 gp',
  weight: '2 lb',
  desc: 'Standard sidearm of Groveclan scouts.'
}, {
  cat: 'Weapons',
  name: 'Bannerblade',
  cost: '15 gp',
  weight: '3 lb',
  desc: 'Long sword carried by officers and champions.'
}, {
  cat: 'Weapons',
  name: 'Badger Axe',
  cost: '10 gp',
  weight: '4 lb',
  desc: 'Broad-bladed martial axe favored by badgers.'
}, {
  cat: 'Weapons',
  name: 'Briar Glaive',
  cost: '20 gp',
  weight: '6 lb',
  desc: 'Polearm tipped with thornsteel.'
}, {
  cat: 'Weapons',
  name: 'Root Maul',
  cost: '10 gp',
  weight: '10 lb',
  desc: 'Massive two-handed hammer.'
}, {
  cat: 'Weapons',
  name: 'Burr Mace',
  cost: '15 gp',
  weight: '4 lb',
  desc: 'Club crowned with hardened burr clusters.'
}, {
  cat: 'Weapons',
  name: 'Hazel Bow',
  cost: '25 gp',
  weight: '2 lb',
  desc: 'Common hunting bow.'
}, {
  cat: 'Weapons',
  name: 'Yew Longbow',
  cost: '50 gp',
  weight: '2 lb',
  desc: 'Military bow of exceptional range.'
}, {
  cat: 'Weapons',
  name: 'Spring Bow',
  cost: '25 gp',
  weight: '5 lb',
  desc: 'Castorman mechanical crossbow.'
}, {
  cat: 'Weapons',
  name: 'Reedpipe',
  cost: '10 gp',
  weight: '1 lb',
  desc: 'Silent dart weapon.'
}, {
  cat: 'Weapons',
  name: 'Spider Silk Net',
  cost: '5 gp',
  weight: '2 lb',
  desc: 'Strong net woven from giant spider silk.'
},
// Armor
{
  cat: 'Armor',
  ...
  cost: '5 gp',
  weight: '8 lb',
  desc: 'Layered moss and padding.'
}, {
  cat: 'Armor',
  name: 'Tanned Hide',
  cost: '10 gp',
  weight: '10 lb',
  desc: 'Standard light armor.'
}, {
  cat: 'Armor',
  name: 'Acorn Plate',
  cost: '45 gp',
  weight: '13 lb',
  desc: 'Reinforced shell armor made from giant acorn segments.'
}, {
  cat: 'Armor',
  name: 'Beetle Shell Harness',
  cost: '10 gp',
  weight: '12 lb',
  desc: 'Hardened insect carapace armor.'
}, {
  cat: 'Armor',
  name: 'Fish Scale Armor',
  cost: '50 gp',
  weight: '45 lb',
  desc: 'Overlapping river fish scales.'
}, {
  cat: 'Armor',
  name: 'Hazelnut Plate',
  cost: '400 gp',
  weight: '65 lb',
  desc: 'Heavy Harefolk military armor.'
}, {
  cat: 'Armor',
  name: 'Elder Knight Plate',
  cost: '1500 gp',
  weight: '65 lb',
  desc: 'Rare masterwork armor.'
}, {
  cat: 'Armor',
  name: 'Bark Shield',
  cost: '10 gp',
  weight: '6 lb',
  desc: 'Shield carved from spirit oak bark.'
},
// Adventuring Gear
{
  cat: 'Gear',
  ...
  cost: '2 gp',
  weight: '5 lb',
  desc: 'Backpack with many pouches.'
}, {
  cat: 'Gear',
  name: 'Moss Roll',
  cost: '1 gp',
  weight: '5 lb',
  desc: 'Waterproof sleeping roll.'
}, {
  cat: 'Gear',
  name: 'Acorn Flask',
  cost: '2 sp',
  weight: '2 lb',
  desc: 'Holds water or nectar.'
}, {
  cat: 'Gear',
  name: 'Resin Torch',
  cost: '1 cp',
  weight: '1 lb',
  desc: 'Burns for 1 hour.'
}, {
  cat: 'Gear',
  name: 'Glowbug Lantern',
  cost: '5 gp',
  weight: '2 lb',
  desc: 'Lantern containing captive glow beetles.'
}, {
  cat: 'Gear',
  name: 'Sunflower Oil Flask',
  cost: '1 sp',
  weight: '1 lb',
  desc: 'Fuel for lamps and lanterns.'
}, {
  cat: 'Gear',
  name: 'Spider Silk Rope (50 ft)',
  cost: '2 gp',
  weight: '5 lb',
  desc: 'Lightweight climbing rope.'
}, {
  cat: 'Gear',
  name: 'Climbing Claw',
  cost: '2 gp',
  weight: '3 lb',
  desc: 'Hooked climbing tool.'
}, {
  cat: 'Gear',
  name: 'Root Prybar',
  cost: '2 gp',
  weight: '5 lb',
  desc: 'Used for opening roots and doors.'
}, {
  cat: 'Gear',
  name: 'Leaf Shelter',
  cost: '2 gp',
  weight: '15 lb',
  desc: 'One-person field shelter.'
}, {
  cat: 'Gear',
  name: "Herbalist's Satchel",
  cost: '5 gp',
  weight: '3 lb',
  desc: 'Used to stabilize and treat wounds.'
}, {
  cat: 'Gear',
  name: "Scout's Horn",
  cost: '3 gp',
  weight: '2 lb',
  desc: 'Signal instrument for patrols.'
}, {
  cat: 'Gear',
  name: 'Memory Thread Bundle',
  cost: '1 gp',
  weight: '1 lb',
  desc: 'Colored thread used for records and messages.'
},
// Consumables
{
  cat: 'Consumables',
  ...
  cost: '2 cp',
  weight: '0.5 lb',
  desc: 'Dense bread made from root flour.'
}, {
  cat: 'Consumables',
  name: 'Hard Cheese',
  cost: '1 sp',
  weight: '1 lb',
  desc: 'Preserved travel food.'
}, {
  cat: 'Consumables',
  name: 'Smoked Fish',
  cost: '3 sp',
  weight: '1 lb',
  desc: 'Common preserved protein.'
}, {
  cat: 'Consumables',
  name: 'Roasted Sunflower Seeds',
  cost: '5 cp',
  weight: '1 lb',
  desc: 'Staple snack of the grasslands.'
}, {
  cat: 'Consumables',
  name: 'Ant Mead (Mug)',
  cost: '4 cp',
  weight: '1 lb',
  desc: 'Sweet alcoholic drink from Dexter ant farms.'
}, {
  cat: 'Consumables',
  name: 'Ant Mead (Jug)',
  cost: '2 sp',
  weight: '8 lb',
  desc: 'Common bulk supply.'
}, {
  cat: 'Consumables',
  name: 'Berry Wine',
  cost: '2 sp',
  ...
  cat: 'Consumables',
  name: 'Moonberry Reserve',
  cost: '10 gp',
  weight: '3 lb',
  desc: 'Premium vintage wine.'
}, {
  cat: 'Consumables',
  name: 'Scout Rations (1 Day)',
  cost: '5 sp',
  weight: '2 lb',
  desc: 'Dried roots, seeds and smoked fish.'
},
// Medicines & Alchemy
{
  cat: 'Medicine',
  name: 'Golden Sap Elixir',
  cost: '50 gp',
  weight: '0.5 lb',
  desc: 'Restores 2d4+2 hit points.'
}, {
  cat: 'Medicine',
  name: 'Blessed Springwater',
  cost: '25 gp',
  weight: '1 lb',
  desc: 'Consecrated water used against spirits and corruption.'
}, {
  cat: 'Medicine',
  name: 'Talpa Antidote',
  cost: '50 gp',
  weight: '0.5 lb',
  desc: 'Grants advantage against poison for 1 hour.'
}, {
  cat: 'Medicine',
  name: 'Glowcap Poultice',
  cost: '15 gp',
  weight: '0.5 lb',
  desc: 'Removes one level of exhaustion after a rest.'
}, {
  cat: 'Medicine',
  name: 'Spore Suppressant',
  cost: '20 gp',
  weight: '0.5 lb',
  desc: 'Protects against fungal spores for 8 hours.'
}, {
  cat: 'Medicine',
  name: 'Briarthorn Salve',
  cost: '5 gp',
  weight: '0.5 lb',
  desc: 'Heals cuts and thorn wounds.'
},
// Cultural - Groveclans
{
  cat: 'Groveclans',
  ...
  cost: '5 sp',
  weight: '0.2 lb',
  desc: 'Ceremonial stitching needle.'
}, {
  cat: 'Groveclans',
  name: 'Family Patch',
  cost: 'Priceless',
  weight: 'Negligible',
  desc: 'Embroidered family insignia.'
}, {
  cat: 'Groveclans',
  name: 'Scout Harness',
  cost: '3 gp',
  weight: '2 lb',
  desc: 'Climbing gear for canopy travel.'
},
// Cultural - Harefolk
{
  cat: 'Harefolk',
  name: 'Hazelnut Helmet',
  cost: '8 gp',
  weight: '4 lb',
  desc: 'Hardened nut-shell military helmet.'
}, {
  cat: 'Harefolk',
  name: "Watchman's Horn",
  cost: '3 gp',
  weight: '2 lb',
  desc: 'Used to sound alarms.'
}, {
  cat: 'Harefolk',
  name: 'Boundary Marker Kit',
  cost: '1 gp',
  weight: '5 lb',
  desc: 'Used to mark roads and borders.'
},
// Cultural - Castormen
{
  cat: 'Castormen',
  ...
  cost: '10 gp',
  weight: '3 lb',
  desc: 'Tools for maintaining machinery.'
}, {
  cat: 'Castormen',
  name: 'Pressure Gauge',
  cost: '15 gp',
  weight: '2 lb',
  desc: 'Measures hydraulic pressure.'
}, {
  cat: 'Castormen',
  name: "Surveyor's Wheel",
  cost: '5 gp',
  weight: '4 lb',
  desc: 'Measures distance and terrain.'
},
// Cultural - Otterfolk
{
  cat: 'Otterfolk',
  ...
  cost: '4 gp',
  weight: '1 lb',
  desc: 'Utility knife used by oyster farmers.'
}, {
  cat: 'Otterfolk',
  name: 'Oyster Hook',
  cost: '2 gp',
  weight: '2 lb',
  desc: 'Harvesting tool for oyster beds.'
}, {
  cat: 'Otterfolk',
  name: 'Tide Compass',
  cost: '20 gp',
  weight: '1 lb',
  desc: 'Navigation aid used by seafarers.'
},
// Cultural - Desmanfolk
{
  cat: 'Desmanfolk',
  ...
  cost: '8 gp',
  weight: '1 lb',
  desc: 'Protects against fungal spores.'
}, {
  cat: 'Desmanfolk',
  name: 'Mycelial Tablet',
  cost: '25 gp',
  weight: '2 lb',
  desc: 'Living fungal record medium.'
}, {
  cat: 'Desmanfolk',
  name: 'Fungus Cultivation Box',
  cost: '10 gp',
  weight: '5 lb',
  desc: 'Portable fungal garden.'
},
// Cultural - Ravenfolk
{
  cat: 'Ravenfolk',
  ...
  cost: '5 gp',
  weight: '0.5 lb',
  desc: 'Used to draw protective symbols.'
}, {
  cat: 'Ravenfolk',
  name: 'Spirit Bell',
  cost: '15 gp',
  weight: '1 lb',
  desc: 'Chimes when spirits are near.'
}, {
  cat: 'Ravenfolk',
  name: 'Blackfeather Cloak',
  cost: '12 gp',
  weight: '3 lb',
  desc: 'Traditional Ravenfolk cloak.'
},
// Mounts & Beasts
{
  cat: 'Mounts',
  ...
  cost: '8 gp',
  weight: '-',
  desc: 'Carries up to 200 lbs of cargo.'
}, {
  cat: 'Mounts',
  name: 'Rhinoceros Beetle',
  cost: '75 gp',
  weight: '-',
  desc: 'Armored war mount.'
}, {
  cat: 'Mounts',
  name: 'Forest Gecko',
  cost: '50 gp',
  weight: '-',
  desc: 'Agile climbing mount.'
}, {
  cat: 'Mounts',
  name: 'Marsh Gecko',
  cost: '50 gp',
  weight: '-',
  desc: 'Swamp-adapted riding animal.'
}, {
  cat: 'Mounts',
  name: 'Messenger Crow',
  cost: '25 gp',
  weight: '-',
  desc: 'Carries messages between settlements.'
}, {
  cat: 'Mounts',
  name: 'Ant Queen (Young)',
  cost: '200 gp',
  weight: '-',
  desc: 'Extremely valuable Dexter livestock.'
}];
...
function InventoryItemPicker({
  ...
  const items = useMemo(() => {
    ...
    return PRESET_ITEMS.filter(i => (cat === 'All' || i.cat === cat) && (!query || i.name.toLowerCase().includes(query) || i.desc.toLowerCase().includes(query)));
  }, [q, cat]);
  return /*#__PURE__*/React.createElement("div", {
    className: "csheet-block"
  }, /*#__PURE__*/React.createElement("button", {
    type: "button",
    className: `item-picker-toggle ${open ? 'open' : ''}`,
    onClick: () => setOpen(o => !o)
  }, /*#__PURE__*/React.createElement("span", {
    className: "item-picker-chevron"
  }, open ? '▾' : '▸'), "Add item from list", /*#__PURE__*/React.createElement("span", {
    className: "item-picker-count"
  }, PRESET_ITEMS.length)), open && /*#__PURE__*/React.createElement("div", {
    className: "item-picker"
  }, /*#__PURE__*/React.createElement("div", {
    className: "item-picker-controls"
  }, /*#__PURE__*/React.createElement("input", {
  ...
  }, "No matching items.") : items.map((i, idx) => /*#__PURE__*/React.createElement("div", {
    key: idx,
    className: "item-picker-row"
  }, /*#__PURE__*/React.createElement("div", {
    className: "item-picker-info"
  }, /*#__PURE__*/React.createElement("div", {
    className: "item-picker-name"
  }, i.name, /*#__PURE__*/React.createElement("span", {
    className: "item-picker-meta"
  }, i.cost, i.weight && i.weight !== '-' ? ` · ${i.weight}` : '')), /*#__PURE__*/React.createElement("div", {
    className: "item-picker-desc"
  }, i.desc)), /*#__PURE__*/React.createElement("button", {
    type: "button",
    className: "item-picker-add",
    title: `Add ${i.name} to inventory`,
    onClick: () => onAdd(i)
  }, "＋"))))));
}
...
const DND_RACES = [{
  ...
  size: 'Medium',
  height: "5'10\"-6'8\"",
  bonus: 'Breath weapon, damage resistance by ancestry'
}, {
  name: 'Dwarf',
  speed: 25,
  size: 'Medium',
  height: "4'0\"-5'0\"",
  bonus: 'Darkvision, poison resilience, stonecunning'
}, {
  name: 'Elf',
  speed: 30,
  size: 'Medium',
  height: "5'0\"-6'6\"",
  bonus: 'Darkvision, Fey Ancestry, Trance, keen senses'
}, {
  name: 'Gnome',
  speed: 25,
  size: 'Small',
  height: "3'0\"-4'0\"",
  bonus: 'Darkvision, Gnome Cunning (INT/WIS/CHA saves vs magic)'
}, {
  name: 'Half-Elf',
  speed: 30,
  size: 'Medium',
  height: "5'0\"-6'2\"",
  bonus: 'Darkvision, Fey Ancestry, two extra skills'
}, {
  name: 'Half-Orc',
  speed: 30,
  size: 'Medium',
  height: "5'4\"-6'10\"",
  bonus: 'Darkvision, Relentless Endurance, Savage Attacks'
}, {
  name: 'Halfling',
  speed: 25,
  size: 'Small',
  height: "2'7\"-3'5\"",
  bonus: 'Lucky, Brave, Halfling Nimbleness'
}, {
  name: 'Human',
  speed: 30,
  size: 'Medium',
  height: "4'8\"-6'2\"",
  bonus: 'Versatile - +1 to all scores (or a feat, variant)'
}, {
  name: 'Tiefling',
  speed: 30,
  size: 'Medium',
  height: "5'0\"-6'2\"",
  bonus: 'Darkvision, Hellish Resistance (fire), innate magic'
}];
...
const CLASS_INFO = {
  Barbarian: {
    ...
    equipment: 'A: Greataxe, 4 Handaxes, Explorer’s Pack, 15 GP  •  B: 75 GP'
  },
  Bard: {
    primary: 'CHA',
    saves: 'DEX & CHA',
    ...
    tools: 'None',
    armor: 'Light armor',
    equipment: 'A: Leather Armor, 2 Daggers, a Musical Instrument, Entertainer’s Pack, 19 GP  •  B: 90 GP'
  },
  Cleric: {
    primary: 'WIS',
    saves: 'WIS & CHA',
    ...
    tools: 'None',
    armor: 'Light & Medium armor, Shields',
    equipment: 'A: Chain Shirt, Shield, Mace, Holy Symbol, Priest’s Pack  •  B: 110 GP'
  },
  Druid: {
    primary: 'WIS',
    saves: 'INT & WIS',
    ...
    equipment: 'A: Leather Armor, Shield, Sickle, Druidic Focus (Quarterstaff), Explorer’s Pack, Herbalism Kit, 9 GP  •  B: 50 GP'
  },
  Fighter: {
    primary: 'STR or DEX',
    saves: 'STR & CON',
    ...
    equipment: 'A: Chain Mail, Greatsword, Flail, 8 Javelins, Dungeoneer’s Pack, 4 GP  •  B: Studded Leather, Scimitar, Shortsword, Longbow, 20 Arrows, Quiver, Dungeoneer’s Pack, 11 GP  •  C: 155 GP'
  },
  Monk: {
    primary: 'DEX & WIS',
    saves: 'STR & DEX',
    skills: 'Choose 2: Acrobatics, Athletics, History, Insight, Religion, Stealth',
    weapons: 'Simple & Martial weapons with the Light property',
    tools: 'One Artisan’s Tools or Musical Instrument',
    armor: 'None',
    equipment: 'A: Spear, 5 Daggers, Artisan’s Tools or Instrument, Explorer’s Pack, 11 GP  •  B: 50 GP'
  },
  Paladin: {
    primary: 'STR & CHA',
    saves: 'WIS & CHA',
    ...
    tools: 'None',
    armor: 'Light, Medium & Heavy armor, Shields',
    equipment: 'A: Chain Mail, Shield, Longsword, 6 Javelins, Holy Symbol, Priest’s Pack  •  B: 150 GP'
  },
  Ranger: {
    primary: 'DEX & WIS',
    saves: 'STR & DEX',
    ...
    tools: 'None',
    armor: 'Light & Medium armor, Shields',
    equipment: 'A: Studded Leather, Scimitar, Shortsword, Longbow, 20 Arrows, Quiver, Druidic Focus (Sprig of Mistletoe), Explorer’s Pack, 7 GP  •  B: 150 GP'
  },
  Rogue: {
    primary: 'DEX',
    saves: 'DEX & INT',
    ...
    weapons: 'Simple & Martial weapons with Finesse or Light',
    tools: 'Thieves’ Tools',
    armor: 'Light armor',
    equipment: 'A: Leather Armor, 2 Daggers, Shortsword, Shortbow, 20 Arrows, Quiver, Thieves’ Tools, Burglar’s Pack, 8 GP  •  B: 100 GP'
  },
  Sorcerer: {
    primary: 'CHA',
    saves: 'CON & CHA',
    ...
    tools: 'None',
    armor: 'None',
    equipment: 'A: Spear, 2 Daggers, Arcane Focus (crystal), Dungeoneer’s Pack, 28 GP  •  B: 50 GP'
  },
  Warlock: {
    primary: 'CHA',
    saves: 'WIS & CHA',
    ...
    tools: 'None',
    armor: 'Light armor',
    equipment: 'A: Leather Armor, Sickle, 2 Daggers, Arcane Focus (orb), Book (occult lore), Scholar’s Pack, 15 GP  •  B: 100 GP'
  },
  Wizard: {
    primary: 'INT',
    saves: 'INT & WIS',
    ...
    tools: 'None',
    armor: 'None',
    equipment: 'A: 2 Daggers, Arcane Focus (Quarterstaff), Robe, Spellbook, Scholar’s Pack, 5 GP  •  B: 55 GP'
  }
};
...
function ClassSelect({
  ...
  if (editing) {
    return /*#__PURE__*/React.createElement("div", {
      className: "class-select-wrap"
    }, /*#__PURE__*/React.createElement(LiveInput, {
      value: v,
      onCommit: onCommit,
  ...
  return /*#__PURE__*/React.createElement("select", {
    ...
    onChange: e => {
      ...
      if (nv === '__other') setEditing(true);else onCommit(nv);
    }
  }, /*#__PURE__*/React.createElement("option", {
    value: ""
  }, "- Class -"), v && !known && /*#__PURE__*/React.createElement("option", {
    value: v
  }, v), DND_CLASSES.map(c => /*#__PURE__*/React.createElement("option", {
    key: c,
    value: c
...
function RaceSelect({
  ...
  return /*#__PURE__*/React.createElement("select", {
    ...
    value: ""
  }, "- Race -"), v && !known && /*#__PURE__*/React.createElement("option", {
    value: v
  }, v), names.map(c => /*#__PURE__*/React.createElement("option", {
    key: c,
    value: c
  }, c)), /*#__PURE__*/React.createElement("option", {
    value: "__other"
...
function LevelUpHpRoller({
  ...
  useEffect(() => () => {
    ...
    tmr.current.forEach(clearTimeout);
  }, []);
  const go = () => {
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    className: "hp-final"
  }, "Rolled ", /*#__PURE__*/React.createElement("b", null, face), " ", conMod >= 0 ? '+' : '', conMod, " CON = ", /*#__PURE__*/React.createElement("b", null, "+", Math.max(1, face + conMod)), " HP") : /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("button", {
    className: "btn primary sm",
    onClick: go,
    disabled: rolling
  }, rolling ? 'Rolling…' : `🎲 Roll d${die} for your new level`), face != null && /*#__PURE__*/React.createElement("div", {
    className: "hp-cur"
  }, face)));
}
...
function newWeaponAttack() {
  return {
    id: uid('atk_'),
    name: 'Strike',
    toHit: 0,
    range: 5,
    damage: [newWeaponComp()]
  };
}
...
function armorProfsFor(entity) {
  ...
  if (!entity || entity.type !== 'PC' || !info) return {
    light: true,
    medium: true,
    heavy: true,
    shield: true
  ...
  return {
    ...
    medium: s.includes('medium'),
    heavy: s.includes('heavy'),
    shield: s.includes('shield')
  };
}
...
function canUseShield(entity) {
  return !!armorProfsFor(entity).shield;
}
...
function handsTotalOf(entity) {
  const n = Number(entity?.handsTotal);
...
function armorFromPreset(p) {
  return {
    ...p
  };
}
function shieldFromPreset(p) {
  return {
    ...p
  };
}
...
function parseAttacksFromAbilities(text) {
  ...
  const re = new RegExp('([A-Z][A-Za-z\'’ /-]{1,40}?)\\.\\s*(?:Melee|Ranged|Melee or Ranged)\\s+(?:Weapon\\s+)?Attack:\\s*\\+(\\d+)\\s*to hit' + '(?:[,;]?\\s*(?:reach|range)\\s*(\\d+)(?:/\\d+)?\\s*ft\\.?)?\\s*' + sep + '\\s*([0-9dD+ ,a-zA-Z]+)', 'g');
  ...
  while ((m = re.exec(text)) !== null && guard++ < 40) {
    ...
    while ((d = dmgRe.exec(m[4])) !== null && g2++ < 8) {
      ...
      damage.push({
        count: Math.min(20, parseInt(d[1]) || 1),
        sides: parseInt(d[2]) || 6,
        modifier: d[3] ? parseInt(d[3]) : 0,
        type
      });
    }
...
function ArmorManager({
  ...
  const equipArmor = p => {
    ...
    if (!profs[p.category]) {
      alert(`Your class is not proficient with ${p.category} armour.`);
      return;
    }
  ...
  if (!canEdit) {
    return /*#__PURE__*/React.createElement("div", {
      className: "csheet-block"
    }, /*#__PURE__*/React.createElement("label", null, "Armour & Shield"), /*#__PURE__*/React.createElement("div", {
      className: "armor-ro"
    }, /*#__PURE__*/React.createElement("div", null, "Armour: ", /*#__PURE__*/React.createElement("b", null, armor ? armor.name : 'None'), shield ? /*#__PURE__*/React.createElement("span", null, " · Shield: ", /*#__PURE__*/React.createElement("b", null, shield.name)) : null), /*#__PURE__*/React.createElement("div", null, "AC ", /*#__PURE__*/React.createElement("b", {
      className: "mono"
    }, finalAc), stealthDis ? /*#__PURE__*/React.createElement("span", {
      className: "armor-warn"
    }, " · Stealth disadvantage") : null)));
  }
  return /*#__PURE__*/React.createElement("div", {
    className: "csheet-block"
  }, /*#__PURE__*/React.createElement("label", null, "Armour & Shield ", /*#__PURE__*/React.createElement("span", {
    className: "hands-readout",
    title: "Resulting armour class"
  }, "🛡 AC ", finalAc, stealthDis ? ' · stealth ▼' : '')), /*#__PURE__*/React.createElement("div", {
    className: "armor-row"
  }, /*#__PURE__*/React.createElement("span", {
    className: "armor-lbl"
  }, "Body"), /*#__PURE__*/React.createElement("select", {
    ...
    value: ""
  }, "- Unarmoured (10 + Dex) -"), ['light', 'medium', 'heavy'].map(cat => /*#__PURE__*/React.createElement("optgroup", {
    key: cat,
    label: `${cat[0].toUpperCase() + cat.slice(1)}${profs[cat] ? '' : ' (not proficient)'}`
  }, ARMOR_PRESETS.filter(a => a.category === cat).map(a => /*#__PURE__*/React.createElement("option", {
    key: a.name,
    value: a.name,
    disabled: !profs[cat]
  }, a.name, " - AC ", a.baseAc, a.dexCap > 0 ? `+Dex${a.dexCap < 90 ? ` (max ${a.dexCap})` : ''}` : '', a.strReq ? `, STR ${a.strReq}` : '', a.stealthDis ? ', stealth ▼' : '')))))), armor && armor.strReq > strScore && /*#__PURE__*/React.createElement("div", {
    className: "armor-warn"
  }, "⚠ ", armor.name, " needs STR ", armor.strReq, " (you have ", strScore, ") - your speed would drop by 10 ft."), /*#__PURE__*/React.createElement("div", {
    className: "armor-row"
  }, /*#__PURE__*/React.createElement("span", {
    className: "armor-lbl"
  }, "Shield"), shield ? /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("select", {
    className: "armor-select",
    value: shield.name,
    disabled: !profs.shield,
    onChange: e => setShieldPreset(e.target.value)
  }, SHIELD_PRESETS.map(s => /*#__PURE__*/React.createElement("option", {
    key: s.name,
    value: s.name
  }, s.name, " - +", s.acBonus, " AC", s.strReq ? `, STR ${s.strReq}` : '', s.stealthDis ? ', stealth ▼' : ''))), /*#__PURE__*/React.createElement("button", {
    className: "btn sm ghost",
    onClick: () => equipShield(false)
  }, "Remove")) : /*#__PURE__*/React.createElement("button", {
    className: "btn sm",
    disabled: !profs.shield,
    title: profs.shield ? 'Equip a shield' : 'Not proficient with shields',
    onClick: () => equipShield(true)
  }, "+ Equip shield")), /*#__PURE__*/React.createElement("div", {
    className: "armor-hint"
...
function WeaponManager({
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      onChangeRef.current(weaponsRef.current);
      committedRef.current = weaponsRef.current;
    }, 300);
  ...
  const toggleEquip = w => {
    ...
    const free = handsTotalOf(entity) - occupiedHandsOf({
      ...entity,
      weapons
    });
  ...
  const pullFromStatBlock = () => {
    ...
    if (parsed.length === 0) {
      alert('No attacks found in the stat block to import.');
      return;
    }
    setWeapons(ws => {
  ...
  const patchWeapon = (id, patch) => setWeapons(ws => ws.map(w => w.id === id ? {
    ...w,
    ...patch
  } : w));
  ...
  const patchAttack = (wid, aid, patch) => setWeapons(ws => ws.map(w => w.id !== wid ? w : {
    ...w,
    attacks: w.attacks.map(a => a.id === aid ? {
      ...a,
      ...patch
    } : a)
  }));
  const addAttack = wid => setWeapons(ws => ws.map(w => w.id === wid ? {
    ...w,
    attacks: [...w.attacks, newWeaponAttack()]
  } : w));
  const removeAttack = (wid, aid) => setWeapons(ws => ws.map(w => w.id === wid ? {
    ...w,
    attacks: w.attacks.filter(a => a.id !== aid)
  } : w));
  const patchComp = (wid, aid, ci, patch) => setWeapons(ws => ws.map(w => w.id !== wid ? w : {
    ...w,
    attacks: w.attacks.map(a => a.id !== aid ? a : {
      ...a,
      damage: a.damage.map((c, i) => i === ci ? {
        ...c,
        ...patch
      } : c)
    })
  ...
  const removeComp = (wid, aid, ci) => setWeapons(ws => ws.map(w => w.id !== wid ? w : {
    ...
    attacks: w.attacks.map(a => a.id !== aid ? a : {
      ...a,
      damage: a.damage.filter((_, i) => i !== ci)
    })
  }));
  if (!canEdit) {
    ...
    return /*#__PURE__*/React.createElement("div", {
      className: "csheet-block"
    }, /*#__PURE__*/React.createElement("label", null, "Weapons"), /*#__PURE__*/React.createElement("div", {
      className: "wpn-ro-list"
    }, weapons.map(w => /*#__PURE__*/React.createElement("div", {
      key: w.id,
      className: "wpn-ro"
    }, /*#__PURE__*/React.createElement("div", {
      className: "wpn-ro-head"
    }, /*#__PURE__*/React.createElement("span", {
      className: "wpn-ro-name"
    }, w.name), w.equipped && /*#__PURE__*/React.createElement("span", {
      className: "wpn-eq-badge"
    }, "EQUIPPED")), (w.attacks || []).map(a => /*#__PURE__*/React.createElement("div", {
      key: a.id,
      className: "wpn-ro-atk"
    }, /*#__PURE__*/React.createElement("b", null, a.name, ":"), " ", formatAttack(a)))))));
  }
  return /*#__PURE__*/React.createElement("div", {
    className: "csheet-block"
  }, /*#__PURE__*/React.createElement("label", null, "Weapons ", /*#__PURE__*/React.createElement("span", {
    className: "hands-readout",
    title: "Hands in use / total"
  }, "✋ ", occupiedHandsOf({
    ...entity,
    weapons
  }), "/", handsTotalOf(entity), " hands")), /*#__PURE__*/React.createElement("div", {
    className: "wpn-list"
  }, weapons.map(w => /*#__PURE__*/React.createElement("div", {
    key: w.id,
    className: `wpn-card ${w.equipped ? 'equipped' : ''}`
  }, /*#__PURE__*/React.createElement("div", {
    className: "wpn-card-head"
  }, /*#__PURE__*/React.createElement("button", {
    className: `wpn-eq-toggle ${w.equipped ? 'on' : ''}`,
    title: w.equipped ? 'Equipped' : 'Not equipped',
    onClick: () => toggleEquip(w)
  }, w.equipped ? '★ Equipped' : '☆ Unequipped'), /*#__PURE__*/React.createElement("input", {
    className: "wpn-name",
    value: w.name,
    onChange: e => patchWeapon(w.id, {
      name: e.target.value
    }),
    ...
    className: "wpn-hands",
    value: w.hands || 1,
    title: "Hands required",
    onChange: e => patchWeapon(w.id, {
      hands: Number(e.target.value)
    })
  }, /*#__PURE__*/React.createElement("option", {
    value: 1
    ...
    title: openId === w.id ? 'Collapse' : 'Edit attacks',
    onClick: () => setOpenId(openId === w.id ? null : w.id)
  }, openId === w.id ? '▾' : '▸'), /*#__PURE__*/React.createElement("button", {
    className: "wpn-icon-btn danger",
    title: "Delete weapon",
    onClick: () => {
      if (confirm(`Delete ${w.name}?`)) removeWeapon(w.id);
    }
  }, "×")), openId !== w.id && /*#__PURE__*/React.createElement("div", {
    className: "wpn-card-summary"
  }, (w.attacks || []).map(a => /*#__PURE__*/React.createElement("div", {
    key: a.id,
    className: "wpn-sum-line"
  }, /*#__PURE__*/React.createElement("b", null, a.name, ":"), " ", formatAttack(a)))), openId === w.id && /*#__PURE__*/React.createElement("div", {
    className: "wpn-atks"
  }, (w.attacks || []).map(a => /*#__PURE__*/React.createElement("div", {
    key: a.id,
    className: "wpn-atk"
  }, /*#__PURE__*/React.createElement("div", {
    className: "wpn-atk-top"
  }, /*#__PURE__*/React.createElement("input", {
    className: "wpn-atk-name",
    value: a.name,
    onChange: e => patchAttack(w.id, a.id, {
      name: e.target.value
    }),
    ...
    value: a.toHit,
    onChange: e => patchAttack(w.id, a.id, {
      toHit: parseInt(e.target.value) || 0
    })
  }), " hit"), /*#__PURE__*/React.createElement("label", {
    className: "wpn-mini"
  }, /*#__PURE__*/React.createElement("input", {
    type: "number",
    value: a.range,
  ...
  }), " ft"), w.attacks.length > 1 && /*#__PURE__*/React.createElement("button", {
    className: "wpn-icon-btn danger",
    title: "Remove attack",
    onClick: () => removeAttack(w.id, a.id)
  }, "×")), (a.damage || []).map((c, ci) => /*#__PURE__*/React.createElement("div", {
    key: ci,
    className: "wpn-comp"
  }, /*#__PURE__*/React.createElement("input", {
    type: "number",
    ...
    value: c.count,
    min: 1,
    onChange: e => patchComp(w.id, a.id, ci, {
      count: Math.max(1, parseInt(e.target.value) || 1)
    })
  }), /*#__PURE__*/React.createElement("select", {
    className: "wpn-c-sides",
    value: c.sides,
    onChange: e => patchComp(w.id, a.id, ci, {
      sides: parseInt(e.target.value)
    })
  }, ALLOWED_DIE_SIDES.map(s => /*#__PURE__*/React.createElement("option", {
    key: s,
    value: s
  }, "d", s))), /*#__PURE__*/React.createElement("span", {
    className: "wpn-c-plus"
  }, "+"), /*#__PURE__*/React.createElement("input", {
    type: "number",
    className: "wpn-c-num",
    value: c.modifier,
    ...
    placeholder: "type"
  }), a.damage.length > 1 && /*#__PURE__*/React.createElement("button", {
    className: "wpn-icon-btn danger sm",
    title: "Remove damage",
    onClick: () => removeComp(w.id, a.id, ci)
  }, "×"))), /*#__PURE__*/React.createElement("button", {
    className: "wpn-add-comp",
    onClick: () => addComp(w.id, a.id)
  }, "+ damage type"), !a.effect ? /*#__PURE__*/React.createElement("button", {
    className: "wpn-add-comp wpn-add-effect",
    onClick: () => patchAttack(w.id, a.id, {
      effect: {
        condition: 'Poisoned',
        save: {
          ability: 'CON',
  ...
  }, "+ inflict effect (poison, disease…)") : /*#__PURE__*/React.createElement("div", {
    className: "wpn-effect"
  }, /*#__PURE__*/React.createElement("div", {
    className: "wpn-effect-row"
  }, /*#__PURE__*/React.createElement("span", {
    className: "wpn-effect-label"
  }, "⚠ On hit:"), /*#__PURE__*/React.createElement("select", {
    className: "wpn-eff-cond",
    value: a.effect.condition,
    onChange: e => patchAttack(w.id, a.id, {
      effect: {
        ...a.effect,
  ...
  }), " requires save"), a.effect.save ? /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("select", {
    className: "wpn-eff-abil",
    value: a.effect.save.ability,
    onChange: e => patchAttack(w.id, a.id, {
      effect: {
        ...a.effect,
        save: {
  ...
  }, ABILITIES.map(ab => /*#__PURE__*/React.createElement("option", {
    key: ab,
    value: ab
  }, ab))), /*#__PURE__*/React.createElement("label", {
    className: "wpn-mini"
  }, "DC ", /*#__PURE__*/React.createElement("input", {
    type: "number",
    className: "wpn-c-num",
    value: a.effect.save.dc,
    onChange: e => patchAttack(w.id, a.id, {
      effect: {
        ...a.effect,
        save: {
    ...
    className: "wpn-eff-auto"
  }, "always applies on a hit"))))), /*#__PURE__*/React.createElement("button", {
    className: "wpn-add-atk",
    onClick: () => addAttack(w.id)
  }, "+ add attack mode"))))), /*#__PURE__*/React.createElement("datalist", {
    id: "dmg-types"
  }, DAMAGE_TYPES.map(t => /*#__PURE__*/React.createElement("option", {
    key: t,
    value: t
  }))), /*#__PURE__*/React.createElement("div", {
    className: "wpn-add-row"
  }, /*#__PURE__*/React.createElement("button", {
    className: "btn sm wpn-add",
    onClick: addWeapon
  }, "+ Custom weapon"), /*#__PURE__*/React.createElement("select", {
    className: "wpn-preset-select",
    value: "",
    onChange: e => {
      if (e.target.value) {
    ...
    value: ""
  }, "+ Add from list…"), WEAPON_PRESETS.map(p => /*#__PURE__*/React.createElement("option", {
    key: p.name,
    value: p.name
  }, p.name)))), parseable && /*#__PURE__*/React.createElement("button", {
    className: "btn sm wpn-pull",
    onClick: pullFromStatBlock,
    title: "Create weapons from this creature's attack lines"
  }, "⚔ Pull attacks from stat block"));
}
function CharacterSheet({
  ...
  const setMoney = (coin, v) => onField({
    money: {
      ...money,
      [coin]: Math.max(0, Math.floor(Number(v) || 0))
    }
  });
  const uploadPortrait = async () => {
    const u = await pickCompressedImage();
    if (u) onField({
      imageUrl: u
    });
  };
  ...
  const addInventoryItem = it => {
    ...
    onField({
      inventory: cur.trim() ? cur.replace(/\s+$/, '') + '\n' + line : line
    });
  };
  ...
  const textBlock = (label, field, placeholder, hint) => /*#__PURE__*/React.createElement("div", {
    className: "csheet-block"
  }, /*#__PURE__*/React.createElement("label", null, label, hint && /*#__PURE__*/React.createElement("span", {
    className: "csheet-hint"
  }, " - ", hint)), /*#__PURE__*/React.createElement(LiveTextarea, {
    value: entity[field] || '',
    onCommit: v => onField({
      [field]: v
    }),
  ...
  const TABS = isPC ? [['core', '◆ Core'], ['combat', '⚔ Combat'], ['spells', '✦ Spells'], ['gear', '🜚 Gear'], ['story', '❧ Story']] : [['core', '◆ Core'], ['combat', '⚔ Combat'], ['gear', '🜚 Gear'], ['story', '❧ Story']];
  return /*#__PURE__*/React.createElement("div", {
    className: "csheet"
  }, /*#__PURE__*/React.createElement("div", {
    className: "csheet-tabbar"
  }, TABS.map(([k, l]) => /*#__PURE__*/React.createElement("button", {
    key: k,
    className: `csheet-tab ${tab === k ? 'active' : ''}`,
    onClick: () => setTab(k)
  }, l))), /*#__PURE__*/React.createElement("div", {
    className: "csheet-body"
  }, tab === 'core' && /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
    className: "csheet-identity"
  }, /*#__PURE__*/React.createElement("div", {
    className: "csheet-portrait",
    style: {
      background: entity.color
    }
  }, entity.imageUrl ? /*#__PURE__*/React.createElement("img", {
    ...
    className: "csheet-id-fields"
  }, /*#__PURE__*/React.createElement("div", {
    className: "csheet-row"
  }, /*#__PURE__*/React.createElement("div", {
    className: "csheet-field grow"
  }, /*#__PURE__*/React.createElement("label", null, "Name"), /*#__PURE__*/React.createElement(LiveInput, {
    value: entity.name,
    onCommit: v => onField({
      name: v
    })
  })), /*#__PURE__*/React.createElement("div", {
    className: "csheet-field"
  }, /*#__PURE__*/React.createElement("label", null, "Level"), requestMode ? (() => {
    const pend = ownRequests.find(r => r.kind === 'level_change' && r.status === 'pending');
    return /*#__PURE__*/React.createElement("div", {
      className: "req-inline"
    }, /*#__PURE__*/React.createElement("span", {
      className: "req-val mono"
    }, entity.level), pend ? /*#__PURE__*/React.createElement("span", {
      className: "req-pending"
    }, "⏳ → ", pend.payload.to) : /*#__PURE__*/React.createElement("span", {
      className: "req-btns"
    }, /*#__PURE__*/React.createElement("button", {
      className: "btn xs",
      type: "button",
      disabled: entity.level <= 1,
      onClick: () => onRequest({
        kind: 'level_change',
        data: {
          to: entity.level - 1
  ...
  })() : /*#__PURE__*/React.createElement(LiveNumberInput, {
    value: entity.level,
    onCommit: v => onField({
      level: Math.max(1, v)
    }),
    ...
    className: "csheet-field grow"
  }, /*#__PURE__*/React.createElement("label", null, "Class"), /*#__PURE__*/React.createElement(ClassSelect, {
    value: entity.class || '',
    onCommit: v => onField({
      class: v
    })
    ...
    className: "csheet-field grow"
  }, /*#__PURE__*/React.createElement("label", null, "Race / Kin"), /*#__PURE__*/React.createElement(RaceSelect, {
    value: entity.race || '',
    onCommit: v => onField({
      race: v
    })
    ...
    className: "csheet-field grow"
  }, /*#__PURE__*/React.createElement("label", null, "Background"), /*#__PURE__*/React.createElement(LiveInput, {
    ...
    className: "csheet-field grow"
  }, /*#__PURE__*/React.createElement("label", null, "Alignment"), /*#__PURE__*/React.createElement(LiveInput, {
    ...
    className: "csheet-field grow"
  }, /*#__PURE__*/React.createElement("label", null, "Player"), /*#__PURE__*/React.createElement(LiveInput, {
    value: entity.playerName || '',
    onCommit: v => onField({
      playerName: v
    })
    ...
    className: "csheet-field"
  }, /*#__PURE__*/React.createElement("label", null, "XP"), /*#__PURE__*/React.createElement(LiveNumberInput, {
    value: entity.xp || 0,
    onCommit: v => onField({
      xp: Math.max(0, v)
    }),
  ...
  }, "⇧ Portrait"), entity.imageUrl && /*#__PURE__*/React.createElement("button", {
    className: "btn sm ghost",
    type: "button",
    onClick: () => onField({
      imageUrl: ''
    ...
    className: "csheet-combatrow"
  }, [['AC', 'ac', 0, 40, 1], ['Init', 'initBonus', -10, 20, 1], ['Passive', 'passivePerception', 0, 40, 1], ['Prof', 'proficiencyBonus', 0, 20, 1]].map(([lbl, f, mn, mx, st]) => /*#__PURE__*/React.createElement("div", {
    key: f,
    className: "csheet-combatbox"
  }, /*#__PURE__*/React.createElement("span", {
    className: "csheet-combatlbl"
  }, lbl), /*#__PURE__*/React.createElement(LiveNumberInput, {
    className: "mono",
    value: entity[f] ?? 0,
    onCommit: v => onField({
      [f]: v
    ...
    className: "csheet-block"
  }, /*#__PURE__*/React.createElement("label", null, "Movement speeds (ft)"), /*#__PURE__*/React.createElement("div", {
    className: "csheet-combatrow"
  }, /*#__PURE__*/React.createElement("div", {
    className: "csheet-combatbox"
  }, /*#__PURE__*/React.createElement("span", {
    className: "csheet-combatlbl"
  }, "🥾 Walk"), /*#__PURE__*/React.createElement(LiveNumberInput, {
    className: "mono",
    value: walkSpeedOf(entity),
    onCommit: v => onField({
      speed: v,
    ...
    className: "csheet-combatlbl"
  }, "🪽 Fly"), /*#__PURE__*/React.createElement(LiveNumberInput, {
    className: "mono",
    value: entity.speeds?.fly ?? 0,
    onCommit: v => onField({
      speeds: {
    ...
    className: "csheet-combatlbl"
  }, "🌊 Swim"), /*#__PURE__*/React.createElement(LiveNumberInput, {
    className: "mono",
    value: entity.speeds?.swim ?? 0,
    onCommit: v => onField({
      speeds: {
    ...
    className: "csheet-combatlbl"
  }, "🧗 Climb"), /*#__PURE__*/React.createElement(LiveNumberInput, {
    className: "mono",
    value: entity.speeds?.climb ?? 0,
    onCommit: v => onField({
      speeds: {
    ...
    className: "csheet-combatlbl"
  }, "🦗 Jump"), /*#__PURE__*/React.createElement(LiveNumberInput, {
    className: "mono",
    value: entity.speeds?.jump ?? 0,
    onCommit: v => onField({
      speeds: {
    ...
    className: "csheet-block"
  }, /*#__PURE__*/React.createElement("label", null, "Hit Points"), obfuscateHp ? /*#__PURE__*/React.createElement("div", {
    className: "csheet-hp-hidden"
  }, "HP hidden by DM - ", hpLabel(entity.hp.max > 0 ? entity.hp.current / entity.hp.max * 100 : 0).text) : /*#__PURE__*/React.createElement("div", {
    className: "csheet-hp"
  }, /*#__PURE__*/React.createElement("div", {
    className: "csheet-field"
  }, /*#__PURE__*/React.createElement("label", null, "Current"), /*#__PURE__*/React.createElement(LiveNumberInput, {
    value: entity.hp.current,
    onCommit: v => onField({
      hp: {
        current: v,
    ...
    className: "csheet-field"
  }, /*#__PURE__*/React.createElement("label", null, "Max"), /*#__PURE__*/React.createElement(LiveNumberInput, {
    value: entity.hp.max,
    onCommit: v => onField({
      hp: {
    ...
    className: "csheet-field"
  }, /*#__PURE__*/React.createElement("label", null, "Hit Dice"), /*#__PURE__*/React.createElement(LiveInput, {
    value: entity.hitDice || '',
    onCommit: v => onField({
      hitDice: v
    }),
    placeholder: "3d8"
  })), onHpAdjust && /*#__PURE__*/React.createElement("div", {
    className: "csheet-hpquick"
  }, /*#__PURE__*/React.createElement("button", {
    className: "btn danger sm",
    onClick: () => {
      if (hpDelta) onHpAdjust(-Math.abs(hpDelta));
      setHpDelta(0);
    }
  }, "− Dmg"), /*#__PURE__*/React.createElement("input", {
    type: "number",
    value: hpDelta,
    onChange: e => setHpDelta(Math.abs(Number(e.target.value)) || 0)
  }), /*#__PURE__*/React.createElement("button", {
    ...
    className: "csheet-block"
  }, /*#__PURE__*/React.createElement("label", null, "Ability Scores", requestMode ? /*#__PURE__*/React.createElement("span", {
    className: "csheet-hint"
  }, " - modifiers shown large; ± asks the DM") : ''), /*#__PURE__*/React.createElement("div", {
    className: "csheet-abilities"
    ...
    return /*#__PURE__*/React.createElement("div", {
      key: s,
      className: `csheet-ability ${requestMode ? 'req' : ''}`
    }, /*#__PURE__*/React.createElement("span", {
      className: "csheet-ability-k"
    }, s.toUpperCase()), /*#__PURE__*/React.createElement("span", {
      className: "csheet-ability-mod"
    }, m >= 0 ? `+${m}` : m), requestMode ? /*#__PURE__*/React.createElement("div", {
      className: "req-stat"
    }, /*#__PURE__*/React.createElement("span", {
      className: "req-stat-score mono"
    }, score), pend ? /*#__PURE__*/React.createElement("span", {
      className: "req-pending"
    }, "⏳→", pend.payload.to) : /*#__PURE__*/React.createElement("span", {
      className: "req-btns"
    }, /*#__PURE__*/React.createElement("button", {
      ...
      className: "csheet-ability-score mono",
      value: score,
      onCommit: v => onField({
        stats: {
          [s]: v
  ...
  })), requestMode && entity.awaitingHpRoll && /*#__PURE__*/React.createElement("div", {
    className: "levelup-hp"
  }, /*#__PURE__*/React.createElement("label", null, "⬆ Level ", entity.awaitingHpRoll.level, " - roll your new Hit Points"), /*#__PURE__*/React.createElement(LevelUpHpRoller, {
    die: entity.awaitingHpRoll.die,
    conMod: modFor(entity.stats.con ?? 10),
    onResult: roll => onRollLevelHp?.(roll)
  }))), ['PC', 'Familiar'].includes(entity.type) && /*#__PURE__*/React.createElement("div", {
    className: "csheet-block"
  }, /*#__PURE__*/React.createElement("label", null, "Vision ", /*#__PURE__*/React.createElement("span", {
    className: "csheet-hint"
  }, "- feet")), /*#__PURE__*/React.createElement("div", {
    className: "csheet-row"
  }, /*#__PURE__*/React.createElement("div", {
    className: "csheet-field grow"
  }, /*#__PURE__*/React.createElement("label", null, "Darkvision"), /*#__PURE__*/React.createElement(LiveNumberInput, {
    value: entity.darkvision || 0,
    onCommit: v => onField({
      darkvision: v
    }),
    ...
    className: "csheet-field grow"
  }, /*#__PURE__*/React.createElement("label", null, "Light Radius"), /*#__PURE__*/React.createElement(LiveNumberInput, {
    value: entity.lightRadius || 0,
    onCommit: v => onField({
      lightRadius: v
    }),
    ...
    canEdit: canEdit && !!onToggleCondition
  }))), tab === 'combat' && /*#__PURE__*/React.createElement(React.Fragment, null, textBlock('Attacks & Weapons', 'attacks', 'Shortbow +5 to hit, 1d6+3 piercing, range 80/320\nShortsword +5, 1d6+3 slashing…'), textBlock('Features & Traits', 'features', 'Class features, racial traits, feats, resistances…')), tab === 'spells' && isPC && /*#__PURE__*/React.createElement(React.Fragment, null, textBlock('Spellcasting', 'spells', 'Spellcasting ability, save DC, attack bonus, slots…\n\nCantrips: Guidance, Sacred Flame\n1st (3 slots): Cure Wounds, Bless, Healing Word\n2nd (2 slots): Lesser Restoration…', 'free text - track your spells, slots, and notes however you like')), tab === 'gear' && /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
    className: "csheet-block"
  }, /*#__PURE__*/React.createElement("label", null, "Money"), /*#__PURE__*/React.createElement("div", {
    className: "csheet-coins"
  }, [['PP', 'pp'], ['GP', 'gp'], ['EP', 'ep'], ['SP', 'sp'], ['CP', 'cp']].map(([lbl, k]) => /*#__PURE__*/React.createElement("div", {
    key: k,
    className: `csheet-coin coin-${k}`
  }, /*#__PURE__*/React.createElement("span", {
    className: "csheet-coin-lbl"
  }, lbl), /*#__PURE__*/React.createElement(LiveNumberInput, {
    className: "mono",
    value: money[k] || 0,
    onCommit: v => setMoney(k, v),
    min: 0,
    max: 1e9
  }))))), textBlock('Inventory & Equipment', 'inventory', 'Backpack, rations (5), rope (50 ft), torches (3)\nChain shirt, shield, healer\'s kit…'), canEdit && /*#__PURE__*/React.createElement(InventoryItemPicker, {
    onAdd: addInventoryItem
  }), /*#__PURE__*/React.createElement(ArmorManager, {
    entity: entity,
    canEdit: canEdit,
    onField: onField
  }), /*#__PURE__*/React.createElement(WeaponManager, {
    key: entity.id,
    initial: entity.weapons || [],
    canEdit: canEdit,
  ...
  }), textBlock('Proficiencies & Languages', 'proficiencies', 'Armor: light, medium · Weapons: simple, martial\nTools: thieves\' tools · Languages: Common, Sylvan…')), tab === 'story' && /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
    className: "csheet-row"
  }, textBlock('Personality Traits', 'traits', 'I always have a plan for when things go wrong.'), textBlock('Ideals', 'ideals', 'Freedom. Tyrants must not be allowed to oppress…')), /*#__PURE__*/React.createElement("div", {
    className: "csheet-row"
  }, textBlock('Bonds', 'bonds', 'I would die to recover an ancient relic of my faith.'), textBlock('Flaws', 'flaws', 'I can\'t resist a pretty face - or a fat purse.')), textBlock('Backstory', 'backstory', 'Where your character comes from, who they were before the adventure…'), textBlock('Notes / Description', 'playerDescription', 'A short description others see when your token is revealed…'))));
}
function EditMySheetModal({
  ...
  const onHpAdjust = delta => {
    if (entity && delta) playerActionSender({
      type: 'patch_own_entity',
      payload: {
        entityId: entity.id,
        op: 'hp_adjust',
  ...
  const onRequest = req => playerActionSender({
    type: 'submit_request',
    payload: req
  });
  const onRollLevelHp = roll => playerActionSender({
    type: 'roll_levelup_hp',
    payload: {
      roll
    }
  });
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    className: "modal slide-up csheet-modal"
  }, /*#__PURE__*/React.createElement("div", {
    className: "float-panel-header"
  }, /*#__PURE__*/React.createElement("span", {
    style: {
  ...
  }), "◈ ", entity.name, " - Character Sheet"), /*#__PURE__*/React.createElement("button", {
    className: "close-x",
    onClick: onClose
  }, "×")), /*#__PURE__*/React.createElement("div", {
    className: "float-panel-body csheet-scroll"
  ...
  }, "❋"), /*#__PURE__*/React.createElement("span", null, /*#__PURE__*/React.createElement("em", null, "You feel"), " ", /*#__PURE__*/React.createElement("strong", null, sicknessLabel.toLowerCase()), ".")), /*#__PURE__*/React.createElement(CharacterSheet, {
    entity: entity,
    canEdit: true,
    obfuscateHp: obfuscateHp,
    onField: onField,
    onHpAdjust: onHpAdjust,
    onToggleCondition: onToggleCondition,
    initialTab: initialTab,
    requestMode: entity.type === 'PC',
    onRequest: onRequest,
    ownRequests: ownRequests,
    onRollLevelHp: onRollLevelHp
  }), /*#__PURE__*/React.createElement("div", {
    className: "settings-hint",
    style: {
      marginTop: 12
...
function DMSheetModal({
  ...
  if (!entity) return null;
  const onField = patch => dispatch({
    type: 'ENTITY_PATCH',
    id: entity.id,
    patch
  });
  const onHpAdjust = delta => dispatch({
    type: 'ENTITY_HP_ADJUST',
    id: entity.id,
    delta
  });
  const onToggleCondition = c => dispatch({
    type: 'ENTITY_TOGGLE_CONDITION',
    id: entity.id,
    condition: c
  });
...
function loadCreation() {
  try {
    return JSON.parse(localStorage.getItem(CREATION_LS_KEY)) || null;
  } catch {
    return null;
  }
}
function saveCreation(d) {
  try {
    localStorage.setItem(CREATION_LS_KEY, JSON.stringify(d));
  } catch {}
}
function clearCreation() {
  try {
    localStorage.removeItem(CREATION_LS_KEY);
  } catch {}
}
...
function RollSlot({
  ...
  const tmr = useRef([]);
  useEffect(() => () => {
    cyc.current.forEach(clearInterval);
    tmr.current.forEach(clearTimeout);
  }, []);
  const roll = () => {
    ...
    setStage('cycling');
    setHalted([false, false, false, false]);
    setLowIdx(-1);
    setStrike(false);
    setSmash(false);
    for (let i = 0; i < 4; i++) {
      cyc.current[i] = setInterval(() => setShown(s => {
        const n = [...s];
        n[i] = 1 + Math.floor(Math.random() * 6);
        return n;
      }), 70);
    }
    for (let i = 0; i < 4; i++) {
    ...
    tmr.current.push(setTimeout(() => {
      let li = 0;
      for (let i = 1; i < 4; i++) if (raw[i] < raw[li]) li = i;
      setLowIdx(li);
      setStrike(true);
    }, allHalted));
  ...
  return /*#__PURE__*/React.createElement("div", {
    className: `roll-slot ${stage}`
  }, /*#__PURE__*/React.createElement("div", {
    className: "roll-slot-label"
  }, label), stage === 'idle' ? /*#__PURE__*/React.createElement("button", {
    className: "roll-go",
    onClick: roll,
    disabled: locked
  }, "🎲 Roll") : stage === 'done' ? /*#__PURE__*/React.createElement("div", {
    className: "roll-final",
    title: "4d6, drop lowest"
  }, final) : /*#__PURE__*/React.createElement("div", {
    className: "roll-dice"
  }, shown.map((d, i) => /*#__PURE__*/React.createElement("span", {
    key: i,
    className: `roll-die ${halted[i] ? 'set' : 'spin'} ${i === lowIdx ? 'low' : ''} ${i === lowIdx && strike ? 'struck' : ''} ${smash && i !== lowIdx ? 'smash' : ''}`
  }, d, i === lowIdx && strike ? /*#__PURE__*/React.createElement("span", {
...
function HpRoller({
  ...
  const go = () => {
    ...
    rolls.forEach((r, idx) => {
      ...
      tmr.current.push(setTimeout(() => {
        clearInterval(cyc.current);
        if (lvl > 1) cyc.current = setInterval(() => setCur(1 + Math.floor(Math.random() * die)), 80);else setCur(die);
      }, delay));
    ...
    tmr.current.push(setTimeout(() => {
      ...
      setTotal(Math.max(1, t));
      setRunning(false);
      onResult(Math.max(1, t));
    }, delay + 250));
  };
  return /*#__PURE__*/React.createElement("div", {
    className: "hp-roller"
    ...
    className: "hp-final"
  }, "Max HP ", /*#__PURE__*/React.createElement("b", null, total)) : /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("button", {
    className: "btn primary",
    onClick: go,
    disabled: running
  }, running ? 'Rolling…' : '🎲 Generate HP'), running && /*#__PURE__*/React.createElement("div", {
    className: "hp-stage"
  }, cur != null && /*#__PURE__*/React.createElement("div", {
    className: "hp-cur",
    key: cur
    ...
    key: s.lvl,
    className: "hp-step"
  }, "L", s.lvl, ": ", s.isMax ? `max ${s.roll}` : `d${die}→${s.roll}`, conMod >= 0 ? '+' : '', conMod, " = ", s.sub)))));
}
...
const BACKGROUNDS = [{
  name: 'Soldier',
  blurb: 'Drilled in arms, formation, and discipline.'
}, {
  name: 'Outlaw',
  blurb: 'Lived beyond the law - quick, wary, resourceful.'
}, {
  name: 'Wanderer',
  blurb: 'Roamed far and wide, at home in the wilds.'
}, {
  name: 'Artisan',
  blurb: 'A skilled maker of useful and beautiful things.'
}, {
  name: 'Scholar',
  blurb: 'Steeped in old lore, letters, and secrets.'
}, {
  name: 'Noble',
  blurb: 'Born to status, courtesy, and expectation.'
}, {
  name: 'Healer',
  blurb: 'Tends the wounded and mends the sick.'
}, {
  name: 'Entertainer',
  blurb: 'Wins crowds with wit, music, and daring.'
}, {
  name: 'Guardian',
  blurb: 'Swore to shield a place, a person, or a people.'
}, {
  name: 'Forager',
  blurb: 'Knows every root, track, and hiding place.'
}];
function NewCharacterBuilder({
  playerName,
  busy,
  onCancel,
  onCreate,
  ...
  const raceDesc = raceData ? {
    size: raceData.size,
    height: raceData.height,
    traits: raceData.bonus
  } : {
    size: hbSize,
    height: hbHeight,
    traits: hbTraits
  ...
  const bgValid = !!background && (bgMode === '2-1' ? bgPlus2 && bgPlus1 && bgPlus2 !== bgPlus1 : bgTriple.length === 3 && new Set(bgTriple).size === 3);
  const finalStats = {};
  ...
  const finish = () => {
    ...
    onCreate({
      ...
      speed: effSpeed,
      size: raceDesc.size,
      raceHeight: raceDesc.height,
      raceTraits: raceDesc.traits,
      background,
      traits: story.traits,
      ideals: story.ideals,
      bonds: story.bonds,
  ...
  const Stepper = /*#__PURE__*/React.createElement("div", {
    className: "cc-steps"
  }, ['Class', 'Race', 'Stats', 'Background', 'Health', 'Story'].map((s, i) => /*#__PURE__*/React.createElement("div", {
    key: s,
    className: `cc-step ${phase === i + 1 ? 'on' : ''} ${phase > i + 1 ? 'done' : ''}`
  }, i + 1, ". ", s)));
  return /*#__PURE__*/React.createElement("div", {
    className: "builder cc"
  }, /*#__PURE__*/React.createElement("div", {
    className: "cc-head"
  }, /*#__PURE__*/React.createElement("div", {
    className: "onboarding-section-title"
  }, "Create a character"), /*#__PURE__*/React.createElement("button", {
    className: "btn sm ghost",
    onClick: () => {
  ...
  }, "Cancel")), Stepper, phase === 1 && /*#__PURE__*/React.createElement("div", {
    className: "cc-body"
  }, /*#__PURE__*/React.createElement("div", {
    className: "builder-field grow"
    ...
    value: ""
  }, "- Choose a class -"), DND_CLASSES.map(c => /*#__PURE__*/React.createElement("option", {
    key: c,
    value: c
  }, c)))), cls && CLASS_INFO[cls] && /*#__PURE__*/React.createElement("div", {
    className: "cc-info cc-class"
  }, /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("b", null, "Primary"), " ", CLASS_INFO[cls].primary, " · ", /*#__PURE__*/React.createElement("b", null, "Saves"), " ", CLASS_INFO[cls].saves, " · ", /*#__PURE__*/React.createElement("b", null, "Hit die"), " d", CLASS_HIT_DIE[cls]), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("b", null, "Skills"), " ", CLASS_INFO[cls].skills), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("b", null, "Weapons"), " ", CLASS_INFO[cls].weapons, " · ", /*#__PURE__*/React.createElement("b", null, "Armor"), " ", CLASS_INFO[cls].armor), CLASS_INFO[cls].tools !== 'None' && /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("b", null, "Tools"), " ", CLASS_INFO[cls].tools), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("b", null, "Equipment"), " ", CLASS_INFO[cls].equipment)), /*#__PURE__*/React.createElement("div", {
    className: "builder-field grow"
  }, /*#__PURE__*/React.createElement("label", null, "Starting level"), /*#__PURE__*/React.createElement("input", {
    type: "number",
    min: "1",
    max: "20",
    value: level,
    ...
    disabled: !cls,
    onClick: () => setPhase(2)
  }, "Next: Race →"))), phase === 2 && /*#__PURE__*/React.createElement("div", {
    className: "cc-body"
  }, /*#__PURE__*/React.createElement("div", {
    className: "builder-field grow"
  }, /*#__PURE__*/React.createElement("label", null, "Race"), /*#__PURE__*/React.createElement(RaceSelect, {
    value: race,
    onCommit: setRace
  })), raceData && /*#__PURE__*/React.createElement("div", {
    className: "cc-info"
  }, /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("b", null, "Speed"), " ", raceData.speed, " ft · ", /*#__PURE__*/React.createElement("b", null, "Size"), " ", raceData.size), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("b", null, "Suggested height"), " ", raceData.height), /*#__PURE__*/React.createElement("div", {
    className: "cc-info-bonus"
  }, raceData.bonus)), isHomebrewRace && /*#__PURE__*/React.createElement("div", {
    className: "cc-info"
  }, /*#__PURE__*/React.createElement("div", {
    className: "cc-info-bonus"
  }, "Homebrew race - “", race, "”. Define the same elements a standard race carries; all of these travel onto your sheet."), /*#__PURE__*/React.createElement("div", {
    className: "cc-hb-grid"
  }, /*#__PURE__*/React.createElement("div", {
    className: "cc-hb-field"
  }, /*#__PURE__*/React.createElement("label", null, "Speed"), /*#__PURE__*/React.createElement("div", {
    className: "cc-hb-speed"
  }, /*#__PURE__*/React.createElement("input", {
    type: "number",
    min: "0",
    max: "200",
    ...
    className: "cc-hb-field"
  }, /*#__PURE__*/React.createElement("label", null, "Size"), /*#__PURE__*/React.createElement("select", {
    className: "class-select",
    value: hbSize,
    onChange: e => setHbSize(e.target.value)
  }, ['Tiny', 'Small', 'Medium', 'Large', 'Huge'].map(s => /*#__PURE__*/React.createElement("option", {
    key: s,
    value: s
  }, s)))), /*#__PURE__*/React.createElement("div", {
    className: "cc-hb-field grow"
  }, /*#__PURE__*/React.createElement("label", null, "Suggested height"), /*#__PURE__*/React.createElement("input", {
    type: "text",
    maxLength: 40,
    ...
    className: "cc-hb-field"
  }, /*#__PURE__*/React.createElement("label", null, "Racial traits"), /*#__PURE__*/React.createElement("textarea", {
    className: "cc-hb-traits",
    maxLength: 500,
    rows: 2,
    placeholder: "Notable traits and bonuses, e.g. Keen senses, glide on the wind, resist cold.",
    value: hbTraits,
    onChange: e => setHbTraits(e.target.value)
  }))), /*#__PURE__*/React.createElement("div", {
    className: "builder-actions"
  }, /*#__PURE__*/React.createElement("button", {
    className: "btn ghost",
    ...
    disabled: !race,
    onClick: () => setPhase(3)
  }, "Next: Stats →"))), phase === 3 && /*#__PURE__*/React.createElement("div", {
    className: "cc-body"
  }, /*#__PURE__*/React.createElement("div", {
    className: "cc-sub"
  }, "Generate six values. Each is 4d6, dropping the lowest. Values are saved - you can't re-roll by reloading."), /*#__PURE__*/React.createElement("div", {
    className: "cc-roll-grid"
  }, pool.map((v, i) => /*#__PURE__*/React.createElement(RollSlot, {
    key: i,
    label: `Value ${i + 1}`,
    value: v,
    locked: busy,
    onResult: (sum, raw) => onSlotResult(i, sum, raw)
  }))), allRolled && /*#__PURE__*/React.createElement("div", {
    className: "cc-assign"
  }, /*#__PURE__*/React.createElement("div", {
    className: "cc-sub"
  }, "Assign your values to the six abilities:"), ABILITY_KEYS.map(k => {
    ...
    return /*#__PURE__*/React.createElement("div", {
      key: k,
      className: "cc-assign-row"
    }, /*#__PURE__*/React.createElement("span", {
      className: "cc-assign-label"
    }, k.toUpperCase()), /*#__PURE__*/React.createElement("select", {
      value: curIdx != null ? curIdx : '',
      onChange: e => assignValue(k, e.target.value),
      className: "class-select"
    }, /*#__PURE__*/React.createElement("option", {
      value: ""
    }, "-"), pool.map((pv, pi) => used.has(pi) ? null : /*#__PURE__*/React.createElement("option", {
      key: pi,
      value: pi
    }, pv))), /*#__PURE__*/React.createElement("span", {
      className: "cc-assign-mod"
    }, m != null ? `${m >= 0 ? '+' : ''}${m}` : ''));
  })), /*#__PURE__*/React.createElement("div", {
    className: "builder-actions"
  }, /*#__PURE__*/React.createElement("button", {
    className: "btn ghost",
    onClick: () => setPhase(2)
  }, "← Race"), /*#__PURE__*/React.createElement("button", {
    className: "btn primary",
    disabled: !allRolled || assignedCount < 6,
    onClick: () => setPhase(4)
  }, "Next: Background →"))), phase === 4 && /*#__PURE__*/React.createElement("div", {
    className: "cc-body"
  }, /*#__PURE__*/React.createElement("div", {
    className: "cc-sub"
  }, "Choose a background. It grants an ability boost you assign yourself."), /*#__PURE__*/React.createElement("div", {
    className: "cc-bg-grid"
  }, BACKGROUNDS.map(bg => /*#__PURE__*/React.createElement("button", {
    key: bg.name,
    type: "button",
    className: `cc-bg-card ${background === bg.name ? 'on' : ''}`,
    onClick: () => setBackground(bg.name)
  }, /*#__PURE__*/React.createElement("span", {
    className: "cc-bg-name"
  }, bg.name), /*#__PURE__*/React.createElement("span", {
    className: "cc-bg-blurb"
  }, bg.blurb)))), /*#__PURE__*/React.createElement("div", {
    className: "cc-bg-bonus"
  }, /*#__PURE__*/React.createElement("div", {
    className: "cc-sub"
  }, "Ability boost"), /*#__PURE__*/React.createElement("div", {
    className: "cc-bg-mode"
  }, /*#__PURE__*/React.createElement("button", {
    type: "button",
    className: `btn sm ${bgMode === '2-1' ? 'primary' : 'ghost'}`,
    onClick: () => setBgMode('2-1')
  }, "+2 / +1"), /*#__PURE__*/React.createElement("button", {
    type: "button",
    className: `btn sm ${bgMode === '1-1-1' ? 'primary' : 'ghost'}`,
    onClick: () => setBgMode('1-1-1')
  }, "+1 / +1 / +1")), bgMode === '2-1' ? /*#__PURE__*/React.createElement("div", {
    className: "cc-bg-selects"
  }, /*#__PURE__*/React.createElement("label", null, "+2 to", /*#__PURE__*/React.createElement("select", {
    className: "class-select",
    value: bgPlus2,
    onChange: e => setBgPlus2(e.target.value)
  }, /*#__PURE__*/React.createElement("option", {
    value: ""
  }, "-"), ABILITY_KEYS.map(k => /*#__PURE__*/React.createElement("option", {
    key: k,
    value: k,
    disabled: k === bgPlus1
  }, k.toUpperCase())))), /*#__PURE__*/React.createElement("label", null, "+1 to", /*#__PURE__*/React.createElement("select", {
    className: "class-select",
    value: bgPlus1,
    onChange: e => setBgPlus1(e.target.value)
  }, /*#__PURE__*/React.createElement("option", {
    value: ""
  }, "-"), ABILITY_KEYS.map(k => /*#__PURE__*/React.createElement("option", {
    key: k,
    ...
    className: "cc-bg-triple"
  }, [0, 1, 2].map(i => /*#__PURE__*/React.createElement("label", {
    key: i
  }, "+1 to", /*#__PURE__*/React.createElement("select", {
    className: "class-select",
    value: bgTriple[i] || '',
    onChange: e => setBgTriple(t => {
      const n = [...t];
      n[i] = e.target.value;
      return n;
    })
    ...
    key: k,
    value: k,
    disabled: bgTriple.includes(k) && bgTriple[i] !== k
  }, k.toUpperCase()))))))), bgValid && /*#__PURE__*/React.createElement("div", {
    className: "cc-summary"
  }, /*#__PURE__*/React.createElement("div", {
    className: "cc-summary-stats"
  }, ABILITY_KEYS.map(k => /*#__PURE__*/React.createElement("span", {
    key: k
  }, k.toUpperCase(), " ", finalStats[k], " (", abilityMod(finalStats[k]) >= 0 ? '+' : '', abilityMod(finalStats[k]), ")", bgBonuses[k] ? /*#__PURE__*/React.createElement("b", {
    style: {
      color: 'var(--gold)'
    }
  }, " +", bgBonuses[k]) : '')))), /*#__PURE__*/React.createElement("div", {
    className: "builder-actions"
  }, /*#__PURE__*/React.createElement("button", {
    className: "btn ghost",
    onClick: () => setPhase(3)
    ...
    disabled: !bgValid,
    onClick: () => setPhase(5)
  }, "Next: Health →"))), phase === 5 && /*#__PURE__*/React.createElement("div", {
    className: "cc-body"
  }, !classLocked ? /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
    className: "cc-lockin"
  }, /*#__PURE__*/React.createElement("div", {
    className: "cc-lockin-title"
  }, "🔒 Lock in your character"), /*#__PURE__*/React.createElement("div", {
    className: "cc-sub"
  }, "Once you lock in, your ", /*#__PURE__*/React.createElement("b", null, "class"), ", ", /*#__PURE__*/React.createElement("b", null, "race"), ", and ", /*#__PURE__*/React.createElement("b", null, "ability scores"), " are final and cannot be changed. Your hit points are rolled right after.")), /*#__PURE__*/React.createElement("div", {
    className: "cc-summary"
  }, /*#__PURE__*/React.createElement("b", null, name || `${race} ${cls}`), " - ", race, " ", cls, ", level ", level, background ? `, ${background}` : '', /*#__PURE__*/React.createElement("div", {
    className: "cc-summary-stats"
  }, ABILITY_KEYS.map(k => /*#__PURE__*/React.createElement("span", {
    key: k
  }, k.toUpperCase(), " ", finalStats[k], " (", abilityMod(finalStats[k]) >= 0 ? '+' : '', abilityMod(finalStats[k]), ")")))), /*#__PURE__*/React.createElement("div", {
    className: "builder-actions"
  }, /*#__PURE__*/React.createElement("button", {
    className: "btn ghost",
    onClick: () => setPhase(4)
    ...
    className: "btn primary",
    onClick: () => {
      if (confirm(`Lock in ${name || cls}? Your class, race, and stats become final - only HP is left to roll.`)) setClassLocked(true);
    }
  }, "🔒 Lock in & roll HP"))) : /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
    className: "cc-sub"
  }, level === 1 ? `Level 1 ${cls}: maximum hit die (${die}) + your CON modifier (${conMod >= 0 ? '+' : ''}${conMod}).` : `Level ${level} ${cls}: max die at L1, then roll d${die} for each further level - all + CON (${conMod >= 0 ? '+' : ''}${conMod}).`), /*#__PURE__*/React.createElement(HpRoller, {
    die: die,
    conMod: conMod,
    level: level,
    value: hp,
    onResult: t => {
      setHp(t);
      onRoll?.(`HP generated → ${t} max (level ${level} ${cls}, CON ${conMod >= 0 ? '+' : ''}${conMod})`);
    }
  }), /*#__PURE__*/React.createElement("div", {
    className: "cc-summary"
  }, /*#__PURE__*/React.createElement("span", {
    className: "cc-locked"
  }, "🔒 ", name || `${race} ${cls}`, " locked in"), /*#__PURE__*/React.createElement("div", {
    className: "cc-summary-stats"
  }, ABILITY_KEYS.map(k => /*#__PURE__*/React.createElement("span", {
    key: k
  }, k.toUpperCase(), " ", finalStats[k], " (", abilityMod(finalStats[k]) >= 0 ? '+' : '', abilityMod(finalStats[k]), ")")))), /*#__PURE__*/React.createElement("div", {
    ...
    className: "btn primary",
    disabled: hp == null || busy,
    onClick: () => setPhase(6)
  }, "Next: Story →")))), phase === 6 && /*#__PURE__*/React.createElement("div", {
    className: "cc-body"
  }, /*#__PURE__*/React.createElement("div", {
    className: "cc-sub"
  }, "Your hero is ready. Take a moment to sketch their story - all optional, and you can edit it later from your sheet."), /*#__PURE__*/React.createElement("div", {
    className: "cc-story"
  }, /*#__PURE__*/React.createElement("div", {
    className: "builder-field"
  }, /*#__PURE__*/React.createElement("label", null, "Personality traits"), /*#__PURE__*/React.createElement("textarea", {
    rows: 2,
    maxLength: 400,
    placeholder: "How do they carry themselves? Quirks, habits, manner…",
    value: story.traits,
    ...
    className: "builder-field"
  }, /*#__PURE__*/React.createElement("label", null, "Ideals"), /*#__PURE__*/React.createElement("textarea", {
    rows: 2,
    maxLength: 300,
    placeholder: "What do they believe in or strive for?",
    value: story.ideals,
    onChange: e => setStory(s => ({
      ...s,
    ...
    className: "builder-field"
  }, /*#__PURE__*/React.createElement("label", null, "Bonds"), /*#__PURE__*/React.createElement("textarea", {
    rows: 2,
    maxLength: 300,
    placeholder: "Who or what do they hold dear?",
    value: story.bonds,
    onChange: e => setStory(s => ({
      ...s,
    ...
    className: "builder-field"
  }, /*#__PURE__*/React.createElement("label", null, "Flaws"), /*#__PURE__*/React.createElement("textarea", {
    rows: 2,
    maxLength: 300,
    placeholder: "A weakness, fear, or vice…",
    value: story.flaws,
    onChange: e => setStory(s => ({
      ...s,
    ...
    className: "builder-field"
  }, /*#__PURE__*/React.createElement("label", null, "Backstory"), /*#__PURE__*/React.createElement("textarea", {
    rows: 3,
    maxLength: 1000,
    placeholder: "Where do they come from? What set them on this path?",
...
function PlayerOnboardingGate({
  ...
  const myNewCharReq = Object.values(state.pendingRequests || {}).find(r => r.peerId === myPeerId && r.kind === 'new_character');
  ...
  const createCharacter = payload => {
    ...
    clearCreation();
    playerActionSender({
      type: 'create_and_claim_pc',
      payload
    });
  };
  const logRoll = text => playerActionSender({
    type: 'creation_roll',
    payload: {
      text
    }
  ...
  useEffect(() => {
    if (!claiming) return;
    const resend = () => {
      if (claiming.kind === 'pc') playerActionSender({
        type: 'claim_pc',
        payload: {
          entityId: claiming.entityId,
          playerName
        }
      });else if (claiming.kind === 'spectator') playerActionSender({
        type: 'claim_spectator',
        payload: {
          playerName
        }
      });else if (claiming.kind === 'new') playerActionSender({
        type: 'create_and_claim_pc',
        payload: claiming.payload
      });
    };
  ...
  return /*#__PURE__*/React.createElement("div", {
  ...
  }, "Welcome, ", playerName || 'traveler', ". Choose your presence at the table."), granted ? /*#__PURE__*/React.createElement(NewCharacterBuilder, {
    playerName: playerName,
    busy: !!claiming,
    onCancel: () => playerActionSender({
      type: 'submit_request',
      payload: {
        kind: 'new_character',
        data: {
    ...
    className: "glyph"
  }, "⚔"), "No unclaimed characters - request a new one below, or join as a spectator.") : /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("input", {
  ...
  }, "Level ", e.level, " ", e.class, " · ", obfuscateHp ? hpLabel(e.hp.max > 0 ? e.hp.current / e.hp.max * 100 : 0).text : `${e.hp.max} HP`, " · AC ", e.ac)), claiming && claiming.entityId === e.id ? /*#__PURE__*/React.createElement("div", {
    className: "onboarding-mini-spinner"
  }) : /*#__PURE__*/React.createElement("button", {
    className: "btn primary sm"
  ...
  }, grantStatus === 'pending' ? /*#__PURE__*/React.createElement("div", {
    className: "grant-status pending"
  }, "⏳ Waiting for the DM to approve your new character…") : grantStatus === 'rejected' ? /*#__PURE__*/React.createElement("div", {
    className: "grant-status rejected"
  }, "✖ The DM declined your request.", /*#__PURE__*/React.createElement("button", {
    className: "btn sm",
  ...
  }, "You can change this later from the top bar."), claiming && /*#__PURE__*/React.createElement("div", {
    className: "onboarding-claiming"
  }, /*#__PURE__*/React.createElement("div", {
    className: "onboarding-spinner"
  }), /*#__PURE__*/React.createElement("div", {
    className: "onboarding-claiming-text"
  }, claiming.kind === 'spectator' ? 'Joining as spectator…' : claiming.kind === 'new' ? `Creating ${claiming.label}…` : `Stepping in as ${claiming.label}…`), /*#__PURE__*/React.createElement("div", {
    className: "onboarding-claiming-sub"
  }, "Confirming with the DM"))));
}
...
function HazardsPanel({
  ...
  return /*#__PURE__*/React.createElement(FloatPanel, {
    ...
    return /*#__PURE__*/React.createElement("div", {
    ...
    }, "(hidden)"), dmg && /*#__PURE__*/React.createElement("span", {
      style: {
        color: 'var(--danger)',
        marginLeft: 4
      }
    }, "⚔ ", hazardDamageLabel(dmg))), /*#__PURE__*/React.createElement("button", {
      className: `btn sm ghost ${dmg ? 'active' : ''}`,
      title: "Toggle hazardous damage",
      onClick: () => dispatch({
        type: 'HAZARD_UPSERT',
        mapId: currentMapId,
        hazard: {
          ...
          damage: dmg ? null : {
            ...
            sides: 6,
            flat: 0,
            type: h.hazardKind === 'fire' ? 'Fire' : h.hazardKind === 'cold' ? 'Cold' : h.hazardKind === 'acid' ? 'Acid' : 'Bludgeoning',
            onEntry: false,
            perTurn: true
          }
        }
    ...
    }, "✕")), dmg && /*#__PURE__*/React.createElement("div", {
      style: {
        display: 'flex',
        flexWrap: 'wrap',
        alignItems: 'center',
    ...
    }), /*#__PURE__*/React.createElement("span", null, "d"), /*#__PURE__*/React.createElement("select", {
      value: dmg.sides,
      onChange: e => setDmg({
    ...
    }, [4, 6, 8, 10, 12, 20].map(s => /*#__PURE__*/React.createElement("option", {
      key: s,
      value: s
    }, s))), /*#__PURE__*/React.createElement("span", null, "+"), /*#__PURE__*/React.createElement("input", {
      type: "number",
      min: "0",
      max: "999",
      value: dmg.flat,
      ...
      onChange: e => setDmg({
        flat: Math.max(0, Number(e.target.value) || 0)
      })
    }), /*#__PURE__*/React.createElement("select", {
      value: dmg.type,
      onChange: e => setDmg({
      ...
      title: "Damage at the start of each turn spent inside"
    }, /*#__PURE__*/React.createElement("input", {
      type: "checkbox",
      checked: !!dmg.perTurn,
      onChange: e => setDmg({
        perTurn: e.target.checked
      })
    }), " per turn"), /*#__PURE__*/React.createElement("label", {
      style: {
        display: 'flex',
        alignItems: 'center',
        gap: 3
      },
      title: "Also damage the moment a token enters"
    }, /*#__PURE__*/React.createElement("input", {
      type: "checkbox",
      checked: !!dmg.onEntry,
      onChange: e => setDmg({
        onEntry: e.target.checked
      })
    }), " on entry")));
...
function SoundboardPanel({
  ...
  const list = search.trim() ? allList.filter(s => s.name.toLowerCase().includes(search.trim().toLowerCase())) : allList;
  const connectedPlayers = (peerList || []).map(pid => {
    const claim = state.claims?.[pid];
    return {
      pid,
      name: claim?.playerName || pid.slice(-6)
    };
  });
  ...
  return /*#__PURE__*/React.createElement(FloatPanel, {
  ...
  }), isDM && connectedPlayers.length > 0 && /*#__PURE__*/React.createElement("div", {
    style: {
      marginBottom: 10
    }
  }, /*#__PURE__*/React.createElement("label", {
    className: "settings-label"
  }, "Play for"), /*#__PURE__*/React.createElement("div", {
    style: {
      display: 'flex',
      flexWrap: 'wrap',
      gap: 4
    ...
    className: "settings-label-sub"
  }, "- ", allList.length, " sound", allList.length === 1 ? '' : 's')), isDM && allList.length > 0 && /*#__PURE__*/React.createElement("button", {
    className: "btn sm ghost danger",
    onClick: handleDeleteAll,
    title: "Delete all sounds"
  }, "Delete all")), allList.length > 3 && /*#__PURE__*/React.createElement("input", {
    type: "text",
  ...
  }, allList.length === 0 ? 'No sounds yet. Upload audio files to build your soundboard.' : 'No sounds match your search.'), list.map(s => /*#__PURE__*/React.createElement("div", {
    key: s.id,
    style: {
      display: 'flex',
      alignItems: 'center',
    ...
    className: "btn sm",
    onClick: () => onPlay(s.id, targetPeerId),
    title: targetPeerId ? `Play for ${connectedPlayers.find(p => p.pid === targetPeerId)?.name}` : 'Play for the table'
  }, "▶"), /*#__PURE__*/React.createElement("button", {
    className: "btn sm ghost",
    onClick: () => onStop(s.id),
    title: "Stop"
...
function useSoundPlayback(state) {
  ...
  useEffect(() => {
    ...
    function handleEvent(ev) {
      ...
      if (memCached) {
        localCacheRef.current[ev.soundId] = memCached;
        tryPlay(memCached);
        return;
      }
...
function DiceRollOverlay({
  ...
  const dice = useMemo(() => {
    const arr = [];
    (groups || []).forEach((g, gi) => (g.results || []).forEach((r, ri) => arr.push({
      sides: g.die,
      result: r,
      key: `${gi}-${ri}`
    })));
  ...
  useEffect(() => {
    ...
    dice.forEach((d, i) => {
      cyc.current[i] = setInterval(() => {
        setShown(s => {
          const x = s.slice();
          x[i] = 1 + Math.floor(Math.random() * d.sides);
          return x;
        });
      }, 55 + i % 6 * 7);
    });
  ...
  return /*#__PURE__*/React.createElement("div", {
    className: "dice-roll-anim"
  }, /*#__PURE__*/React.createElement("div", {
    className: `dice-roll-dice ${dice.length > 18 ? 'dense' : ''}`
  }, dice.map((d, i) => /*#__PURE__*/React.createElement("span", {
    key: d.key,
    className: `roll-die ${landed[i] ? 'set' : 'spin'}`
  }, shown[i]))), /*#__PURE__*/React.createElement("div", {
    className: `dice-roll-total ${allLanded ? 'show' : ''}`
  }, sum));
}
function DiceTray({
  state,
  ...
  const beginRoll = entry => {
    const groups = entry.groups || (entry.dice ? [{
      die: entry.dice[0]?.die,
      results: entry.dice.map(d => d.result)
    }] : []);
    const flat = groups.reduce((a, g) => a + (g.results?.length || 0), 0);
    if (flat === 0 || flat > 80) {
      onRoll(entry);
      return;
    } // skip anim for empty/huge rolls
    setRolling({
      entry,
      groups,
      seq: ++rollSeqRef.current
    });
  };
  const finishRoll = () => {
    setRolling(r => {
  ...
  return /*#__PURE__*/React.createElement(FloatPanel, {
    ...
    className: "float-panel-body"
  }, rolling && /*#__PURE__*/React.createElement(DiceRollOverlay, {
    key: rolling.seq,
    groups: rolling.groups,
    onDone: finishRoll
  }), /*#__PURE__*/React.createElement("label", {
    className: "settings-label"
...
function ActiveToolBanner({
  ...
  if (measureMode === 'line') {
    glyph = '📏';
    title = 'Measuring - line';
    hint = 'Click-drag on the map to measure distance.';
  } else if (measureMode === 'radius') {
    glyph = '◎';
    title = 'Measuring - radius';
    hint = 'Drag out from a center point.';
  } else if (measureMode === 'tokenToToken') {
    glyph = '⤴';
    title = 'Measuring - token → token';
    hint = 'Click two tokens to measure between them.';
  } else if (placingBlock) {
    glyph = '◼';
    title = 'Placing block - rectangle';
    hint = 'Click-drag on the map to draw a rectangular block.';
  } else if (placingFreeBlock) {
    glyph = '✎';
    title = 'Placing block - freeform';
    hint = 'Drag out a polygon to block line of sight.';
  } else if (placingCircleBlock) {
    glyph = '⬤';
    title = 'Placing block - circle';
    hint = 'Drag from the center to set the radius.';
  } else if (erasingBlock) {
    glyph = '✕';
    title = 'Cut / eraser';
    hint = 'Drag across blocks to subtract them.';
  } else if (placingHazard) {
    glyph = '⚠';
    title = `Placing hazard - ${placingHazard}`;
    hint = 'Drag out a polygon to mark the hazard area.';
  } else if (placingReminder) {
    glyph = '◆';
    title = 'Placing reminder';
    hint = 'Click on the map to drop a private pin.';
  } else if (drawMode === 'erase') {
    glyph = '⌫';
    title = 'Erasing drawings';
    hint = 'Click a drawing to remove it.';
  } else if (drawMode === 'free') {
    glyph = '✒';
    title = 'Drawing - freehand';
    hint = 'Click-drag to draw a freehand stroke.';
  } else if (drawMode === 'line') {
    glyph = '╱';
    title = 'Drawing - line';
    hint = 'Click-drag to draw a straight line.';
  } else if (drawMode === 'circle') {
    glyph = '◯';
    title = 'Drawing - circle';
    hint = 'Drag from the center to draw a circle.';
  } else return null;
  return /*#__PURE__*/React.createElement("div", {
    className: "canvas-overlay top-center"
  }, /*#__PURE__*/React.createElement("div", {
    className: "active-tool-banner"
  }, /*#__PURE__*/React.createElement("span", {
    className: "atb-glyph"
...
function DrawingPanel({
  ...
  }, "◯ Circle"), /*#__PURE__*/React.createElement("button", {
    className: `btn sm danger ${drawMode === 'erase' ? 'active' : ''}`,
  ...
  }, drawMode === 'erase' ? isDM ? 'Click any drawing to erase it.' : 'Click one of your drawings to erase it.' : drawMode ? 'Drag on the map to draw.' : 'Pick a mode, then draw on the map.'), /*#__PURE__*/React.createElement("label", {
    className: "settings-label"
...
function parseWhisper(text, recipients) {
  ...
  const rest = (m[1] || '').trim();
  if (!rest) return {
    error: 'Usage: /whisper [name] your message'
  };
  ...
  if (!best) {
    ...
    return {
      error: `No one named "${firstWord}" to whisper to.`
    };
  }
  const message = rest.slice(best.name.length).trim();
  if (!message) return {
    error: `Add a message after the name to whisper to ${best.display || best.name}.`
  };
  return {
    target: {
      peerId: best.peerId,
      name: best.display || best.name
    },
    message
  };
}
function ChatPanel({
  ...
  const send = () => {
    ...
    if (isDM) {
      ...
      if (w.target) {
        whisperTo = w.target.peerId;
        whisperToName = w.target.name;
        text = w.message;
      }
    }
    if (!text.trim()) return;
    ...
    onSend({
      ...
      whisperTo,
      whisperToName,
      whisperToDm: !isDM && whisperDm
    });
    setInput('');
    setError('');
  };
  const pickSpeaker = name => {
    setSpeakAs(name);
    setShowSpeaker(false);
  };
  const effCollapsed = embedded ? false : collapsed;
  return /*#__PURE__*/React.createElement("div", {
    className: `chat-panel ${effCollapsed ? 'collapsed' : ''} ${embedded ? 'embedded' : ''}`
  }, !embedded && /*#__PURE__*/React.createElement("div", {
    className: "chat-header",
    onClick: () => setCollapsed(c => !c)
  }, /*#__PURE__*/React.createElement("span", {
    className: "chat-title"
  }, "✦ Chat"), collapsed && unread > 0 && /*#__PURE__*/React.createElement("span", {
    className: "chat-unread"
  }, unread), /*#__PURE__*/React.createElement("span", {
    className: "chat-collapse"
  }, collapsed ? '▴' : '▾')), !effCollapsed && /*#__PURE__*/React.createElement("div", {
    className: "chat-body"
  }, /*#__PURE__*/React.createElement("div", {
    className: "chat-list",
    ref: listRef
  ...
  }, "No messages yet. Say hello!") : messages.map(m => /*#__PURE__*/React.createElement("div", {
    key: m.id,
    className: `chat-msg ${m.senderId === myPeerId ? 'own' : ''} ${m.whisperTo ? 'whisper' : ''}`
  }, /*#__PURE__*/React.createElement("span", {
    className: "chat-sender"
  }, m.whisperTo && /*#__PURE__*/React.createElement("span", {
    className: "chat-lock"
  }, "🔒"), m.senderName, m.whisperTo && /*#__PURE__*/React.createElement("span", {
    className: "chat-whisper-to"
  }, " → ", m.whisperToName), /*#__PURE__*/React.createElement("span", {
    className: "chat-colon"
  }, ":")), ' ', /*#__PURE__*/React.createElement("span", {
    className: "chat-text"
  }, m.text)))), error && /*#__PURE__*/React.createElement("div", {
    className: "chat-error"
  }, error), /*#__PURE__*/React.createElement("div", {
    className: "chat-input-row"
  }, isDM && showSpeaker && /*#__PURE__*/React.createElement("div", {
    className: "chat-speaker-pop"
  }, /*#__PURE__*/React.createElement("div", {
    className: "chat-speaker-head"
  }, "Speak as…"), /*#__PURE__*/React.createElement("button", {
    className: `chat-speaker-opt ${!speakAs ? 'active' : ''}`,
    onClick: () => pickSpeaker('')
  }, "♛ Dungeon Master"), tokensOnMap.length > 0 && /*#__PURE__*/React.createElement("div", {
    className: "chat-speaker-label"
  }, "Tokens on this map"), /*#__PURE__*/React.createElement("div", {
    className: "chat-speaker-tokens"
  }, tokensOnMap.map(n => /*#__PURE__*/React.createElement("button", {
    key: n,
    className: `chat-speaker-opt ${speakAs === n ? 'active' : ''}`,
    onClick: () => pickSpeaker(n)
  }, n))), /*#__PURE__*/React.createElement("div", {
    className: "chat-speaker-label"
  }, "Custom name"), /*#__PURE__*/React.createElement("div", {
    className: "chat-speaker-custom"
  }, /*#__PURE__*/React.createElement("input", {
    value: customName,
    onChange: e => setCustomName(e.target.value),
    placeholder: "The wind, A whisper…",
    onKeyDown: e => {
      if (e.key === 'Enter' && customName.trim()) pickSpeaker(customName.trim());
    }
  }), /*#__PURE__*/React.createElement("button", {
    disabled: !customName.trim(),
    onClick: () => customName.trim() && pickSpeaker(customName.trim())
  }, "Use"))), isDM ? /*#__PURE__*/React.createElement("button", {
    className: "chat-as",
    title: "Click to change who you speak as",
    onClick: () => setShowSpeaker(s => !s)
  }, /*#__PURE__*/React.createElement("span", {
    className: "chat-as-name"
  }, myName), /*#__PURE__*/React.createElement("span", {
    className: "chat-as-caret"
  }, "▾")) : /*#__PURE__*/React.createElement("button", {
    className: `chat-whisper-toggle ${whisperDm ? 'on' : ''}`,
    title: whisperDm ? 'Whispering to the DM (click for public)' : 'Whisper privately to the DM',
    onClick: () => setWhisperDm(v => !v)
  }, whisperDm ? '🔒 DM' : '🔓 All'), /*#__PURE__*/React.createElement("input", {
    className: "chat-input",
    value: input,
    maxLength: TUNING.chatMaxChars,
...
function requestSummary(state, r) {
  if (r.kind === 'new_character') return {
    icon: '✨',
    title: 'New character',
    detail: 'wants to create a new PC'
  };
  if (r.kind === 'join_request') return {
    icon: '🚪',
    title: 'Player wants to join',
    detail: 'is asking to enter your session'
  };
  if (r.kind === 'place_token') {
    ...
    return {
      ...
      detail: `wants to place ${nm} on the map`
    };
  }
  if (r.kind === 'level_change') {
    ...
    return {
      icon: '⬆',
      title: 'Level change',
      detail: `${ent?.name || 'PC'}: level ${r.payload?.from} → ${r.payload?.to}`
    };
  }
  if (r.kind === 'stat_change') {
    ...
    return {
      icon: '✦',
      title: 'Stat change',
      detail: `${ent?.name || 'PC'}: ${String(r.payload?.stat || '').toUpperCase()} ${r.payload?.from} → ${r.payload?.to}`
    };
  }
  return {
    icon: '?',
...
function ActionRequestCard({
  ...
  const comps = Array.isArray(p.components) && p.components.length ? p.components : [{
    dice: p.dice || [],
    diceSum: p.diceSum || 0,
  ...
  const compLabel = c => {
    ...
    for (const d of c.dice || []) {
      (groups[d.sides] = groups[d.sides] || []).push(d.result);
    }
    const keys = Object.keys(groups);
    return /*#__PURE__*/React.createElement("span", {
      className: "ar-comp"
    }, keys.length === 0 && (c.flat || 0) > 0 && /*#__PURE__*/React.createElement("span", {
      className: "ar-comp-dice"
    }, "manual ", c.flat), keys.map(s => /*#__PURE__*/React.createElement("span", {
      key: s,
      className: "ar-comp-dice"
    }, groups[s].length, "d", s, ": ", /*#__PURE__*/React.createElement("b", null, groups[s].join(', ')))), (c.flat || 0) > 0 && keys.length > 0 && /*#__PURE__*/React.createElement("span", {
      className: "ar-comp-dice"
    }, "+", c.flat), (c.modifier || 0) !== 0 && /*#__PURE__*/React.createElement("span", {
      className: "ar-comp-mod"
    }, c.modifier > 0 ? `+${c.modifier}` : c.modifier), isDmg && c.type && /*#__PURE__*/React.createElement("span", {
      className: "ar-dtype"
    }, c.type));
  };
  return /*#__PURE__*/React.createElement("div", {
    className: "modal-overlay ar-overlay"
  }, /*#__PURE__*/React.createElement("div", {
    className: "modal ar-modal"
  }, /*#__PURE__*/React.createElement("div", {
    className: "ar-head"
  }, /*#__PURE__*/React.createElement("span", {
    className: "ar-head-title"
  }, isHeal ? '✚ Heal Request' : isCond ? '✦ Condition Request' : '⚔ Attack Request'), /*#__PURE__*/React.createElement("span", {
    className: "ar-head-who"
  }, "from ", r.playerName, p.weaponName ? ` · ${p.weaponName}${p.attackName ? ' - ' + p.attackName : ''}` : '')), /*#__PURE__*/React.createElement("div", {
    className: "ar-tokens"
  }, /*#__PURE__*/React.createElement(Avatar, {
    e: src
  }), /*#__PURE__*/React.createElement("div", {
    className: `ar-arrow ${isHeal ? 'heal' : ''}`
  }, isHeal ? '✚' : '➜'), /*#__PURE__*/React.createElement(Avatar, {
    e: tgt
  })), /*#__PURE__*/React.createElement("div", {
    className: "ar-body"
  }, isCond ? /*#__PURE__*/React.createElement("div", {
    className: "ar-cond"
  }, /*#__PURE__*/React.createElement("span", {
    className: "ar-cond-chip",
    style: {
      background: CONDITION_COLORS[p.condition] || '#9b6ac4'
    }
  }, p.condition), /*#__PURE__*/React.createElement("span", {
    className: "ar-cond-text"
  }, "to be applied to ", tgt?.name || 'the target')) : /*#__PURE__*/React.createElement(React.Fragment, null, toHitTotal != null && /*#__PURE__*/React.createElement("div", {
    className: "ar-tohit"
  }, "To hit: ", /*#__PURE__*/React.createElement("strong", null, toHitTotal), " ", /*#__PURE__*/React.createElement("span", {
    className: "ar-tohit-sub"
  }, "(d20 ", p.toHitRoll, " ", (p.toHit || 0) >= 0 ? '+' : '', p.toHit || 0, ")"), tgt?.ac != null && /*#__PURE__*/React.createElement("span", {
    className: `ar-vs-ac ${toHitTotal >= tgt.ac ? 'hit' : 'miss'}`
  }, "vs AC ", tgt.ac, " · ", toHitTotal >= tgt.ac ? 'HIT' : 'MISS')), /*#__PURE__*/React.createElement("div", {
    className: "ar-comps"
  }, comps.map((c, i) => /*#__PURE__*/React.createElement("div", {
    key: i,
    className: "ar-comp-row"
  }, compLabel(c)))), /*#__PURE__*/React.createElement("div", {
    className: "ar-total"
  }, "= ", /*#__PURE__*/React.createElement("strong", null, full), isHeal && /*#__PURE__*/React.createElement("span", {
    className: "ar-dtype heal"
  }, "healing")))), /*#__PURE__*/React.createElement("div", {
    className: "ar-actions"
  }, isDmg ? /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("button", {
    className: "btn primary",
    onClick: () => onApply(r, 'full')
  }, "Apply ", full), /*#__PURE__*/React.createElement("button", {
    className: "btn",
    onClick: () => onApply(r, 'weak'),
    title: "Vulnerable: dice damage doubled (modifiers unchanged)"
  }, "Weak → ", weak), /*#__PURE__*/React.createElement("button", {
    className: "btn",
    onClick: () => onApply(r, 'resist'),
    title: "Resistant: half total, rounded up"
  }, "Resist → ", resist), /*#__PURE__*/React.createElement("button", {
    className: "btn",
    onClick: () => onApply(r, 'immune'),
    title: "Immune: no damage"
  }, "Immune 0")) : isHeal ? /*#__PURE__*/React.createElement("button", {
    className: "btn primary",
    onClick: () => onApply(r, 'full')
  }, "Apply heal ", full) : /*#__PURE__*/React.createElement("button", {
    className: "btn primary",
    onClick: () => onApply(r, 'full')
  }, "Apply ", p.condition), /*#__PURE__*/React.createElement("button", {
    className: "btn danger",
    onClick: () => onReject(r)
  }, "Reject")), queued > 0 && /*#__PURE__*/React.createElement("div", {
    className: "ar-queued"
...
function AttackCinematic({
  ...
  const tmr = useRef([]);
  const cyc = useRef(null);
  const dcyc = useRef(null);
  const done = useRef(false);
  useEffect(() => () => {
    tmr.current.forEach(clearTimeout);
    clearInterval(cyc.current);
    clearInterval(dcyc.current);
  }, []);
  useEffect(() => {
    ...
    const startDamage = base => {
      T(() => {
        setPhase('damage');
        dcyc.current = setInterval(() => setTick(t => t + 1), 80);
      }, base);
      T(() => {
        clearInterval(dcyc.current);
        setDiceShown(true);
    ...
    if (hasToHit) {
      T(() => {
        setPhase('tohit');
        cyc.current = setInterval(() => {
          setFaceA(1 + Math.floor(Math.random() * 20));
          setFaceB(1 + Math.floor(Math.random() * 20));
        }, 70);
      }, 600);
      T(() => {
        clearInterval(cyc.current);
  ...
  const setType = (t, v) => setSettings(s => ({
    ...s,
    [t]: v
  }));
  ...
  const Die = ({
    ...
    className: `acin-d20 ${phase === 'tohit' ? 'spin' : 'set'} ${phase !== 'tohit' && used ? hit ? 'hit' : 'miss' : ''} ${phase !== 'tohit' && dim ? 'dim' : ''}`
  }, phase === 'tohit' ? face : val);
  return /*#__PURE__*/React.createElement("div", {
    className: "acin-overlay"
  }, /*#__PURE__*/React.createElement("div", {
    className: `acin-card ${isManeuver ? 'is-maneuver' : ''} ${pi >= 2 && hasToHit ? hit ? 'is-hit' : 'is-miss' : ''}`
  }, /*#__PURE__*/React.createElement("div", {
    className: "acin-head"
  }, /*#__PURE__*/React.createElement("div", {
    className: `acin-side ${isContest ? atk.saveResult ? 'acin-clash-l' : 'acin-clash-loop-l' : ''}`
  }, /*#__PURE__*/React.createElement(Avatar, {
    name: atk.attackerName,
    color: atk.attackerColor,
    img: atk.attackerImg
  }), /*#__PURE__*/React.createElement("div", {
    className: "acin-side-name"
  }, atk.attackerName)), /*#__PURE__*/React.createElement("div", {
    className: "acin-mid"
  }, /*#__PURE__*/React.createElement("div", {
    className: `acin-swords ${isManeuver ? 'acin-man-static' : ''}`
  }, isContest ? '💥' : isManeuver ? '✦' : '⚔'), isManeuver && /*#__PURE__*/React.createElement("div", {
    className: "acin-man-tag"
  }, isContest ? 'CONTEST' : 'EFFECT'), (atk.weaponName || atk.attackName) && /*#__PURE__*/React.createElement("div", {
    className: `acin-weapon ${isManeuver ? 'acin-man-name' : ''}`
  }, atk.weaponName, atk.attackName ? ` · ${atk.attackName}` : '')), /*#__PURE__*/React.createElement("div", {
    className: `acin-side ${isContest ? atk.saveResult ? 'acin-clash-r' : 'acin-clash-loop-r' : ''}`
  }, /*#__PURE__*/React.createElement(Avatar, {
    name: atk.targetName,
    color: atk.targetColor,
    img: atk.targetImg
  }), /*#__PURE__*/React.createElement("div", {
    className: "acin-side-name"
  }, atk.targetName, atk.targetAc != null && /*#__PURE__*/React.createElement("span", {
    className: "acin-ac"
  }, "AC ", atk.targetAc)))), hasToHit && pi >= 1 && /*#__PURE__*/React.createElement("div", {
    className: "acin-tohit"
  }, advMode !== 'normal' ? /*#__PURE__*/React.createElement("div", {
    className: "acin-d20-pair"
  }, /*#__PURE__*/React.createElement(Die, {
    val: d20a,
    face: faceA,
    used: phase !== 'tohit' && d20a === effRoll,
    dim: phase !== 'tohit' && d20a !== effRoll
  }), /*#__PURE__*/React.createElement(Die, {
    val: d20b,
    face: faceB,
    used: phase !== 'tohit' && d20b === effRoll && d20a !== effRoll,
    dim: phase !== 'tohit' && !(d20b === effRoll && d20a !== effRoll)
  })) : /*#__PURE__*/React.createElement(Die, {
    val: d20a,
    face: faceA,
    used: pi >= 2,
    dim: false
  }), advMode !== 'normal' && pi >= 2 && /*#__PURE__*/React.createElement("div", {
    className: `acin-adv-badge ${advMode}`
  }, advMode === 'advantage' ? '▲ ADVANTAGE - highest kept' : '▼ DISADVANTAGE - lowest kept'), pi >= 2 && /*#__PURE__*/React.createElement("div", {
    className: "acin-tohit-calc"
  }, /*#__PURE__*/React.createElement("span", {
    className: "acin-calc"
  }, effRoll, /*#__PURE__*/React.createElement("i", null, (atk.toHit || 0) >= 0 ? ' + ' : ' - ', Math.abs(atk.toHit || 0)), " = ", /*#__PURE__*/React.createElement("b", {
    className: "acin-glow-num"
  }, toHitTotal)), atk.targetAc != null && /*#__PURE__*/React.createElement("span", {
    className: "acin-tohit-vs"
  }, "vs AC ", atk.targetAc))), isDM && hasToHit && pi >= 2 && /*#__PURE__*/React.createElement("div", {
    className: "acin-adv-toggle"
  }, [['normal', 'Normal'], ['advantage', 'Advantage'], ['disadvantage', 'Disadvantage']].map(([v, l]) => /*#__PURE__*/React.createElement("button", {
    key: v,
    className: `acin-adv-opt ${v} ${advMode === v ? 'on' : ''}`,
    onClick: () => onSetAdv?.(v)
  }, l))), hasToHit && pi >= 2 && (atk.targetAc != null ? /*#__PURE__*/React.createElement("div", {
    className: `acin-verdict ${hit ? 'hit' : 'miss'}`
  }, hit ? '✦ HIT ✦' : '✕ MISS') : /*#__PURE__*/React.createElement("div", {
    className: "acin-verdict noac"
  }, "⚑ Rolled ", toHitTotal, " · set the target's AC or the DM rules the hit")), hit && pi >= 3 && comps.length > 0 && /*#__PURE__*/React.createElement("div", {
    className: "acin-damage"
  }, comps.map((c, i) => /*#__PURE__*/React.createElement("div", {
    key: i,
    className: "acin-comp"
  }, /*#__PURE__*/React.createElement("span", {
    className: "acin-comp-dice"
  }, (c.dice || []).map((d, j) => /*#__PURE__*/React.createElement("span", {
    key: j,
    className: `acin-die ${diceShown ? 'set' : 'roll'}`
  }, diceShown ? d.result : 1 + (tick + j) % (d.sides || 6)))), (c.modifier || 0) !== 0 && /*#__PURE__*/React.createElement("span", {
    className: "acin-comp-mod"
  }, c.modifier > 0 ? `+${c.modifier}` : c.modifier), c.type && /*#__PURE__*/React.createElement("span", {
    className: "acin-comp-type"
  }, c.type))), diceShown && /*#__PURE__*/React.createElement("div", {
    className: "acin-dmg-total"
  }, /*#__PURE__*/React.createElement("span", {
    className: "acin-dmg-total-eq"
  }, "total"), /*#__PURE__*/React.createElement("b", {
    key: total,
    className: "acin-glow-num big"
  }, total))), hit && pi >= 3 && eff && eff.condition && /*#__PURE__*/React.createElement("div", {
    className: "acin-effect"
  }, eff.save ? /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
    className: "acin-effect-title"
  }, "⚠ ", eff.condition, " ", /*#__PURE__*/React.createElement("span", {
    className: "acin-effect-sub"
  }, eff.contest ? `${eff.save.ability} contest` : `${eff.save.ability} save DC ${eff.save.dc}`)), eff.contest && /*#__PURE__*/React.createElement("div", {
    className: "acin-contest-atk"
  }, atk.attackerName, "'s ", eff.contest.ability, ": ", eff.contest.atkD20, eff.contest.atkMod >= 0 ? ' + ' : ' - ', Math.abs(eff.contest.atkMod), " = ", /*#__PURE__*/React.createElement("b", {
    className: "acin-glow-num"
  }, eff.contest.atkTotal), " · target must meet or beat it"), !sr && isDM && !eff.contest && onSetDc && /*#__PURE__*/React.createElement("div", {
    className: "acin-dc-edit"
  }, /*#__PURE__*/React.createElement("span", null, "DC"), /*#__PURE__*/React.createElement("button", {
    className: "acin-dc-btn",
    onClick: () => onSetDc(Math.max(1, eff.save.dc - 1))
  }, "−"), /*#__PURE__*/React.createElement("b", null, eff.save.dc), /*#__PURE__*/React.createElement("button", {
    className: "acin-dc-btn",
    onClick: () => onSetDc(Math.min(40, eff.save.dc + 1))
  }, "+")), !sr ? canRoll ? /*#__PURE__*/React.createElement("div", {
    className: "acin-save-roll"
  }, /*#__PURE__*/React.createElement("div", {
    className: "acin-save-roll-who"
  }, isDM ? 'DM rolls for' : 'Roll for', " ", atk.targetName), /*#__PURE__*/React.createElement("div", {
    className: "acin-save-adv"
  }, [['normal', 'Normal'], ['advantage', 'Adv'], ['disadvantage', 'Disadv']].map(([v, l]) => /*#__PURE__*/React.createElement("button", {
    key: v,
    className: `acin-adv-opt ${v} ${saveAdv === v ? 'on' : ''}`,
    onClick: () => setSaveAdv(v)
  }, l))), physicalDice ? saveEntry ? /*#__PURE__*/React.createElement("div", {
    className: "acin-save-entry"
  }, /*#__PURE__*/React.createElement("span", {
    className: "acin-save-entry-lbl"
  }, "d20", saveAdv !== 'normal' ? ' ×2' : ''), /*#__PURE__*/React.createElement("input", {
    className: "acin-die-input",
    type: "number",
    min: "1",
    max: "20",
    autoFocus: true,
    value: saveEntry.a,
    onChange: e => setSaveEntry(s => ({
      ...s,
      a: e.target.value
    }))
  }), saveAdv !== 'normal' && /*#__PURE__*/React.createElement("input", {
    className: "acin-die-input",
    type: "number",
    min: "1",
    max: "20",
    ...
    className: "btn sm primary",
    onClick: () => setSaveEntry({
      a: '',
      b: ''
    })
  }, "✎ Enter ", eff.save.ability, " ", eff.contest ? 'check' : 'save') : /*#__PURE__*/React.createElement("button", {
    className: "btn sm primary",
    onClick: rollSave
  }, "🎲 Roll ", eff.save.ability, " ", eff.contest ? 'check' : 'save')) : /*#__PURE__*/React.createElement("div", {
    className: "acin-await"
  }, "Awaiting ", atk.targetName, "'s ", eff.contest ? 'contest' : 'save', "…") : /*#__PURE__*/React.createElement("div", {
    className: `acin-save-result ${sr.success ? 'pass' : 'fail'}`
  }, sr.ability, " ", eff.contest ? 'check' : 'save', ": ", sr.roll, sr.mod >= 0 ? ' + ' : ' - ', Math.abs(sr.mod), " = ", /*#__PURE__*/React.createElement("b", {
    key: sr.total,
    className: "acin-glow-num"
  }, sr.total), " ", eff.contest ? `vs ${eff.save.dc}` : `vs DC ${eff.save.dc}`, /*#__PURE__*/React.createElement("span", {
    className: "acin-save-verdict"
  }, sr.success ? eff.contest ? '✦ Wins!' : '✦ Resisted' : `✕ ${eff.condition}!`))) : /*#__PURE__*/React.createElement("div", {
    className: "acin-effect-title"
  }, "⚠ Inflicts ", /*#__PURE__*/React.createElement("b", null, eff.condition), " on hit")), phase === 'await' && hit && (isDM ? /*#__PURE__*/React.createElement("div", {
    className: "acin-resolve"
  }, !isManeuver && /*#__PURE__*/React.createElement("div", {
    className: "acin-resolve-hint"
  }, "Set weakness / resistance / immunity per damage type:"), types.map(t => /*#__PURE__*/React.createElement("div", {
    key: t,
    className: "acin-res-row"
  }, /*#__PURE__*/React.createElement("span", {
    className: "acin-res-type"
  }, t), /*#__PURE__*/React.createElement("div", {
    className: "acin-res-opts"
  }, [['normal', 'Normal'], ['weak', 'Weak'], ['resist', 'Resist'], ['immune', 'Immune']].map(([v, l]) => /*#__PURE__*/React.createElement("button", {
    key: v,
    className: `acin-res-opt ${v} ${(settings[t] || 'normal') === v ? 'on' : ''}`,
    onClick: () => setType(t, v)
  }, l))))), /*#__PURE__*/React.createElement("div", {
    className: "acin-resolve-actions"
  }, /*#__PURE__*/React.createElement("button", {
    className: "btn primary",
    onClick: () => {
      if (!done.current) {
        done.current = true;
        onApply?.(total, settings, applyEffect);
      }
    }
  }, isManeuver ? applyEffect ? `Apply ${eff.condition}` : 'Resolve' : `Apply ${total}${applyEffect ? ` + ${eff.condition}` : ''}`), /*#__PURE__*/React.createElement("button", {
    className: "btn danger",
    onClick: () => {
      if (!done.current) {
        done.current = true;
    ...
    className: "acin-await"
  }, "Awaiting the DM's ruling…")), hasToHit && !hit && pi >= 2 && isDM && /*#__PURE__*/React.createElement("div", {
    className: "acin-resolve-actions"
  }, /*#__PURE__*/React.createElement("button", {
    className: "btn",
    onClick: () => {
...
function AttackCinematicLayer({
  ...
  useEffect(() => {
    if (!isDM || atk) return;
    const next = Object.values(state.pendingRequests || {}).filter(r => r.status === 'pending' && r.kind === 'apply_damage').sort((a, b) => a.ts - b.ts)[0];
    if (!next) return;
    ...
    dispatch({
      ...
      attack: {
        ...
        targetImg: tgt.imageUrl || null,
        targetAc: ac,
        weaponName: p.weaponName || '',
        attackName: p.attackName || '',
        toHit: p.toHit ?? null,
        toHitRoll: p.toHitRoll ?? null,
        hit,
        d20a: p.d20a,
        d20b: p.d20b,
        advMode: p.advMode,
        effect: p.effect,
        targetStats: tgt.stats || {},
  ...
  useEffect(() => {
    ...
    if (backing && backing.status !== 'pending') dispatch({
      type: 'ATTACK_CLEAR',
      id: atk.id
    });
  }, [isDM, atk, state.pendingRequests, dispatch]);
  if (!atk) return null;
  const publicLog = text => dispatch({
    type: 'CHAT_ADD',
    message: {
      id: uid('msg_'),
      ts: Date.now(),
  ...
  const resolveReq = status => {
    if (state.pendingRequests?.[atk.id]) dispatch({
      type: 'REQUEST_RESOLVE',
      id: atk.id,
      status
    });
  ...
  const onSaveResult = result => dispatch({
    type: 'ATTACK_UPDATE',
    id: atk.id,
    patch: {
      saveResult: result
  ...
  const playerSaveRoll = result => playerActionSender?.({
    type: 'attack_save_roll',
    attackId: atk.id,
    saveResult: result
  });
  const onApply = (total, settings, applyEffect) => {
    ...
    if (applyEffect && atk.effect?.condition && tgt) {
      if (!(tgt.conditions || []).includes(atk.effect.condition)) dispatch({
        type: 'ENTITY_TOGGLE_CONDITION',
        id: tgt.id,
        condition: atk.effect.condition
      });
  ...
  return ReactDOM.createPortal(/*#__PURE__*/React.createElement(AttackCinematic, {
    key: atk.id,
    atk: atk,
    isDM: isDM,
    physicalDice: physicalDice,
    canRollSave: canRollSave,
    onApply: isDM ? onApply : null,
    onDismiss: isDM ? onDismiss : null,
    onSetAdv: isDM ? onSetAdv : null,
    onSetDc: isDM ? onSetDc : null,
    onSaveResult: isDM ? onSaveResult : ownsTarget ? playerSaveRoll : null
  }), document.body);
}
function DMRequestsOverlay({
  state,
  dispatch,
  toast,
  onApproveJoin,
  ...
  const pending = Object.values(state.pendingRequests || {}).filter(r => r.status === 'pending').sort((a, b) => a.ts - b.ts);
  ...
  useEffect(() => {
    ...
    for (const r of pending) {
      if (now - r.ts >= REQUEST_TTL_MS) dispatch({
        type: 'REQUEST_RESOLVE',
        id: r.id,
        status: 'rejected'
      });
  ...
  const resolve = (r, status) => {
    ...
    if (r.kind === 'join_request') {
      ...
      if (status === 'accepted') onApproveJoin?.(r.peerId);else onRejectJoin?.(r.peerId);
      return;
    }
    if (status === 'accepted') {
      const ent = state.entities[r.payload?.entityId];
  ...
  const applyAction = (r, variant) => {
    ...
    if (variant === 'immune') {
      ...
      value = Math.max(0, diceDmg * 2 + mod);
      vlabel = ' (vulnerable)';
    } else if (variant === 'resist') {
      value = Math.max(0, Math.ceil((diceDmg + mod) / 2));
      vlabel = ' (resisted)';
  ...
  return ReactDOM.createPortal(/*#__PURE__*/React.createElement(React.Fragment, null, centeredReqs.length > 0 && /*#__PURE__*/React.createElement("div", {
    className: "req-center-overlay"
  }, /*#__PURE__*/React.createElement("div", {
    className: "req-center-card"
  }, /*#__PURE__*/React.createElement("div", {
    className: "req-center-title"
  }, "⚑ Player request", centeredReqs.length > 1 ? `s (${centeredReqs.length})` : ''), centeredReqs.map(r => {
    const s = requestSummary(state, r);
    const left = Math.max(0, Math.ceil((REQUEST_TTL_MS - (Date.now() - r.ts)) / 1000));
    return /*#__PURE__*/React.createElement("div", {
      key: r.id,
      className: "req-center-row"
    }, /*#__PURE__*/React.createElement("div", {
      className: "req-center-icon"
    }, s.icon), /*#__PURE__*/React.createElement("div", {
      className: "req-center-body"
    }, /*#__PURE__*/React.createElement("div", {
      className: "req-center-head"
    }, s.title), /*#__PURE__*/React.createElement("div", {
      className: "req-center-who"
    }, r.playerName), /*#__PURE__*/React.createElement("div", {
      className: "req-center-detail"
    }, s.detail), /*#__PURE__*/React.createElement("div", {
      className: "req-card-timer"
    }, "Auto-declines in ", left, "s")), /*#__PURE__*/React.createElement("div", {
      className: "req-center-actions"
    }, /*#__PURE__*/React.createElement("button", {
      className: "btn sm primary",
      onClick: () => resolve(r, 'accepted')
    }, "Accept"), /*#__PURE__*/React.createElement("button", {
      className: "btn sm danger",
      onClick: () => resolve(r, 'rejected')
    }, "Refuse")));
  }))), cornerReqs.length > 0 && /*#__PURE__*/React.createElement("div", {
    ...
    return /*#__PURE__*/React.createElement("div", {
      ...
      className: "req-card-head"
    }, /*#__PURE__*/React.createElement("span", {
      className: "req-icon"
    }, s.icon), " ", s.title), /*#__PURE__*/React.createElement("div", {
      className: "req-card-who"
    }, r.playerName), /*#__PURE__*/React.createElement("div", {
      className: "req-card-detail"
    }, s.detail), /*#__PURE__*/React.createElement("div", {
      className: "req-card-timer"
    }, "Auto-declines in ", left, "s"), /*#__PURE__*/React.createElement("div", {
      className: "req-card-actions"
    }, /*#__PURE__*/React.createElement("button", {
      className: "btn sm danger",
      onClick: () => resolve(r, 'rejected')
  ...
  })), activeAction && /*#__PURE__*/React.createElement(ActionRequestCard, {
    r: activeAction,
    state: state,
    onApply: applyAction,
    onReject: rejectAction,
    queued: popupReqs.length - 1
  })), document.body);
}
...
function DMClaimsPanel({
  ...
  return /*#__PURE__*/React.createElement(FloatPanel, {
    ...
    className: "claim-theme-all"
  }, /*#__PURE__*/React.createElement("span", {
    className: "claim-theme-label"
  }, "🎨 Set theme for everyone"), /*#__PURE__*/React.createElement("select", {
    className: "claim-theme-select",
    value: "__pick",
    onChange: e => {
      const v = e.target.value;
      if (v !== '__pick') pushTheme(allPeerIds, v === '__release' ? '' : v, 'all players');
    }
  }, /*#__PURE__*/React.createElement("option", {
    value: "__pick"
  }, "Choose…"), THEMES.map(t => /*#__PURE__*/React.createElement("option", {
    key: t.id,
    value: t.id
  }, t.label)), /*#__PURE__*/React.createElement("option", {
    value: "__release"
  }, "↺ Release (player's choice)"))), peers.map(([peerId, claim]) => {
    ...
    return /*#__PURE__*/React.createElement("div", {
    ...
    }, "id: ", peerId.slice(0, 12), "…"), /*#__PURE__*/React.createElement("div", {
      className: "claim-theme-row"
    }, /*#__PURE__*/React.createElement("span", {
      className: "claim-theme-label"
      ...
      value: ""
    }, "- Player's choice -"), THEMES.map(t => /*#__PURE__*/React.createElement("option", {
      key: t.id,
      value: t.id
    }, t.label)))), pc ? /*#__PURE__*/React.createElement("div", {
    ...
    }), !claim.spectator && /*#__PURE__*/React.createElement(React.Fragment, null, (claim.controlledPcs || []).map(cid => {
      ...
      return /*#__PURE__*/React.createElement("div", {
        ...
        className: "familiar-badge loan-badge"
      }, "LOAN"), /*#__PURE__*/React.createElement("button", {
        className: "btn sm ghost",
        title: "Revoke control",
        onClick: () => dispatch({
          type: 'REVOKE_PC_CONTROL',
          peerId,
          entityId: cid
        })
      }, "×"));
    }), /*#__PURE__*/React.createElement("select", {
      className: "claim-loan-select",
      value: "",
      onChange: e => {
        if (e.target.value) {
          dispatch({
            type: 'GRANT_PC_CONTROL',
            peerId,
            entityId: e.target.value
          });
          toast('Temporary control granted');
        }
      }
    }, /*#__PURE__*/React.createElement("option", {
      value: ""
    }, "+ Lend a character to control…"), Object.values(state.entities).filter(e => e.type === 'PC' && e.id !== claim.pc && !(claim.controlledPcs || []).includes(e.id)).map(e => /*#__PURE__*/React.createElement("option", {
      key: e.id,
      value: e.id
    }, e.name)))));
  }))));
...
function TokenContextMenu({
  ...
  return /*#__PURE__*/React.createElement("div", {
  ...
  }), /*#__PURE__*/React.createElement("span", null, entity?.name || 'Token'), entity && /*#__PURE__*/React.createElement("span", {
    className: "token-ctx-hp"
  }, entity.hp.current, "/", entity.hp.max)), entity && /*#__PURE__*/React.createElement("div", {
    className: "token-ctx-hprow"
  }, /*#__PURE__*/React.createElement("button", {
    className: "ctx-hp-btn dmg",
    onClick: () => applyHp(-1),
    title: "Apply damage"
  }, "− Dmg"), /*#__PURE__*/React.createElement("input", {
    ...
    className: "ctx-hp-btn heal",
    onClick: () => applyHp(+1),
    title: "Apply healing"
  }, "+ Heal")), entity && /*#__PURE__*/React.createElement("div", {
    className: "token-ctx-conds"
  }, CTX_QUICK_CONDITIONS.map(c => /*#__PURE__*/React.createElement("button", {
    key: c,
    className: `ctx-cond ${conditions.includes(c) ? 'active' : ''}`,
    style: conditions.includes(c) ? {
      '--cc': CONDITION_COLORS[c] || '#9b6ac4'
    } : undefined,
    onClick: () => toggle(c),
    ...
    className: "ctx-icon"
  }, token.visible ? '🕶' : '👁'), token.visible ? 'Hide from players' : 'Reveal to players'), /*#__PURE__*/React.createElement("button", {
    className: "token-ctx-item",
    onClick: () => {
      onOpenDetails?.(token.id);
      onClose();
    }
  }, /*#__PURE__*/React.createElement("span", {
    className: "ctx-icon"
  }, "◈"), " Open details"), entity && onOpenSheet && /*#__PURE__*/React.createElement("button", {
    className: "token-ctx-item",
    onClick: () => {
      onOpenSheet(entity.id);
      onClose();
    }
  }, /*#__PURE__*/React.createElement("span", {
    className: "ctx-icon"
  }, "📜"), " Character sheet"), entity && /*#__PURE__*/React.createElement("button", {
    className: "token-ctx-item",
    onClick: () => {
      onEditEntity?.(entity);
      onClose();
...
function HazardResolutionScreen({
  state,
  dispatch,
  toast
}) {
  ...
  const applyEvent = ev => {
    ...
    dispatch({
      ...
      message: {
        ...
        senderId: 'dm',
        senderName: '⚠ Hazard',
        text: `${ev.entityName} takes ${dmg} ${ev.dmgType} from ${ev.hazardKind}${ev.reason === 'entry' ? ' (on entry)' : ' (turn start)'}${m !== 'normal' ? ` [${m}]` : ''}.`,
        whisperTo: null,
        whisperToName: null
      }
    });
    dispatch({
      type: 'HAZARD_PENDING_REMOVE',
      id: ev.id
    });
    toast(`${ev.entityName}: ${dmg} ${ev.dmgType}`, 'warning');
  };
  const skipEvent = ev => dispatch({
    type: 'HAZARD_PENDING_REMOVE',
    id: ev.id
  });
  return ReactDOM.createPortal(/*#__PURE__*/React.createElement("div", {
    className: "hazres-overlay"
  }, /*#__PURE__*/React.createElement("div", {
    className: "hazres-card"
  }, /*#__PURE__*/React.createElement("div", {
    className: "hazres-head"
  }, /*#__PURE__*/React.createElement("span", {
    className: "hazres-title"
  }, "⚠ Hazard damage"), /*#__PURE__*/React.createElement("span", {
    className: "hazres-count"
  }, pending.length, " pending")), /*#__PURE__*/React.createElement("div", {
    className: "hazres-sub"
  }, "Resolve each token individually. Set weak / resist / immune, then apply or skip."), /*#__PURE__*/React.createElement("div", {
    className: "hazres-list"
  }, pending.map(ev => {
    const m = mults[ev.id] || 'normal';
    const dmg = hazardFinalDamage(ev.rolled, m);
    return /*#__PURE__*/React.createElement("div", {
      key: ev.id,
      className: "hazres-row"
    }, /*#__PURE__*/React.createElement("div", {
      className: "hazres-row-top"
    }, /*#__PURE__*/React.createElement("span", {
      className: "hazres-who"
    }, /*#__PURE__*/React.createElement("span", {
      className: "hazres-dot",
      style: {
        background: ev.entityColor
      }
    }), ev.entityName), /*#__PURE__*/React.createElement("span", {
      className: "hazres-meta"
    }, HAZARD_GLYPHS[ev.hazardKind] || '⚠', " ", ev.hazardKind, " · ", ev.reason === 'entry' ? 'entered' : 'turn start')), /*#__PURE__*/React.createElement("div", {
      className: "hazres-dmg"
    }, "rolled ", /*#__PURE__*/React.createElement("b", {
      className: "acin-glow-num"
    }, ev.rolled), " ", ev.dmgType, " ", /*#__PURE__*/React.createElement("span", {
      className: "hazres-arrow"
    }, "→"), " ", /*#__PURE__*/React.createElement("b", {
      className: "hazres-final"
    }, dmg)), /*#__PURE__*/React.createElement("div", {
      className: "hazres-wri"
    }, [['normal', 'Normal'], ['weak', 'Weak'], ['resist', 'Resist'], ['immune', 'Immune']].map(([v, l]) => /*#__PURE__*/React.createElement("button", {
      key: v,
      className: `acin-res-opt ${v} ${m === v ? 'on' : ''}`,
      onClick: () => setMults(s => ({
        ...s,
        [ev.id]: v
      }))
    }, l))), /*#__PURE__*/React.createElement("div", {
      className: "hazres-actions"
    }, /*#__PURE__*/React.createElement("button", {
      className: "btn sm primary",
      onClick: () => applyEvent(ev)
    }, "Apply ", dmg), /*#__PURE__*/React.createElement("button", {
      className: "btn sm ghost",
      onClick: () => skipEvent(ev)
    }, "Skip")));
  })), /*#__PURE__*/React.createElement("div", {
    className: "hazres-foot"
  }, /*#__PURE__*/React.createElement("button", {
    className: "btn sm",
    onClick: () => pending.forEach(applyEvent)
  }, "Apply all"), /*#__PURE__*/React.createElement("button", {
    className: "btn sm ghost danger",
    onClick: () => dispatch({
      type: 'HAZARD_PENDING_CLEAR'
    })
  }, "Skip all")))), document.body);
}
function DMInterface({
  ...
  const toggleLayer = k => setHiddenLayers(prev => {
  ...
  const sendChat = useCallback(({
    ...
    dispatch({
      ...
      message: {
        ...
        ts: Date.now(),
        senderId: DM_KEY,
        senderName: senderName || 'Dungeon Master',
        text,
        whisperTo: whisperTo || null,
        whisperToName: whisperToName || null
      }
    });
  }, [dispatch]);
  ...
  const tokenMove = (tokenId, x, y) => {
    ...
    // player's. The MapCanvas drag was already clamped to the remaining range.
    {
      ...
      const init = state.initiative,
        mv = state.movement;
      const isActiveMonster = init?.active && ent && ent.type !== 'PC' && ent.type !== 'Familiar' && init.entries[init.turn]?.entityId === ent.id && mv?.entityId === ent.id;
      if (isActiveMonster) {
        const movedFt = Math.hypot(x - draggedTok.x, y - draggedTok.y) / PX_PER_FOOT;
        if (movedFt > 0.01) dispatch({
          type: 'MOVEMENT_USE',
          addFt: movedFt
        });
      }
  ...
  return /*#__PURE__*/React.createElement("div", {
  ...
  }, "🕶 Hide All"), /*#__PURE__*/React.createElement("button", {
    className: `btn ${hiddenLayers.size > 0 ? 'active' : ''}`,
    onClick: () => setShowLayers(s => !s),
    title: "Show or hide whole map layers on your screen"
  }, "▤ Layers", hiddenLayers.size > 0 ? ` (${hiddenLayers.size})` : ''), /*#__PURE__*/React.createElement(ToolsMenu, {
    isDM: true,
  ...
  }, "⛭ Long Rest"), /*#__PURE__*/React.createElement("button", {
    className: "btn",
    onClick: shortRestAll,
    title: "Restore half of max HP to all party members"
  }, "◑ Short Rest"), /*#__PURE__*/React.createElement("div", {
  ...
  }, "· ", peerCount, " ", peerCount === 1 ? 'player' : 'players')), /*#__PURE__*/React.createElement("button", {
    className: "btn",
    ...
    className: `main ${dmSbCollapsed ? 'dm-sb-collapsed' : ''}`
  }, /*#__PURE__*/React.createElement("button", {
    className: `panel-toggle left ${dmSbCollapsed ? 'closed' : ''}`,
    title: dmSbCollapsed ? 'Show panel' : 'Hide panel',
    onClick: () => setDmSbCollapsed(c => !c)
  }, dmSbCollapsed ? '›' : '‹'), /*#__PURE__*/React.createElement("div", {
    className: "sidebar"
    ...
    onLayerTransform: (id, patch) => dispatch({
      type: 'LAYER_UPDATE',
      mapId: state.currentMapId,
      id,
      patch
    ...
    x: cursorPos.x,
    y: cursorPos.y
  }), /*#__PURE__*/React.createElement(ActiveToolBanner, {
    measureMode: measureMode,
    placingReminder: placingReminder,
    placingBlock: placingBlock,
    placingFreeBlock: placingFreeBlock,
    ...
    toast: toast
  }), showLayers && /*#__PURE__*/React.createElement(FloatPanel, {
    style: {
      right: 16,
      top: 80,
      width: 260
    ...
    className: "float-panel-header"
  }, /*#__PURE__*/React.createElement("span", null, "▤ Map Layers"), /*#__PURE__*/React.createElement("button", {
    className: "close-x",
  ...
  }, "Hide whole layers on your own screen to reduce clutter. This does not change what players see."), [['tokens', '♟ Tokens'], ['hazards', '⚠ Hazards'], ['walls', '▦ Walls & blocks'], ['drawings', '✎ Drawings'], ['reminders', '📌 Pins & reminders']].map(([k, label]) => /*#__PURE__*/React.createElement("label", {
    key: k,
    className: "layer-row"
  }, /*#__PURE__*/React.createElement("input", {
    type: "checkbox",
    checked: !hiddenLayers.has(k),
    onChange: () => toggleLayer(k)
  }), /*#__PURE__*/React.createElement("span", null, label), /*#__PURE__*/React.createElement("span", {
    className: "layer-state"
  }, hiddenLayers.has(k) ? 'hidden' : 'shown'))))), showClaims && /*#__PURE__*/React.createElement(DMClaimsPanel, {
    state: state,
    dispatch: dispatch,
    sync: sync,
    onClose: () => setShowClaims(false),
    toast: toast
  }), /*#__PURE__*/React.createElement(DMRequestsOverlay, {
    state: state,
    dispatch: dispatch,
    toast: toast,
    onApproveJoin: pid => {
    ...
    onRejectJoin: pid => {
      heldPeersRef.current.delete(pid);
      try {
        syncRef.current?.connections.get(pid)?.send({
          type: 'kicked',
          reason: 'The DM did not admit you to this session.'
        });
      } catch {}
      toast('Join request declined', 'info');
    }
  }), /*#__PURE__*/React.createElement(AttackCinematicLayer, {
    state: state,
    dispatch: dispatch,
    isDM: true,
    toast: toast,
    physicalDice: settings?.physicalDice === true
  }), /*#__PURE__*/React.createElement(HazardResolutionScreen, {
    state: state,
    dispatch: dispatch,
    toast: toast
    ...
    isDM: true,
    myPeerId: DM_KEY,
    defaultName: "Dungeon Master",
    tokensOnMap: chatTokenNames,
  ...
  }), selectedToken && selectedTokenEntity && /*#__PURE__*/React.createElement(RadialTokenMenu, {
    state: state,
    token: selectedToken,
    ...
    if (!t) return null;
    const ent = state.entities[t.entityId];
    return /*#__PURE__*/React.createElement(TokenContextMenu, {
      token: t,
      entity: ent,
      x: ctxMenu.x,
      y: ctxMenu.y,
  ...
  }, "Delete Entity")))))), sheetEntityId && state.entities[sheetEntityId] && /*#__PURE__*/React.createElement(DMSheetModal, {
    state: state,
...
function PartySidebar({
  ...
  const partyMembers = Object.values(state.entities).filter(e => (e.type === 'PC' || e.type === 'Familiar') && (entityIdsOnMap.has(e.id) || isOwnedId(e.id)))
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    }, "FAM"), isYou && !onMap && /*#__PURE__*/React.createElement("span", {
      className: "offmap-badge",
      title: "Not on this map yet"
    ...
    }, obfuscate ? /*#__PURE__*/React.createElement("span", {
      className: `party-hp-text mono ${label.cls}`,
      title: "HP hidden by DM"
    }, label.text) : /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
...
function CompactSheet({
  ...
  return /*#__PURE__*/React.createElement("div", {
    className: "csheet-compact"
  }, /*#__PURE__*/React.createElement("div", {
    className: "csc-name"
  }, entity.name), /*#__PURE__*/React.createElement("div", {
    className: "csc-sub"
  }, "L", entity.level || 1, " ", entity.class || '', entity.race ? ` · ${entity.race}` : ''), /*#__PURE__*/React.createElement("div", {
    className: "csc-hp"
  }, /*#__PURE__*/React.createElement("div", {
    className: "csc-hp-top"
  }, /*#__PURE__*/React.createElement("span", {
    className: "csc-hp-lbl"
  }, "HIT POINTS"), /*#__PURE__*/React.createElement("span", {
    className: "csc-hp-num mono"
  }, hp.current, /*#__PURE__*/React.createElement("span", {
    className: "csc-hp-max"
  }, "/", hp.max))), /*#__PURE__*/React.createElement("div", {
    className: "csc-hp-bar"
  }, /*#__PURE__*/React.createElement("div", {
    className: `csc-hp-fill ${hpClass}`,
    style: {
      width: `${hpPct}%`
    }
  }))), /*#__PURE__*/React.createElement("div", {
    className: "csc-vitals"
  }, /*#__PURE__*/React.createElement("div", {
    className: "csc-vital"
  }, /*#__PURE__*/React.createElement("span", {
    className: "csc-vital-lbl"
  }, "AC"), /*#__PURE__*/React.createElement("span", {
    className: "csc-vital-val"
  }, entity.ac ?? '-')), /*#__PURE__*/React.createElement("div", {
    className: "csc-vital"
  }, /*#__PURE__*/React.createElement("span", {
    className: "csc-vital-lbl"
  }, "INIT"), /*#__PURE__*/React.createElement("span", {
    className: "csc-vital-val"
  }, fmt(initB))), /*#__PURE__*/React.createElement("div", {
    className: "csc-vital"
  }, /*#__PURE__*/React.createElement("span", {
    className: "csc-vital-lbl"
  }, "SPEED"), /*#__PURE__*/React.createElement("span", {
    className: "csc-vital-val"
  }, walkSpeedOf(entity)))), /*#__PURE__*/React.createElement("div", {
    className: "csc-ability-grid"
  }, ABILITIES.map(a => {
    ...
    return /*#__PURE__*/React.createElement("div", {
      ...
      className: "csc-ab-name"
    }, a), /*#__PURE__*/React.createElement("span", {
      className: "csc-ab-mod"
    }, fmt(abilityMod(score))), /*#__PURE__*/React.createElement("span", {
      className: "csc-ab-score"
    }, score));
  })), extraSpeeds.length > 0 && /*#__PURE__*/React.createElement("div", {
    className: "csc-speeds"
  }, extraSpeeds.map(([lbl, v]) => /*#__PURE__*/React.createElement("span", {
    key: lbl,
    className: "csc-speed-chip"
  }, lbl, " ", v))), (entity.conditions || []).length > 0 && /*#__PURE__*/React.createElement("div", {
    className: "csc-conditions"
  }, entity.conditions.map(c => /*#__PURE__*/React.createElement("span", {
    key: c,
    className: "party-cond-pill",
    style: {
      background: CONDITION_COLORS[c] || '#555'
    }
  }, c))), onOpenFull && /*#__PURE__*/React.createElement("button", {
    className: "btn sm csc-full-btn",
    onClick: onOpenFull
  }, "◈ Open full sheet"));
}
function RevealedMonstersSidebar({
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    className: "sidebar-section"
  }, setView ? /*#__PURE__*/React.createElement("div", {
    className: "dock-tabs"
  }, /*#__PURE__*/React.createElement("button", {
    className: `dock-tab ${view === 'revealed' ? 'active' : ''}`,
    onClick: () => setView('revealed')
  }, "❖ Foes"), /*#__PURE__*/React.createElement("button", {
    className: `dock-tab ${view === 'sheet' ? 'active' : ''}`,
    onClick: () => setView('sheet')
  }, "📜 Sheet"), /*#__PURE__*/React.createElement("button", {
    className: `dock-tab ${view === 'chat' ? 'active' : ''}`,
    onClick: () => setView('chat')
  }, "✦ Chat")) : /*#__PURE__*/React.createElement("div", {
    className: "sidebar-title"
  }, /*#__PURE__*/React.createElement("span", null, "❖ Revealed"))), view === 'sheet' ? /*#__PURE__*/React.createElement("div", {
    className: "sidebar-section grow dock-sheet-scroll",
    style: {
      overflow: 'auto',
      minHeight: 0
    }
  }, sheetNode || /*#__PURE__*/React.createElement("div", {
    className: "empty-state"
  }, /*#__PURE__*/React.createElement("span", {
    className: "glyph"
  }, "📜"), "No character to show.")) : view === 'chat' ? /*#__PURE__*/React.createElement("div", {
    className: "sidebar-section grow",
    style: {
      minHeight: 0,
      display: 'flex',
  ...
  }, chatNode || /*#__PURE__*/React.createElement("div", {
    className: "empty-state"
  }, /*#__PURE__*/React.createElement("span", {
    className: "glyph"
  }, "✦"), "Chat unavailable.")) : /*#__PURE__*/React.createElement("div", {
    className: "sidebar-section grow"
  }, /*#__PURE__*/React.createElement("div", {
    ...
    return /*#__PURE__*/React.createElement("div", {
      ...
      return /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("button", {
        className: "revealed-desc-toggle",
        onClick: () => toggleDesc(e.id),
        "aria-expanded": descOpen
      }, /*#__PURE__*/React.createElement("span", {
        className: "revealed-desc-chevron"
      }, descOpen ? '▾' : '▸'), descOpen ? 'Hide description' : 'Description'), descOpen && (hasDesc ? /*#__PURE__*/React.createElement("div", {
    ...
    })(), e.conditions.length > 0 && /*#__PURE__*/React.createElement("div", {
...
function PlayerInterface({
  ...
  useEffect(() => {
    for (const r of Object.values(state.pendingRequests || {})) {
      ...
      if ((r.status === 'accepted' || r.status === 'rejected') && !notifiedReqs.current.has(r.id + r.status)) {
        ...
        const what = r.kind === 'new_character' ? 'New character' : r.kind === 'level_change' ? `Level → ${r.payload?.to}` : `${String(r.payload?.stat || '').toUpperCase()} → ${r.payload?.to}`;
        toast(r.status === 'accepted' ? `✓ DM approved: ${what}` : `✗ DM declined: ${what}`, r.status === 'accepted' ? 'success' : 'error');
      }
    }
  ...
  useEffect(() => {
    ...
    if (claimedEntityId) {
      ...
      setCreatingPayload(null);
      return;
    }
    const iv = setInterval(() => {
      if (creatingPayload) playerActionSender({
        type: 'create_and_claim_pc',
        payload: creatingPayload
      });
  ...
  const newCharControl = !claimedEntityId ? newCharStatus === 'pending' ? /*#__PURE__*/React.createElement("span", {
    className: "mono",
    style: {
      fontSize: 12,
      color: 'var(--ink-dim)'
    }
  }, "⏳ Awaiting DM…") : newCharGranted ? /*#__PURE__*/React.createElement("button", {
    className: "btn sm primary",
    onClick: () => setBuilderDismissed(false)
  ...
  return /*#__PURE__*/React.createElement("div", {
  ...
  }, state.obfuscateHp ? hpLabel(claimedEntity.hp.max > 0 ? claimedEntity.hp.current / claimedEntity.hp.max * 100 : 0).text + ' HP' : `${claimedEntity.hp.current}/${claimedEntity.hp.max} HP`), /*#__PURE__*/React.createElement("div", {
    className: "hud-sheet-tabs",
    title: "Open your character sheet"
  }, (claimedEntity.type === 'PC' ? [['core', '◆', 'Core'], ['combat', '⚔', 'Combat'], ['spells', '✦', 'Spells'], ['gear', '🜚', 'Gear'], ['story', '❧', 'Story']] : [['core', '◆', 'Core'], ['combat', '⚔', 'Combat'], ['gear', '🜚', 'Gear'], ['story', '❧', 'Story']]).map(([key, icon, label]) => /*#__PURE__*/React.createElement("button", {
    key: key,
    className: "btn sm hud-sheet-tab",
    onClick: () => {
      setSheetTab(key);
      setShowSheet(true);
    },
    ...
    className: "hud-sheet-icon"
  }, icon), " ", label))), (availableFamiliars.length > 0 || (myClaim.familiars || []).length > 0) && /*#__PURE__*/React.createElement("button", {
    className: "btn sm",
    ...
    className: "btn sm",
    onClick: () => setShowClaim(true)
  }, "⚐ Claim Character"), newCharControl) : /*#__PURE__*/React.createElement("div", {
    style: {
      display: 'flex',
      alignItems: 'center',
      gap: 8
    ...
    className: `main player-view ${pLeftCollapsed ? 'pleft-collapsed' : ''} ${pRightCollapsed ? 'pright-collapsed' : ''}`
  }, /*#__PURE__*/React.createElement("button", {
    className: `panel-toggle pleft ${pLeftCollapsed ? 'closed' : ''}`,
    title: pLeftCollapsed ? 'Show party' : 'Hide party',
    onClick: () => setPLeftCollapsed(c => !c)
  }, pLeftCollapsed ? '›' : '‹'), /*#__PURE__*/React.createElement("button", {
    className: `panel-toggle pright ${pRightCollapsed ? 'closed' : ''}`,
    title: pRightCollapsed ? 'Show revealed' : 'Hide revealed',
    onClick: () => setPRightCollapsed(c => !c)
  }, pRightCollapsed ? '‹' : '›'), /*#__PURE__*/React.createElement(PartySidebar, {
    state: state,
    ...
    onDrawingDelete: id => playerActionSender({
      type: 'drawing_delete',
      payload: {
        mapId: currentMapId,
        id
    ...
    onLayerTransform: (id, patch) => playerActionSender({
    ...
    className: "glyph"
  }, "⚑"), "DM-controlled view · ", currentMap?.name)), syncStatus !== 'live' && /*#__PURE__*/React.createElement("div", {
    className: "canvas-overlay bottom-center"
  }, /*#__PURE__*/React.createElement("div", {
    className: "forced-view-banner"
  ...
  }), showBuilderOverlay && /*#__PURE__*/React.createElement("div", {
    className: "onboarding-overlay"
  }, /*#__PURE__*/React.createElement("div", {
    className: "onboarding-card"
  }, /*#__PURE__*/React.createElement("div", {
    className: "onboarding-title"
  }, "Create your character"), /*#__PURE__*/React.createElement("div", {
    className: "onboarding-subtitle"
  }, "The DM approved your request. Build your hero below."), /*#__PURE__*/React.createElement(NewCharacterBuilder, {
    playerName: playerName,
    busy: creatingChar,
    onCancel: () => setBuilderDismissed(true),
    onCreate: createNewCharacter,
    onRoll: text => playerActionSender({
      type: 'creation_roll',
      payload: {
        text
      }
  ...
  }, "You can reopen this from the \"Create character\" button up top."))), showClaim && /*#__PURE__*/React.createElement("div", {
    className: "modal-overlay",
  ...
  }, "Level ", e.level, " ", e.class, " · ", e.hp.max, " HP · AC ", e.ac)), /*#__PURE__*/React.createElement("button", {
    className: "btn primary sm"
  ...
  }, /*#__PURE__*/React.createElement("label", null, "New character"), newCharStatus === 'pending' ? /*#__PURE__*/React.createElement("div", {
    className: "grant-status pending"
  }, "⏳ Waiting for the DM to approve your new character…") : newCharGranted ? /*#__PURE__*/React.createElement("button", {
    className: "btn primary",
    style: {
      width: '100%'
    },
  ...
  }, "✨ Build your approved character") : /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("button", {
    className: "btn",
    style: {
      width: '100%'
    },
  ...
  }, "✨ Request a new character"), newCharStatus === 'rejected' && /*#__PURE__*/React.createElement("div", {
    className: "settings-hint",
    style: {
      marginTop: 6
    }
  }, "The DM declined last time. You can ask again.")))))), showSheet && /*#__PURE__*/React.createElement(EditMySheetModal, {
    state: state,
    myPeerId: myPeerId,
    claim: myClaim,
    playerActionSender: playerActionSender,
    ...
    sheetNode: claimedEntity ? /*#__PURE__*/React.createElement("div", {
      style: {
        padding: '8px 8px 20px'
      }
    }, /*#__PURE__*/React.createElement(CompactSheet, {
      entity: claimedEntity,
      onOpenFull: () => setShowSheet(true)
    })) : null,
    chatNode: /*#__PURE__*/React.createElement(ChatPanel, {
      messages: state.chat || [],
      isDM: false,
      myPeerId: myPeerId,
      embedded: true,
...
const PLAYER_FIELD_WHITELIST = new Set(['name', 'color', 'ac', 'speed', 'initBonus', 'passivePerception', 'class', 'playerName', 'notes', 'playerDescription', 'imageUrl', 'faction', 'role', 'darkvision', 'lightRadius',
// v7.6: expanded character-sheet fields
'money', 'xp', 'proficiencyBonus', 'hitDice', 'race', 'background', 'alignment', 'attacks', 'spells', 'features', 'proficiencies', 'inventory', 'traits', 'ideals', 'bonds', 'flaws', 'backstory', 'weapons',
// v7.9: structured weapons (players manage their own)
'armor', 'shield', 'handsTotal',
// v8.8: equipment (armour, shields, hands)
...
function Session({
  ...
  useEffect(() => {
    ...
    const sync = new SyncManager({
      ...
      onAwaitingApproval: () => {
        if (auth.mode === 'player') setAwaitingApproval(true);
      },
      ...
      onPlayerHello: (hello, newPeerId) => {
        ...
        if (settingsRef.current?.approveNewPlayers === true && !isReconnect && !approvedPeersRef.current.has(hello.playerId)) {
          heldPeersRef.current.set(newPeerId, {
          ...
          try {
            syncRef.current?.connections.get(newPeerId)?.send({
    ...
    for (const pid of Array.from(heldPeersRef.current.keys())) {
      ...
      if (!stillPending) {
        ...
        try {
          syncRef.current?.connections.get(pid)?.send({
            type: 'kicked',
            reason: 'Your request to join was not approved in time.'
          });
        } catch {}
      }
    }
    ...
    for (const h of hits) {
      ...
      dispatch({
        ...
        event: {
          ...
          entityId: activeId,
          entityName: ent.name,
          entityColor: ent.color || '#888',
          tokenId: tok.id,
          hazardId: h.id,
          hazardKind: h.hazardKind,
          dmgType: h.damage.type || h.hazardKind,
          rolled,
          reason: 'turn',
          ts: Date.now()
        }
  ...
  const handlePlayerAction = useCallback((action, peerId) => {
    ...
    try {
      ...
      return;
    } // unserializable / circular - reject
      ...
      case 'submit_request':
        {
          ...
          if (ACTION_KINDS.includes(kind)) {
            ...
            if (kind === 'apply_condition') {
              ...
              payload = {
                ...
                targetTokenId: data0.targetTokenId || null,
                condition: cond
              };
            } else {
              ...
              const mkComp = c => {
                ...
                return {
                  dice,
                  diceSum: dice.reduce((a, d) => a + d.result, 0),
              ...
              const comps = Array.isArray(data0.components) && data0.components.length ? data0.components.slice(0, 12).map(mkComp) : [mkComp({
                dice: data0.dice,
                modifier: data0.modifier,
                flat: data0.flat,
                type: data0.damageType
              })];
              const cl20 = v => Math.max(1, Math.min(20, Math.round(Number(v)) || 1));
              const ADVS = ['normal', 'advantage', 'disadvantage'];
              let d20a, d20b, advMode;
              if (data0.d20a != null) {
                d20a = cl20(data0.d20a);
                d20b = data0.d20b != null ? cl20(data0.d20b) : d20a;
                advMode = ADVS.includes(data0.advMode) ? data0.advMode : 'normal';
              } else if (data0.toHitRoll != null) {
                d20a = cl20(data0.toHitRoll);
                d20b = d20a;
                advMode = 'normal';
              } else {
                d20a = null;
              ...
              if (e0 && typeof e0 === 'object' && CONDITIONS.includes(e0.condition)) {
                ...
                if (e0.save && ABILITIES.includes(String(e0.save.ability))) {
                  save = {
                    ability: String(e0.save.ability),
                    dc: Math.max(1, Math.min(40, Math.round(Number(e0.save.dc)) || 10))
                  };
                }
                let contest = null;
              ...
              payload = {
                ...
                advMode,
                effect,
                weaponName: String(data0.weaponName || '').slice(0, 40),
                attackName: String(data0.attackName || '').slice(0, 40)
              };
            }
            const req = {
              id: uid('req_'),
              peerId,
              playerName: displayNameForPeer(curr, peerId),
              kind,
              payload,
              ts: Date.now(),
              status: 'pending',
            ...
            if (kind === 'apply_damage' && !curr.activeAttack) {
              ...
              dispatch({
                ...
                attack: {
                  ...
                  targetColor: target.color || '#888',
                  targetImg: target.imageUrl || null,
                  targetAc: ac,
                  weaponName: payload.weaponName || '',
                  attackName: payload.attackName || '',
                  toHit: payload.toHit ?? null,
                  toHitRoll: payload.toHitRoll ?? null,
                  hit,
                  d20a: payload.d20a,
                  d20b: payload.d20b,
                  advMode: payload.advMode,
          ...
          const dup = Object.values(curr.pendingRequests || {}).some(r => r.peerId === peerId && r.kind === kind && r.status === 'pending');
          if (dup) return;
          ...
          if (kind === 'place_token') {
            ...
            data = {
              ...
              x,
              y,
              mapId: String(data.mapId || curr.currentMapId)
            };
          }
          const req = {
            id: uid('req_'),
      ...
      case 'creation_roll':
        {
          ...
          dispatch({
            ...
            message: {
              ...
              senderId: 'dm',
              senderName: '🎲 Creation',
              text: `${displayNameForPeer(curr, peerId)}: ${txt}`,
              whisperTo: 'dm',
              whisperToName: 'DM'
            }
          });
      ...
      case 'create_and_claim_pc':
        {
          ...
          const grant = Object.values(curr.pendingRequests || {}).find(r => r.peerId === peerId && r.kind === 'new_character' && r.status === 'accepted');
          if (!grant) {
            console.warn('[plagues-call] create_and_claim_pc without grant');
            return;
          }
          ...
          const speed = Number.isFinite(reqSpeed) && reqSpeed >= 0 && reqSpeed <= 200 ? reqSpeed : raceData?.speed || 30;
      ...
      case 'attack_save_roll':
        {
          ...
          dispatch({
            ...
            patch: {
              saveResult: {
                ability: atk.effect.save.ability,
                dc,
                d20a: sr.d20a,
                d20b: sr.d20b,
      ...
      case 'move_token':
        {
          ...
          {
            const entered = damagingHazardsAt(curr, token.mapId, nx, ny).filter(h => h.damage.onEntry && !pointInPoly(token.x, token.y, h.points));
      ...
      case 'patch_own_entity':
        {
          ...
          if (op === 'hp_adjust') {
            ...
            for (const gf of ['armor', 'shield', 'speeds']) {
              if (gf in patch) {
                try {
                  if (JSON.stringify(patch[gf] ?? null).length > 4000) delete patch[gf];
                } catch {
                  delete patch[gf];
                }
              }
      ...
      case 'dice_roll':
        {
          ...
          if (Array.isArray(e.groups)) {
            for (const g of e.groups) {
              ...
              const sanitized = trustEntered ? g.results.slice(0, 100).map(r => clamp(r | 0, 1, g.die | 0)) : rollN(g.die | 0, count);
              groups.push({
                die: g.die | 0,
                results: sanitized
              });
            ...
            if (dice.length > 0) {
              ...
              const results = trustEntered ? dice.map(d => clamp(d.result | 0, 1, d.die | 0)) : dice.map(d => 1 + Math.floor(Math.random() * (d.die | 0)));
              groups = [{
                die: sides,
                results
              }];
  ...
  return /*#__PURE__*/React.createElement(React.Fragment, null, awaitingApproval && /*#__PURE__*/React.createElement("div", {
    className: "onboarding-overlay",
    style: {
      zIndex: 9000
    }
  ...
  }, "Waiting to be admitted"), /*#__PURE__*/React.createElement("p", {
    style: {
      color: 'var(--ink-dim)',
      fontSize: 14,
      lineHeight: 1.5
    }
  }, "This table requires the DM to approve new players. Hold tight - you'll join automatically the moment they let you in."), /*#__PURE__*/React.createElement("div", {
    className: "spinner",
    style: {
      margin: '16px auto 0'
    }
  }))), /*#__PURE__*/React.createElement(PlayerInterface, {
    state: state,
    dispatch: dispatch,
    myPeerId: myPeerId,
